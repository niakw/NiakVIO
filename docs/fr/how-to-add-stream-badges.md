...
> [!TIP]
> Pour la majorité des utilisateurs, **Fusion v5** est le bon feed.
...
Utilisez-les uniquement si vous recherchez explicitement une variante liée au thème ; **Fusion v5 reste la configuration normale**.
...
5. Collez l'URL JSON Fusion v5 de NiakVIO.
...
7. Dans la page web ouverte sur votre téléphone, collez l'URL JSON Fusion v5 de NiakVIO.
...
> [!IMPORTANT]
> La TV conserve localement les règles déjà importées. Si une ancienne version du feed Fusion NiakVIO ne se met pas correctement à jour, supprimez l'ancien import puis ajoutez à nouveau l'URL **Fusion v5** actuelle.
...
- réimportez l'URL Fusion v5 actuelle ;
...
- `assets/stream-badges-fusion-v5.json` — feed recommandé multi-thème ;
- `assets/stream-badges-dark-v5.json` — fonds d'application sombres ;
- `assets/stream-badges-light-v5.json` — fonds d'application clairs ;
- `assets/stream-badges-transparent-v5.json` — artwork transparent.
