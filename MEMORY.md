...
## 2026-09-18 — authoritative continuation checkpoint: repair authority rewired, semantic regressions fixed, publication still blocked

- Verified public `main` remains **c02c9919c98dd8ff6729350f44a48f19a5cb571f**, root manifest **5.21.51**. `MEMORY.md` on public `main` is still stale (older 2026-09-16 / 5.21.48 checkpoint); do not treat main MEMORY as current until a clean repair PR is merged.
- Active repair work remains on **`fix/provider-activation-certification-v1`**. This branch contains substantial diagnostic/repair history and **must not be merged wholesale**. Final publication must be reconstructed as a clean net diff from current `main`.
- PR #122 head **4e9729d865f6a069bda567fbeac520d714bef4bd** remains historical evidence only and was never merged. Historical positive lane proof is evidence, not automatic certification for regenerated bundles.
- The lost repair authority is now physically re-wired in `provider-overrides.json` for AllAnime, AllWish, AnikotoTV v3, AnimeSama.co, AnimeVOSTFR, Coflix, Flemmix, MovieBox, Neko-Sama, Sekai, VidFast, VidLove, VoirAnime.rip, WookaFR and Yflix. Restored rows are deliberately reset to **`repair-reproof-required`** until exact regenerated bundle proof exists.
- Durable survival authority exists in `automation/provider-repair-survival-registry.json`, `scripts/check_provider_repair_survival.py` and `scripts/restore_provider_repair_survival_v1.py`. Missing required Lego is a hard failure after rematerialization/minimization.
- Important correction to the prior “consolidated runtime supersedes #122” assumption: comparison of the exact #122 script stack proved that current shared non-display V1 **does not fully supersede** the #122 V2/inline repairs. V2 adds direct Sibnet resolution and additional AnimeSama/Neko/VoirAnime/Sekai behavior; Sekai also had a distinct current-site inline-media runtime. Those V2/inline capabilities have therefore been restored and added to the survival registry instead of being silently discarded.
- Current non-display durable stack: AnimeSama.co = site runtime + provider-owned V1 + V2; AnimeVOSTFR = provider-owned V1; Coflix = provider-owned V1; Neko-Sama = base runtime + provider-owned V1 + V2; Sekai = provider-owned V1 + V2 + inline-media runtime; VoirAnime.rip = provider-owned V1 + V2.
- Six provider-owned non-display adapters originally used an invalid package import under clean materialization. They now import their sibling shared runtime through the materializer-supported patch-directory path.
- **HellMode S2E12 regression root cause confirmed:** shared DLE runtime parsed `season` but historically resolved title + episode without proving the season. French-Manga / VoirAnime.homes could therefore return an E12 from the wrong season. The shared DLE runtime is now season-aware, searches season-qualified titles, understands explicit season branches, and fails closed for season >1 unless the selected page/payload proves the requested season.
- Regression fixture is now **Hell Mode S02E12** in the Native trigger. New executable test proves S02E12 resolves to the S2 candidate and rejects an ambiguous seasonless E12 fallback.
- **VF/VOSTFR/Kurage presentation root cause confirmed:** current V24 Core presentation ignored `languageFallback` when no explicit stream language existed. V25 restores evidence-backed language extraction from structured metadata / explicit VF/VFF/VFQ/VOSTFR/VO/MULTI labels, does not invent VF merely because a provider is French, and preserves authoritative VO fallback for VO-only providers such as Kurage. Regression tests prove explicit VOSTFR, no fabricated French-Manga VF, and Kurage VO badge/description.
- Run **35287761940 / #11** passed the entire pre-materialization gate including repair survival, executable S02E12 identity test, V25 presentation tests, active-provider coverage and Native certification contracts. It then failed during rematerialization because French-Manga/VoirAnime.homes facade code still patched an obsolete textual DLE parser anchor. Both facades have now been rebased onto the season-aware parser anchor; this failure does not invalidate the S02E12 behavior test.
- Publication remains **blocked**. Mandatory sequence: run repair-survival restore -> pre-gates -> full rematerialization -> minimizer fixed point -> survival check -> version/projection sync -> pinned Hub46/integrity -> push regenerated generation -> exact-bundle targeted certification including Kurage and restored providers -> reconcile Native evidence. Only then build a clean main-based publication branch/PR.
- Do not claim any restored provider “delivered” until the exact post-rematerialization bundle survives the registry and has current positive/native evidence. Do not merge the diagnostic branch wholesale.
