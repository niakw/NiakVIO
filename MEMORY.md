...
### 2026-09-23 — Final private-guided Repair retry prepared after Learning contract correction

- The first end-to-end private-guided Repair run `35914883115` imported external Brain-LLM guidance successfully from orphan guidance commit `971a0b93f04384ce129f49cd172519eafbe8be6b`, but it still ran before the production filter for unexecuted `profile_unavailable` debt. It therefore accepted 0/9 repairs and converged all 9 targets back to Learning; this run is connection proof, not functional repair proof.
- Main now includes the generic debt correction (`c0abf0c...`), guidance drift preservation (`b7e3c58...`), targeted preflight optimization (`d9f0510...`), persisted advisor metadata (`2e51601...`) and control-plane non-regression no-op optimization (`090bbc1...`).
- Learning runs on the corrected HEAD initially failed only because `tests/brain_llm_advisor_execution_test.py` represented synthetic exhausted variants without `executionObserved=true`. That contradicted the new semantics and made the test report the synthetic debt as unexecuted. Commit `a2021d2f1248399298bdc4d40ca6205d25adeda6` marks those synthetic negative rows as executed/rejected, while preserving the separate rule that genuine `planned_profile_not_applicable_to_current_bytes` rows without execution proof cannot suppress Repair.
- Current persisted Repair queue remains exactly 9 providers: `4khdhub, allanime, anime-ultime, animesultra, animevostfr, mallumv, moviebox, vidfast, yflix`. The next run must use that exact queue, prove whether `llmAdvisorApplied/llmAdvisorRescue` is selected for private-informed providers, and accept nothing without current-byte playback, identity and non-regression evidence.
