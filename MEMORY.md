...
- Historical `assets/badge_catalog_v2_complete.json` and `assets/mapping_core_brain_ui_v2_complete.json` stay frozen in place. Explicit v4 snapshots mirror the de-facto v4 state, and v6 becomes the current catalogue/mapping. Current tooling resolves the highest versioned filename instead of hardcoding v2.
- Public StreamBadge v6 exposes eight compact global score badges: `S+`, `S`, `A+`, `A`, `B`, `C`, `D`, `E`. Numeric 0–100 remains detail text, not a badge explosion.
...
- The first v5 score snapshot is preserved frozen after its regex escaping validation failure; it is not rewritten. v6 corrects only the score-pattern escaping and becomes the active version, demonstrating the immutable-version rule in practice.
...
- Current provider authority/lifecycle now matches the manual evidence supplied for the ambiguous catalogue cases:
  - AnimeVOSTFR: current terminal is `https://animevostfr.org`; stale `v2.animevostfr.org` remains a redirect seed only. Domain Refresh now also migrates top-level provider Bloc site-root options when a terminal rotates.
...
- Domain/address authority corrections from the user's manual checks are now encoded: AnimeVOSTFR terminal is `https://animevostfr.org`; generic Domain Refresh migrates provider Bloc top-level site roots when the terminal rotates (`d484ebbc54d8` + `34b90104ab4a`), so a stale Bloc `base/site/referer` cannot keep using `v2`. MoviesMod retains `https://modlist.in/?type=hollywood` as authoritative hub and `https://moviesmod.ai.in` as current site.
...
- AnimeVOSTFR authority is now coherent with the manual address correction: registry direct/current = `https://animevostfr.org/`; `v2.animevostfr.org` is only a redirect seed/history; provider override Bloc option `base` and the runtime module default both use `https://animevostfr.org`. `d484ebbc54d8` adds the generic fix: Domain Refresh now migrates top-level provider-Bloc site roots (`base/site/referer/origin`) when they point at a terminal that Domain has just replaced, while deliberately leaving mirror arrays intact. `34b90104ab4a` locks this behavior.
...
- `566fae3b9448` changes backend authority to explicit provider execution fields only (`official_api`, fixed/api recipe backend fields, provider Bloc `api/db/api_base/apiBase/backend/endpoint`) and excludes `api.themoviedb.org`, `v3-cinemeta.strem.io`, `arm.haglund.dev`; `fallbackBases` are no longer backend authority. `7da36921377b` locks both exclusions.
...
- `008010714c54` aligns Flemmix structured runtime DATA with `.party`: `official_site`, search-plan base and runtime Bloc base now use `.party`; historical site-domain replacement maps terminate at `.party` and the current `.party` host is removed as a replacement source to avoid a cycle. `67116e9435d5` changes the Flemmix runtime default base to `.party`; contracts `1159df4ec6a8` and `6334d4dab2e9` were updated accordingly.
...
- `ec19c1cdb0ad` adds `DOMAIN_REFRESH_CURRENT_SCOPE_PROJECTION_DRIFT_V1`: after structured authority is persisted, domain projection drift is now checked across the full current provider scope, regardless of resolver outcome. The drift routine still compares only `officialSite`, `knownSite`, `officialHub`, and `domainSubstitutions`; it does not mutate structured authority and rebuilds only stale CONFIG Bloc bytes. `fba522ef5391` locks this current-scope behavior and forbids regression back to `resolved_provider_ids`-only drift.
...
3. provider-owned `PROVIDER.*` Bloc;
...
- **published Provider JS contains only common ProviderBase + structured DATA/CONFIG + managed Provider/Core Bloc + envelope/minimizer-preserved structure**;
...
- provider behavior belongs in DATA or owned Provider Bloc;
- Core is provider-agnostic and each concern has one owner;
- managed Bloc uses `STARTFIX` / `CLOSEFIX` (+ `FIXDATA` when needed);
- Provider Bloc precedes Core boundary; Core Bloc follows it;
...
- `c8bc366`: active-96 Provider JS Bloc ownership test.
...
- `ec8867c` + `7296e20`: Python test bootstraps fixed for direct Core Bloc imports.
...
2. recompose **all 96 active bundles** from common Base + existing DATA/CONFIG + Provider/Core Bloc;
...
- Provider JS architecture remains: common ProviderBase + structured DATA/CONFIG + PROVIDER.* Bloc + CORE.* Bloc. Reconstruction/discovery tooling is never embedded as provider-specific runtime business logic.
...
- Documentation debt added by user: after runtime/publication stabilization, replace user-facing/documentation terminology **"Bloc"** with a generic term such as **block / Core block / Provider block**. Preserve marker semantics and compatibility while removing trademarked wording from docs/log labels/names where practical. Do this as a later clean step, not inside the current functional run.
...
- Cleanup reminder after final publication: replace the word `Bloc` in docs/workflow labels with a neutral term such as `block`/`bloc`.
...
- The same PlayIMDb acceptance job passed the full deterministic 96-provider gates: published CONFIG 96/96, Provider/Core Bloc ownership, global stream output guard 96/96, episodic identity/year regressions, global media-type resolver, dual IMDb/TMDB identity, stream presentation, and presentation pipeline.
...
- Candidate provider Bloc V1 exists in `scripts/provider_patches/nakios_livewire_runtime_v1.py`. First harness pass did not install into the lexical ProviderBase export; second pass proved the Bloc is now invoked (`server_accessible=true`, HTTP 200) but still returned zero streams. Continue debugging the resolver; do not mark Nakios green yet.
...
- Published HTML security was repaired at its actual Bloc sources; run 34713445212 passed the source+published HTML gate before stopping later on an unrelated stale playback test.
...
- VoirAnime anime regression was repaired with a current-site provider Bloc layered over the existing movie resolver: current series page -> requested chapter -> `LECTEUR` selector -> external iframe -> shared bounded direct-media crawler.
...
- **VidLove** is an evidence-backed exception: separate current A/B proved PR122 V1 + `api.vidlove.cc` FULL movie+tv while newer V2 was red. Main now contains only the proved V1 source-selector Bloc + API recipe + route_data_state=on. Provider Overrides Gate is green; final Verify & Publish remained blocked by unrelated Kehflix state and must be rerun after Kehflix converges.
...
- VidLove publication drift is confirmed: `provider-overrides.json` contains the current A/B-proved V1 `api.vidlove.cc` recipe and `PROVIDER.VIDLOVE.CURRENT.API.V1` Bloc, while the manifest-published bundle still has `apiRecipe=null` and no V1 Bloc marker. The override correction therefore was never materially published.
...
- The manifest-referenced VidLove bytes are the correct current runtime (`api.vidlove.cc`, `sources=moviebox`, V1 Bloc), so this is publication metadata drift rather than loss of the provider repair.
...
- The same targeted run gave sanitized failure chains:
  - **VoirAnime**: TMDB 200 -> `voir-anime.to/anime/jujutsu-kaisen/` 200, then no provider-local episode-host request; execution fell back to the older `voiranime.homes` path. This proved the V3 anime Bloc discarded episode links before the `?host=LECTEUR...` stage.
...
- Commit **1d0a1998c697375b2961722ca11465d2a9bc6303** goes beyond the provider-local domain correction and reconstructs AnimeSalt's current runtime chain as a NiakVIO-owned Bloc.
...
- The new Bloc is `scripts/provider_patches/animesalt_runtime_v1.py`, registered only for provider `animesalt` in `provider-overrides.json`; it uses Core/TMDB identity, exact season/episode selection, and keeps AnimeSalt's domain/route authority provider-local.
...
- Commits **23c93201d2eee1d68ad20bfa9fb87216e46de3dc**, **8b2a18964cc2cd0e1974cd09c5fb62c2cdaa09b6**, **f4797c13e35bce2659f2f44de86c78eec306c0b7** and **a99f123d11622830ab3e8290ddb976e986d8c63a** align Flemmix source authority, Bloc option/default base and contract to `flemmix.me`. Domain aliases remain provider-local and Domain Refresh remains responsible for projection.
...
- Commit **0ccccd75b2d179631b02ef0132275a0359579379** inserts domain projection + ProviderBase store + full Provider v3 materialization + CONFIG/Bloc validation before the non-regression quick-yield census. Commit **defec7ddb13c7dddb3cb74b4a557252aad9d0ded** makes this rematerialized-candidate requirement part of workflow ownership.
...
- 4KHDHub has a NiakVIO-owned provider Bloc for its current provider-local chain: `4khdhub.one` search -> movie/series detail -> download/episode item -> HubCloud/HubDrive -> workers/R2 media. HDHub4u remains a separate catalogue and is not a domain replacement. TMDB stays Core-owned; upstream provider JS/credential is not executed.
- UHDMovies provider Bloc is now `uhdmovies-search-gateway-driveseed-v2-final-url-first`: redirected download responses are checked for a final direct media URL before attempting to consume the response body. This targets the R13 trace where the chain already reached HTTP 206 Matroska media but returned zero provider streams.
- Papadustream/Vostfree helper contamination is addressed generically by the materializer helper-`apiRecipe` boundary; a fresh census must prove whether their provider-local Bloc then succeeds or exposes the next provider-local failure.
...
- Vostfree's provider Bloc was found to reject the Nuvio anime transport alias `tv` before any provider request. Commit **f36725da9f5c2002f00970c62bdf775bd223296b** maps tv transport back to anime provider-locally; `tests/vostfree_transport_contract_test.py` locks this.
- Vostfree and Flemmix still contained stale executable ARM recipes in DATA even though their Provider Blocs use provider-local site chains. Commit **974a4bc23b365dab2f68fd024b938c0c92435263** removes those stale recipes and records provider-local search plans (Vostfree DLE POST; Flemmix `/search?q={query}`).
...
- A brand-new MovieBox direct playback Bloc was not committed because the execution safety layer blocked creation of new code automating that external playback chain. MovieBox remains ZERO/quarantined; its historical route evidence is preserved in `automation/USER-PROVIDER-EVIDENCE-LEDGER.md`.
...
- Commit **490d71d22025331b666b38d34538c0f5ee02977a** bound provider `allanime` to this provider-local Bloc and reclassified its source runtime family as `catalogue-html`.
...
- Commit **4ebef0558e324e18c45fb9e71470ac7fd34f911c** makes `rebuild_provider_configs()` canonicalize CONFIG replacements through the safe provider minimizer, reject a non-fixed published input, and still enforce byte identity outside the CONFIG Bloc.
...
### 2026-09-19 — Provider Bloc HTML scanner hardening applied
...
### 2026-09-19 — Provider Bloc HTML hardening completed before release retry

- The accepted-release rematerialization exposed 9 provider-local Bloc sources that still used CodeQL-forbidden regex HTML stripping. Source-level fixes are now committed on main for **Flemmix, UHDMovies, MoviesHunt, 4KHDHub, AnimeSalt, AnimeSultra, AnimeVOSTFR, MoviesMod and VoirAnime.rip**. Each now uses a deterministic character scanner that skips tags and script/style blocks without regex tag stripping.
- The six other currently declared provider Bloc sources — **Coflix, French-Manga, Sekai, VoirAnime-Homes, AllAnime and Anime-Ultime** — were rechecked and contain none of the three forbidden HTML-filter patterns.
...
- **AllAnime NO PROOF root cause confirmed and patched:** current upstream episode-source lookup uses GET persisted GraphQL query, not the older POST `SOURCE_GQL`. Static decode of current upstream yielded API `https://api.allanime.day/api`, persisted hash `d405d0edd690624b66baba3068e0edc3ac90f1597d898a1ec8db4e5c43c00fec`, search Origin/Referer `https://allmanga.to`, source Referer `https://youtu-chan.com`, source Origin `https://allanime.day`. AES-CTR key contract remains SHA256(`Xot36i3lK3:v1`). Bloc migrated to the current GET persisted-query protocol and contract-tested. Live stream proof still pending.
...
- The push/PR trigger now watches `scripts/provider_patches/**` plus generic provider runtime/transport contract patterns and the shared evidence/fixture/DATA files. A new or repaired provider Bloc therefore automatically starts the targeted recovery engine without adding another workflow path entry.
...
- `tests/provider_allwish_runtime_contract_test.py` now applies the AllWish Bloc to the actual current AllWish bundle and runs `node --check` on that generated artifact. This tests the same insertion shape that the finalizer validates, not merely the standalone wrapper source.
...
- The new canonical byte validator exposed AllAnime as the first generated provider whose clean rematerialization became syntactically invalid. The current AllAnime Bloc itself parses successfully both standalone and when directly replacing its managed block in the already-published bundle.
- Root cause found in the generic compositor: `scripts/apply_provider_overrides.py` still unconditionally ran `provider_v3_minimizer.minimize_text()` on every complete v3 bundle, even though `materialize_provider_v3_all.py` and `materialize_provider_v3_one.py` already own an explicit final-stage-only minimizer gate. This created two minimization authorities and transformed provider Bloc before canonical byte validation.
- Fix: remove all minimization from `apply_provider_overrides.py`. It now only composes owned Provider/Core Bloc; optional minification remains exclusively behind `NIAKVIO_PROVIDER_V3_FINAL_MINIMIZE` in the materializers. `tests/provider_v3_final_stage_minimizer_gate_test.py` now forbids hidden minimizer imports/calls in the compositor.
...
- This validates the combined generic fixes: no hidden minimization in `apply_provider_overrides.py`, and canonical one-newline restoration when stripping/rebuilding the Core boundary. The AllAnime provider Bloc did not need to be weakened or disabled.
...
### 2026-09-19 — Animetsu current Gojo API Bloc reconstructed
...
- NiakVIO previously had no active Animetsu Provider Bloc; its clean v3 base still followed stale generic route evidence (including the old `omg10.com` path), which explains the fresh `provider_network_zero_result`.
- Added `scripts/provider_patches/animetsu_runtime_v1.py` and wired it as the sole provider Bloc. The runtime uses Core TMDB metadata first, requires Animation + original language ja/zh/ko before the first Animetsu request, performs current JSON search/title-year matching, computes absolute episodes from Core season counts when available, calls the decoded `oppai` routes, and emits the upstream proxy URLs for Core HLS validation. Upstream JS remains unembedded/unexecuted.
...
- Animetsu source DATA is likewise ahead of committed materialization: authoritative Provider DATA now binds `scripts/provider_patches/animetsu_runtime_v1.py` and `https://animetsu.live/v2/api`, while committed `provider-v3-materialization.json` still records only the old CONFIG fix and no provider Bloc. Its persisted targeted trace still shows legacy `omg10.com`, so it must not be judged until rematerialized bytes are published.
...
- Cross-checking authoritative `provider_patches[*].provider_lego_scripts` against committed `provider-v3-materialization.json.applied[].path` on current main found **19 providers** whose declared provider-local Bloc is absent from the published materialization record.
...
- Accepted-release finalizer run **35466561339** was triggered from current main to atomically reapply those Provider Bloc plus current DATA, regenerate content-addressed manifests/Hub46, validate generated bytes and publish only on the pinned SHA.
...
- Accepted-release finalizer run **35466659585** successfully reapplied durable overrides to all **44 active providers** and produced new content-addressed refs for all 44, proving the pending Provider Bloc set is composable.
...
- Recroisement : le source upstream Gowaru courant expose `BASE_URL=https://flemmix.me` et la page publique `flemmix.me` sert actuellement les catalogues films/séries. Le Bloc NiakVIO était incohérent : son parser était déjà celui du nouveau site (`film-en-streaming`, `serie-en-streaming`, server tabs) mais sa recherche restait l'ancien DLE `/index.php?...story=` sur `.cloud`.
...
- NiakVIO's published model had already observed `pengu.uk` and `stremio-moviebox-1.onrender.com`, but the active provider Bloc still forced `vidsrcme.ru`, which repeatedly reached `cloudorchestranova.com/embed/iframe_player/cache.php` with HTTP 400.
- The MovieBox Bloc now uses current Cinescrape/IMDb first and retains vidsrcme as compatibility fallback only. Core TMDB metadata/cache is the IMDb authority; no upstream JavaScript is executed.
...
- Added NiakVIO-owned provider Bloc `scripts/provider_patches/mallumv_runtime_v1.py` and bound it in `provider-overrides.json`. The runtime reconstructs only the observable chain TMDB title/year -> search -> exact movie -> confirm -> internal -> bounded terminal-media crawl; no upstream JavaScript is embedded or executed.
...
- Live provider probing was already concurrent (`audit_provider_quick_yield.py` ThreadPool and targeted recovery pool). The remaining scalability defect was repair orchestration: too many provider-specific Blocs/tests were being handled as isolated cases.
...
- Next Repair must verify `acceptedProgram` survives report/orchestrator transport, then reproduce a strict accepted Brain repair and persist the sanitized runtime program toward durable v3 DATA/Bloc only after current-byte playback+identity proof.
...
- Added `scripts/compile_brain_accepted_program_v3.py` in fail-closed mode. It consumes only a strict `acceptedProgram` captured by Deep and compiles representable request programs into existing `search_request_plan` / `provider_value_plan` Bloc. Only safe GET/POST, HTTP(S) origins, known header names, structured form/JSON bodies and provider-local `{id}/{slug}` route bindings are representable.
...
- AnimeVOSTFR current structured DATA is already correct (`official_site=https://animevostfr.org`, provider Bloc option `base=https://animevostfr.org`, old-host domain substitutions present), but the published provider bytes still contained one executable pre-Core occurrence inside `PROVIDER.ANIMEVOSTFR.RUNTIME.V1`: `base=https://v2.animevostfr.org`. This is confirmed projection/rematerialization drift, not merely stale registry metadata.
...
### 2026-09-22 — Positive replay executed; winning-trace memory + declared-Bloc publication drift fixed
...
- Generic correction: census now computes a provider-byte materialization scope from the commit diff plus any provider DATA changes produced by the domain-metadata reconciliation step. Provider-neutral changes use mode `none`; provider-local DATA/Bloc/manifest changes use `materialize_provider_v3_one.py` only for affected providers; common ProviderBase/Core/materializer changes use mode `all`.
- Unknown/unowned provider patch modules fail safe to full rematerialization. Top-level non-provider override changes also force full rematerialization. The selector has synthetic regression coverage for no-op, single-provider, provider-Bloc, manifest-row and global-input cases.
...
- Root publication-fingerprint defect: schema v2 treated all provider overrides, all provider Bloc scripts and the whole static-knowledge file as one global contract. One provider-local change therefore marked every provider input stale, contradicting incremental materialization.
- Schema v3 separates shared Core/materializer policy from per-provider policy. Provider patch/capability and declared provider Bloc source are provider-local; the follow-up correction also moves `automation/provider-v3-static-knowledge.json.providers[provider]` into that same local fingerprint while keeping top-level static policy global. Legacy schema-v2 calculation remains available only to verify pre-migration provenance.
...
### 2026-09-22 — Projection reconcile gate: published Bloc test was stale, not 43 providers
...
- The Bloc contract now derives the required media-type revision from the authoritative Core source and scopes portfolio completeness to active `providers/` rows. This validates source/published parity without freezing the test to an obsolete revision.
- Required proof: rerun Projection Reconcile from this commit; it must pass Bloc ownership, publish only the seven drifted providers plus metadata/manifests, then verify Flemmix current bytes and launch a fresh census.
...
- Generic pipeline correction: after provider reconstruction/reconciliation, publication fingerprints are reindexed metadata-only before the post-rebuild drift detector runs. The drift detector then judges the new byte/build pair, followed by CONFIG/static/Bloc/fixed-point audits.
...
- The failure exposed a real source contradiction rather than a test-only issue: upgrade_provider_v3_batch_routes_v2.py explicitly classifies fetch.php as stale/live-404 and consolidates the current learned route to /catalogue/?search={query}, but the provider Bloc still executed fetch.php.
...
- A projection-reconcile trigger is issued from the same commit. The owner workflow must detect exact drift, rebuild only providers whose accepted DATA/Bloc projection actually drifted, reindex `PROVENANCE.json` fingerprints, prove `reapply_published_overrides.py --check`, and dispatch exact-head census/Quick/security validation.
...
- The owned runtime Bloc `animesamaco_site_runtime_v1.py`, `upgrade_provider_v3_batch_routes_v2.py`, domain reconciliation tests and user-evidence crosscheck all agree that `/catalogue/?search={query}` is the executable catalogue route and `/template-php/defaut/fetch.php` is stale/candidate history.
...
- A second identical drift check runs immediately before census evidence persistence. Even if a future workflow path bypasses the scale decision, census cannot advance `main` while provider DATA/Bloc and published bytes are out of fixed point.
...
### 2026-09-25 01:29 Europe/Paris — Fast/LLM loop root causes isolated; Learning control-plane fix is PR #189

- Fast run `36072231705` executed the complete 14-provider repair cohort on source SHA `858a6ae...` and persisted evidence as `b347269...`. External Brain-LLM guidance imported all 14 rows with provider drift=0. The run finished in 314.28 s with 0 accepted repairs, 0 raw Lab acceptances, 7 sandbox diagnostic progressions, and all 14 providers handed back to Learning. No provider was promoted or published.
- Advisor observability is now causal rather than inferred: `provider-brain-repair-latest.json` persists `llmAdvisorExperiment` and its exact fingerprint. The current wave proves the imported experiments really executed; the absence of playable acceptance is therefore a strategy outcome, not a missing-guidance/reporting artifact.
- Brain-LLM negative-memory ingestion/rotation was repaired before this run. It now reads provider-local failed experiment history, avoids republishing already executed fingerprints, materializes a bounded deterministic next experiment when available, and reserves Qwen inference for genuinely exhausted/novel advisor space.
- Independent Learning run `36070349557` confirmed a deeper control-plane plateau: 10 providers were explicitly classified as having exhausted the bounded Core Repair strategy family, but the generated architecture proposal recycled profiles already present in those providers' negative memory (for example route/terminal transition graph families). The same push-triggered Learning path could not publish its architecture evolution proposal because the publication job excluded push events.
- PR #189 (`fix/learning-architecture-loop-20260925`) fixes both defects: architecture blueprints are filtered provider-locally against executed failed profiles, and Fast→Learning push runs may materialize the review-only architecture proposal. Production/provider writes remain disabled and human merge/fresh CI remain required.
- The same PR removes a scaling regression in Brain Autopilot: it previously dispatched Fast with `waves=3` and `time_budget_seconds=1200`, even though later rotations belong to Learning. Autopilot and Fast defaults are now one wave, one Deep round and 600 s.
- PR CI exposed an unrelated pre-existing canonical projection drift: `provider_catalog.json` declared UHDMovies as `movie,tv` while published bytes, manifests, census and provider override authority were movie-only. An initial manifest expansion was rejected by Verify & Publish and reverted; PR #189 instead corrects the stale catalogue authority to `movie` until an episodic lane is actually proven.
...
### 2026-09-25 — StreamBadge technical surface expanded and materialized

- Audited `assets/badge_catalog_v2_complete.json` against the media facts already recognized by Core and the standard film/series/anime technical surface requested for StreamBadge.
- Canonical commit `77c13db388d91f1eefd055c78927791f1fd8c491` expanded the catalogue from 74 to 122 badges and from 11 to 15 groups. Added CAM/TS/TC, 8K/4320p/360p/240p, common containers, MPEG-4 Part 2, canonical frame rates, video bitrate presence, 44.1/48/88.2/96/192 kHz sample rates, and natural-language badges for every non-French language already normalized by Core. French remains represented by the existing VF/VFF/VFQ/MULTI/VOSTFR model.
- The initial badge run `36079239674` successfully rendered the expected 288 new assets (`122 × 3 themes × 2 sizes = 732` total) but then failed because the validator still asserted revision `full-surface-v4-native-chip`. This was a validation-contract regression, not an artwork-generation failure.
- Commit `4e6b96d9d24285a905f869be1f85447947372aba` aligned both `scripts/validate_badge_system.py` and `tests/badge_asset_contract_test.py` with revision `full-surface-v5-technical-metadata`.
- Replacement badge run `36079388616` passed materialization, validation, deterministic fixed-point checking, publication, and no-drift verification. GitHub Actions published commit `5b8f1717aad5ca67510d76ffc0362dc25815ef93`.
- Verified the materialized repository at `5b8f171...`: every one of the six `transparent/dark/light × 72x32/96x40` folders contains exactly 122 WebP files; `BADGE_QA.json` reports `catalogBadges=122`, `assetCount=732`, `changedCount=288`, lossless WebP and idempotent generation.
- Verified Dark, Light, Fusion and Fusion v2 feeds each contain 122 filters and 15 groups. `stream-badges-fusion.json` and recommended `stream-badges-fusion-v2.json` now share the same blob SHA, fixing the prior risk that Fusion v2 could remain stale because it was not regenerated/staged by the badge workflow.
- Explicit example coverage verified end-to-end in the generated feeds/assets: AVC/H.264, 1080p, video bitrate presence, AAC, Stereo/2.0, 48 kHz and Korean. Exact bitrate values remain in technical text while the static BITRATE badge indicates that a bitrate measurement is present.
- No provider implementation was changed by this work; the provider non-regression gate was green on the initial catalog expansion.

### 2026-09-25 — StreamBadge v3 universal language/rating contract and four-feed versioning

- User required removal of France-centric public badge semantics and uniform versioning. Public v3 no longer exposes `VF`, `VFF`, `VFQ`, `VO`, `MULTI` or `VOSTFR` as badge IDs. Legacy provider vocabulary remains allowed only as input aliases so historical provider output can resolve to universal v3 badges (`VF/VFF -> FR`, `VFQ -> FR-CA`, `VOSTFR -> SUB FR`; `VO` alone is not guessed as English).
- Canonical commit `0aa0bea915f4182f1d10e28635bd44f3ce7f57d2` expanded the current catalogue to 299 badges: 47 language badges, 49 subtitle badges and 118 age-rating badges. Language coverage includes the requested country set and locale variants such as FR-CA, FR-CH, PT-BR, PT-PT, ES-419, ES-MX, ZH-HK and ZH-TW.
- Age coverage is no longer France-only. v3 includes universal numeric limits plus explicit regional systems, including Korea ALL/12/15/19, US MPA and TV ratings, Germany FSK, Japan, Brazil, Quebec, Spain, France with neutral `FR` labels, Hong Kong, India CBFC, Finland, Greece, Portugal, Philippines, Indonesia, Russia, Poland, Sweden, Taiwan and LATAM tokens.
- Badge workflow run `36081710814` passed materialization, validation, deterministic fixed-point proof, publication and no-drift verification. GitHub Actions published materialized commit `00e22c446222d1085d23f51df82815aefbe0d058`. `BADGE_QA.json` proves `catalogBadges=299`, `assetCount=1794`, `changedCount=1116`, lossless WebP and idempotent generation.
- Public feed versioning is now uniform: v3 is published simultaneously as `stream-badges-fusion-v3.json`, `stream-badges-dark-v3.json`, `stream-badges-light-v3.json` and `stream-badges-transparent-v3.json`. Each latest alias is byte-identical to its v3 snapshot at publication.
- Mandatory rule for all future badge work: every material catalog/pattern/group/style/asset/semantic change must increment `PUBLIC_FEED_VERSION` and publish all four new versioned feeds together. Existing versioned files are immutable; the generator fails if a published version would be rewritten. Public README/docs must be updated to the new numbered version in the same change.
- Backward compatibility rule: do not delete old versioned feed files or the old assets they reference. `stream-badges-fusion-v2.json` remains available with its original 122-filter contract. Unversioned feed files are moving `latest` aliases only and must not be documented as stable integration URLs.
- The legacy `normalize_fusion_badge_feed.py` path must not bypass versioning; it now delegates writes/checks to the canonical `normalize_badge_feeds.py` generator.

- Documentation consistency follow-up: all current Markdown installation guidance now names v3 explicitly; the primary asset table exposes Fusion/Dark/Light/Transparent v3 URLs and no longer recommends unversioned theme feeds. The old fusion-v2 URL remains documented only as historical compatibility, not as the current feed.

### 2026-09-25 04:38 Europe/Paris — Force 36085407669 proved stale/exhausted advisor import and neutral-main persistence gaps

- Final targeted Learning run `36081170479` completed successfully end-to-end on the 14-provider handoff: sandbox, sanitized memory publication, repair proposal publication and architecture proposal publication all succeeded. It produced 64 Learning proposals, 1 learned skill and 631 experiment-memory rows, but no provider candidate directly publishable from Learning (`provider_proposals=0`), preserving the Learn=proposal-only boundary.
- Explicit Force run `36085407669` consumed the 14-provider trigger but canonical Repair failed closed: 12 providers entered the candidate phase, 0 playable/verified streams were obtained, and the preservation gate rejected loss of the retained AnimeVostFR anime positive. No provider bytes were published.
- Root cause was not a missing Learning handoff: sanitized Learning memory imported successfully. The external Brain-LLM prior was rejected because `select_provider_materialization_scope.py` returned global mode from provider-local AnimeSalt drift plus the network-neutral `scripts/provider_patches/global_stream_presentation_v1.py` presentation patch. Repair then fell back to persistent Learning guidance whose experiment fingerprints were already present in negative memory, so every 12-provider Brain plan reported `llmAdvisorApplied=false` / exhausted variants.
- `scripts/import_external_brain_llm_guidance.py` now preserves unaffected advisor rows when `mode=all` is caused only by provider-local ownership reasons plus explicitly network-neutral presentation/docs/assets drift; true global provider-runtime drift remains fail-closed. Regression coverage includes AnimeSalt-local drift plus the global presentation patch.
- Canonical Repair's persistent Learning-guidance fallback now filters provider/profile/experiment fingerprints against current `automation/brain-repair-memory.json`, exactly as external guidance does. An empty exhausted fallback is no longer exported as active LLM guidance.
- Repair persistence now treats a newer `main` with `provider_drift=none` as provider-byte-current after rebase. This allows a validated Force candidate/census to persist across concurrent docs/badges/UI commits while real provider-input drift still requires fresh proof.
- Next required proof: refresh the Brain-LLM `niakvio-guidance` branch from this corrected NiakVIO HEAD so exhausted experiments rotate, verify new fingerprints, then rerun explicit Force with `directApplyValidated=true`. Do not claim provider repair until playable/identity/non-regression proof and durable census publication succeed.

### 2026-09-25 08:45 Europe/Paris — Force retries blocked by residential WAF cancellation

- Force `36087457494` did not reach canonical Repair: the GitHub runner was interrupted before the Repair phase. Retry `36088679711` also did not reach canonical Repair; its residential Tailscale WAF reprobe ran for ~9m25s, emitted `browser_challenge_persisted=2` and `browser_timeout=7` across 9 targets, then GitHub marked the operation cancelled. Canonical Repair, candidate gate and direct application were therefore skipped; no provider fix was published and the durable census remained unchanged.
- This is an execution-path failure, not provider proof. Tailscale connected successfully and the residential exit was active. The blocking behavior came from allowing the optional residential browser/profile matrix to occupy/cancel the critical Repair path.
- `.github/workflows/provider-recognition-repair-v6.yml` now time-bounds residential qualification with GNU `timeout` (240s hard budget, TERM + 10s kill), one attempt, 4 workers, max 16 targets, 20s request timeout and 6s virtual browser time. Timeout/failure is converted to an explicit soft fallback and removes any incomplete residential artifact, allowing the normal GitHub/WAF evidence path and canonical Brain/Repair to continue.
- Contract coverage in `tests/provider_repair_waf_integration_workflow_test.py` locks the bounded residential probe and fallback markers.
- Current provider census is still the pre-Force durable state: 26 FULL OK, 2 PARTIAL OK, 1 CANDIDATE OK, 2 CHAIN REACHED, 10 ROUTE PROVEN, 1 NO PROOF and 4 DISABLED; repairQueue remains the 14-provider cohort. Do not claim these 14 repaired until a Force reaches canonical Repair, passes playable/identity/non-regression gates, and persists the resulting census.


### 2026-09-25 — StreamBadge v4 published, shared-Core projection fixed, Kehflix v28 verified

- StreamBadge public contract is now v4: 301 canonical badges across 16 groups, with 1,806 generated WebP variants. Badge asset workflow run `36087763873` passed generation, deterministic fixed point and publication; materialized asset commit is `58f446690f12edcfc3c6d75ac7f76d355a3af76d`.
- v4 adds truthful delivery-format facts `HLS` and `MPEG-DASH` while keeping source, resolution, container, codec, bit depth/HDR, frame rate, bitrate, audio, language/subtitles and regional age ratings separate. All four current public snapshots are versioned together: fusion/dark/light/transparent v4. v3 snapshots and historical fusion-v2 remain immutable and available.
- The Core/catalog audit found that several already-existing badge families were not fully emitted through `badgeIds`. Stream presentation was expanded to map resolutions including 1080i/8K, HLS/DASH and containers, AVC/HEVC/AV1/VP9/legacy video codecs, 8/10/12-bit, HDR/HLG/SDR/IMAX/3D, common frame rates, bitrate presence, audio codec/technology/channels/sample rate, universal languages/subtitles and age ratings.
- Stream presentation source revision is now `all-providers-client-projection-technical-badges-v28`. Quality remains part of the visible provider title only when proven: e.g. `Kehflix - 1080p`, `Kehflix - 1080i`, `Kehflix - 4K`, `Kehflix - 8K`. Placeholder quality values such as `Unknown`, `Inconnue`, `N/A` or `Auto` must never create a title suffix; sparse streams retain only the provider name and any independently proven facts such as HLS.
- A generic publication bug was confirmed: provider publication fingerprint schema v3 did not include shared Core Bloc source files, so a changed shared Core could pass tests while existing provider bytes remained stale. Commit `1cae0e2c4d2b7defa102875b6538ce942c503f90` moved the publication contract to schema v4 and hashes shared Core publication files plus globally referenced patch scripts. The projection drift fixture was aligned in `b8cc9782462c3af8457888807f484cb8b95bb5a0`.
- Projection reconcile run `36105932877` then detected the v28 shared-Core drift, rebuilt all drifted active providers, proved fixed point, published atomically and dispatched exact-head validation. Publication commits: `6f21e2b77e0efcd208c8bc9120e3a3d8807196fe` and pin `c62b57b416aa44e9096eeadc4eeeff5d6c868c91`.
- Kehflix was inspected directly at published pin `c62b57b...`: manifest version `1.0.76`, file `providers/kehflix--nuvio--3dde106a48b201d5.js`, blob SHA `f6453a73551a50ac331d01195db4787cad0d5467`. The published bytes contain Core v28, HLS badge mapping, 1080i/8K mappings and the placeholder-quality guard; v27 is absent.
- Exact-head `CORE - Verify & Publish` run `36106506009` passed on `c62b57b...`. Full current-bytes census `36106504347` and CodeQL `36106507699` were still running when this checkpoint was written. Security Final Gate `36106509408` failed only at the release-reachable CodeQL-alert gate; provider hardening/unsafe-shape scans passed. The previously observed high alert was `.github/workflows/brain-learning-lab.yml` / `actions/cache-poisoning/direct-cache`; do not call the overall security gate green until the exact CodeQL state is rechecked.
- Added EN/FR cinephile technical guides explaining how to read StreamBadges (source vs resolution vs delivery/container, codecs, bit depth/HDR, FPS, bitrate, audio, languages/subtitles and regional ratings) with real badge images and an explicit no-invention policy.
- Reworked EN/FR NiakVIO update/recovery documentation so the first action is Nuvio's native repository Refresh from Settings/Plugins. Close/reopen comes next, repository reinstall after that, cache cleanup only as escalation, and full Nuvio reset last. Official Nuvio source was checked: Mobile/Desktop expose `refreshRepository` from Plugins settings and NuvioTV's full build re-downloads manifest + scrapers/providers.
- Public docs, badge install guides, assets README, README EN/FR, normalization tooling and README-sync workflow now reference the current v4 feeds rather than v3. Historical v3 references remain only where they intentionally enforce backward compatibility.

### 2026-09-25 — Next-episode/provider-load/multiflux audit

- Audited NuvioTV and Nuvio Mobile/Desktop current source for same-provider next-episode behavior. Nuvio uses `behaviorHints.bingeGroup` as the provider/source-affinity key. TV already injects `local-plugin-${scraper.id}` for local plugin streams and next-episode autoplay prefers the current binge group when enabled.
- Mobile/Desktop share the same binge-group-aware autoplay selector, but their current `PluginRuntimeResult.toStreamItem()` bridge only maps proxy/request headers into behaviorHints and drops bingeGroup. This is a host-side Nuvio gap; a NiakVIO-only JS change cannot guarantee same-provider next episode on Mobile/Desktop until that bridge carries a stable plugin binge group.
- Duplicate provider execution safeguards remain present in Nuvio: Mobile/Desktop short-circuit unchanged request keys and cancel superseded jobs; TV reuses stream-search sessions and deduplicates in-flight local-plugin scraper calls by scraper+TMDB+type+season+episode.
- Media-type guard remains enforced before provider network by the existing global semantic gate and its non-regression tests; anime may use TV transport when declared, but anime-only providers must not be projected onto movie.
- Multiflux audit found a real StreamZo loss: movie extraction selected only the first embed, episodic extraction deduped by language (dropping multiple same-language players), and resolveEmbed returned after the first media. StreamZo runtime is updated to preserve up to 8 distinct embeds/final URLs, dedupe only exact final URLs, and retain player/source labels.
- PapaDuStream already fanned out multiple players but was artificially capped at 4; its provider-local ceiling is raised to 8 (the current sanitizer validation ceiling) and player host identity is retained in player/sourceLabel/sourceName.
- Purstream ProviderBase already recursively walks plural `sources[]` and preserves multiple direct media URLs. Regression coverage now requires three distinct direct episode sources to survive while non-direct embeds are rejected.

### 2026-09-25 10:05 Europe/Paris — HLS safety/facts verified; Bloc terminology and architecture synchronized

- User TV evidence reported a Neko-Sama temporary ~4–5 s “host offline” video and Kehflix titles showing `- Inconnue` while Nuvio's player details already knew e.g. `1440×720 (720p)`.
- Root cause was not Brain/Repair dropping shared components: published Neko-Sama and Kehflix bytes at `ea11f937...` were inspected and both contain the shared HLS Integrity, Runtime Media Safety, HLS master-facts enrichment and Stream Presentation v28 Blocs. Projection reconcile `36112947048` had already rematerialized/published the HLS safety Blocs.
- The common HLS Bloc now rejects both complete tiny VOD and static-looking tiny media playlists; regression fixtures explicitly cover a 4.5-second `#EXTINF:4.5` offline placeholder and preserve sliding live windows/full VOD.
- HLS master facts are propagated before presentation: RESOLUTION, AVERAGE-BANDWIDTH/BANDWIDTH, CODECS, FRAME-RATE, VIDEO-RANGE, audio LANGUAGE/CHANNELS and subtitle LANGUAGE. Integration coverage proves `1440x720` → `Kehflix - 720p` plus `720p-hd,hls,avc,23.976fps,video-bitrate,aac,2.0,lang-ko,sub-fr,sdr`; unknown placeholders are removed from titles.
- Full current-bytes census `36113342452` did not reach provider probing because `tests/streamzo_episodic_identity_contract_test.py` still asserted the old pre-multiflux StreamZo helper signature. This is a stale test contract, not provider evidence. The test is updated to the current player-aware multi-stream contract and exact-URL dedupe.
- Public/project Markdown terminology is now **Bloc**, not the trademarked prior term. Legacy internal filenames/identifiers containing the old token are compatibility artifacts only and must not be used as new terminology. New docs/comments/features must use Bloc.
- `ARCHITECTURE.md` now documents shared-Core publication fingerprint schema v4, Projection Reconcile ownership, HLS short-media safety, HLS master-fact propagation, title-quality rules, StreamBadge v4 and dynamic provider scope instead of stale current-count invariants.


### 2026-09-25 11:36 Europe/Paris — consolidate Brain PRs and enforce main-only branch hygiene

- PR #193 was 63 commits behind current main. Its still-useful contracts are rebased directly onto current main instead of merging the stale branch: Fast-Handoff cache and canonical Force filter LLM guidance against both authoritative Repair negative memory and persistent Learning experiment memory; the external guidance importer accepts multiple negative-memory files; CodeQL concurrency is ref-scoped so a main push cannot cancel an unrelated PR scan.
- PR #192 is a generated architecture snapshot, 78 commits behind current main, not executable production code. Its recommendations were superseded by the later Brain strategy-exhaustion, Fast-Handoff and negative-memory work. The persistent Learning ref intentionally cleans proposal artifacts, so the stale 619-line JSON/71-line Markdown snapshot must not be reintroduced into production main.
...
### 2026-09-25 15:30 Europe/Paris — global multiflux locked; census false-drift and duplicate Repair loop removed

- Multiflux preservation is global, not StreamZo/PapaDuStream-specific. The shared ProviderBase adaptive fanout remains the common path; tests/provider_shared_player_fanout_contract_test.py now scans the provider-local Blocs actually materialized and prevents local shared-crawler caps below the common 12-stream budget. Audit found and removed residual local ceilings in VidRock (5) and VoirAnime anime/movie (4/6); exact-URL dedupe remains allowed, language/player collapse is not.
- Exact-head census runs 36139784815 and 36140218245 both reached provider probing successfully on 8816d23392c5: 25 FULL OK + 2 PARTIAL OK = 27 operational, zero wrong-content. The unresolved Brain queue is 15: 4khdhub, allanime, allwish, anime-ultime, animesalt, animesultra, animevost-fr, animevostfr, flemmix, mallumv, moviebox, moviesmod, vidfast, vostfree, yflix. Vostfree therefore needs requalification; do not preserve the earlier FULL assumption without fresh proof.
- Those censuses failed to persist for a false reason: persistence ran detect_provider_projection_drift.py against the mutated census workspace and reported all 42 active providers as drifted, while Projection Reconcile on the immutable same SHA reported drift=0/no-op. Census evidence is already copied to /tmp before persistence, so persistence now restores the exact GITHUB_SHA and cleans candidate workspace state before running the projection drift detector, then rehydrates only census evidence.
- The legacy TEMP - Individual Provider Max Repair Sweep is removed. It duplicated Repair V6, re-probed the whole visible catalogue, and could mutate provider bytes only in its workspace while committing evidence. Canonical Repair V6 is the sole Repair path: empty target means census repairQueue, route recovery is targeted, a contract proves no network reprobe escapes selected census symptoms, and Force owns candidate/non-regression gating plus publication.
- Validation of this census/Repair cleanup is pending on the new main SHA. After gates are green, persist a fresh exact-head census and launch canonical Force only from its current repairQueue.


### 2026-09-25 16:05 Europe/Paris — root cause of non-converging LLM Repair: advisor-only path could not mutate providers

- Exact-head census `36141266929` / SHA `655c776...` and Provider Non-Regression/Verify/Workflow/CodeQL gates proved the recent global HLS/Core/multiflux changes did not regress the operational cohort. Durable state remains 26 FULL OK + 2 PARTIAL OK, zero wrong-content/harness/network-blocked, with the same 14-provider `repairQueue`.
- The historical `experiment_variants_exhausted` report was stale and predated the cleaned single Repair V6 path. More importantly, audit of NiakVIO-Brain-LLM proved the architectural reason for non-convergence: the public NiakVIO guidance job ran the model in `advisor_only` mode, which forced `allowed_mutations=[]`, and `publish_niakvio_guidance.py` intentionally retained only strategy/profile/experiment knobs. The model therefore could rotate traversal parameters but could not repair a provider-local implementation.
- A second bug made normal mutation planning ineffective: Brain-LLM provider context did not descend into the real `provider-overrides.json > provider_patches` container. All 14 unresolved providers currently have a registered provider-local Bloc under `scripts/provider_patches/`, but that authored mutation surface was invisible to the model.
- Brain-LLM main now exposes only already-registered provider Bloc sources, adds a bounded `provider_patch` mutation scope, validates diffs against the exact registered Bloc path, and keeps Core/ProviderBase forbidden. A separate sanitized `niakvio-force-mutations.json` artifact has sandbox mutation authority only; it has no publication/proof authority. Learning remains proposal/advisor-only.
- The first Force-mutation generation attempt proved 13/14 providers route to `llm_repair`, but all 13 requests returned HTTP 400 from llama.cpp before model execution because the server rejected the strict JSON-Schema response format. Brain-LLM now retries only HTTP-400 schema rejection without `response_format`; the returned JSON is still reparsed and fully validated locally by causal-prior, mutation-policy and mutation guards.
- NiakVIO main now contains `scripts/apply_brain_llm_force_mutations.py` plus workflow integration before canonical Repair. It accepts only current `repairQueue` providers, rejects provider drift since the guidance SHA, permits structured provider-data mutations or diffs to an already-registered provider Bloc only, and has no publication authority. Explicit Force applies these candidates in the sandbox; canonical rematerialization/playback/identity/non-regression gates remain the only path to persistence.
- Status: bridge implementation is under CI/runtime validation. No provider is considered repaired by this mechanism yet. Required next proof is a Brain-LLM run producing non-empty sanitized Force mutations, followed by one canonical Force where those mutations are applied, tested and either accepted into `main` or recorded as failed mutation fingerprints without repetition.


### 2026-09-25 19:24 Europe/Paris — durable transport ledger + false no-request harness bug fixed

- Transport evidence ownership is now merged instead of mutually destructive. Full census, sharded census, dedicated WAF refresh and Repair all use `scripts/merge_waf_latest_evidence.py`: current GitHub/browser rows replace only the provider lanes actually refreshed, while the last valid residential/Tailscale evidence is preserved when a run has no residential measurement. A Tailscale-offline run records its failed/unavailable attempt without erasing prior residential proof.
- Added regression coverage for both data semantics and workflow topology: `tests/waf_latest_evidence_merge_test.py`, `tests/waf_latest_workflow_contract_test.py`, and the Repair WAF integration contract. Provider Non-Regression, Core Verify/Publish, Workflow Gate and CodeQL are green on SHA `33758ae5a1aebd46dd35ed7c8a14a2b7017a440a`.
- Root cause of the widespread Brain sandbox `provider_unreachable / no_provider_request_observed` false negatives was identified in `scripts/provider_worker.cjs`: signature inference can be erased by generated Core/Bloc wrappers such as `async function(){ native.apply(this, arguments) }`. The worker previously treated an empty array from the first guessed signature as conclusive even when that attempt made zero provider-owned requests.
- Generic fix `2c70b88039fdffd5b5b9e8e22d47e68a63363009`: `[] + 0 provider observations` is now inconclusive and the alternate object/positional signature is tried. `[] + >=1 provider-owned request` remains a valid provider result. Test commit `33758ae5a1aebd46dd35ed7c8a14a2b7017a440a` covers wrapper-erased object signatures and prevents regression.
- This harness bug explains why the pre-Brain current-byte route evidence could show real provider requests while the subsequent Brain baseline suddenly reported no provider request for many unrelated provider families. Do not classify those historical no-request rows as independent provider/network failures.
- Brain-LLM Force generation is still under validation and is not yet proven repaired: the earlier compact run moved from timeout to truncated JSON at a 160-token cap. Brain-LLM is being changed to a mutation-only wire payload, with deterministic NiakVIO reconstructing target layer/strategy/confidence/tests and retaining the same local mutation guards. No provider correction from that new wire format is accepted until Brain-LLM CI is green and canonical Force proves it.

## 2026-09-25 — Brain Repair harness parity, causal Learning gate and Domain Refresh rotation

- The three-day Repair/Learning plateau is no longer treated as a shortage of provider examples. Durable Repair memory already contained hundreds of experiments; the blocking issue was execution ownership and orchestration.
- A concrete harness split was confirmed: current-byte Quick Yield executes through `nuvio_tv_probe_tmdb_ci.cjs`, while Brain Deep executes through `health_check.mjs -> provider_worker.cjs`. Deep previously spawned the worker with a stripped environment that omitted TMDB credentials, while the production-like probe exposed TMDB authority only during provider/Core module initialization. This could stop Core/provider execution before any provider-owned request and falsely create `no_provider_request_observed` provider debt.
- Commits from `afce84ffe81e` through `debee5b51eb9` add a private `NIAKVIO_TMDB_BOOTSTRAP_*` bridge. The worker removes bootstrap env immediately, exposes canonical TMDB globals only long enough for Core/module initialization, clears them before `getStreams`, and regression coverage asserts that no raw credential remains visible or serialized.
- Same-byte harness ownership is now executable architecture. `scripts/audit_brain_harness_differential.py` verifies staged SHA + exact fixture and compares Deep zero-request evidence with the production-like Nuvio probe. If Nuvio observes provider-owned requests while Deep observes none, the provider becomes `harness-compatibility`: provider mutation is blocked before candidate generation, provider-negative memory is not written, the provider is excluded from Learning debt and later Repair waves. Main implementation landed through `8ef01783369a`; dedicated regression tests are part of the Core Workflow Gate.
- Automatic Repair -> Learning now has a sanitized causal dispatch ledger: `scripts/provider_learning_dispatch_gate.py` + `automation/provider-learning-dispatch-ledger.json`. The fingerprint contains provider cause/signature and the actual method/profile/generation/LLM experiment, deliberately excluding run IDs/timestamps. Same fingerprint is suppressed; missing causal method fails closed; a changed signature/method can dispatch again. Canonical Repair and Fast Repair pass exact `target_providers` only. Fast Repair no longer writes a Learning push trigger and explicitly dispatches at the same time.
- `BRAIN_REPAIR_ARCHITECTURE.md` is the dedicated human contract for authority, harness parity, provider Repair, Force/LLM, Learning anti-loop, Domain Refresh separation, Core preservation, acceptance ladder and representative-before-portfolio run discipline. `ARCHITECTURE.md` and `README.md` link it.
- Domain Refresh root cause was confirmed rather than patched provider-by-provider: `direct_authority=explicit_current` was accidentally immutable twice — a synthetic score 1000 always beat fresh hub candidates, and registry persistence rejected any contradictory terminal. This made the daily workflow structurally incapable of advancing a previously explicit current address.
- Domain authority semantics are now split: `explicit_current` is a strong **refreshable LKG/current** address; `operator_pin` is the only immutable automatic-refresh pin. Fresh primary/current/active labels or authoritative deterministic redirect evidence may supersede `explicit_current`; stale/backup/old cards may not.
- Failed Domain Refresh run `36185227138` is useful evidence: the resolver already discovered `https://flemmix.style` at score 100 from the configured `https://neufneuf.space/` redirect source, while the old synthetic `flemmix.party` candidate remained score 1000. The run failed later because `domain_refresh_workflow_test.py` still encoded the obsolete “explicit_current is immutable” contract. It published no domain changes.
- Artifact inspection of `36185227138` also proved that the official redirect source returns HTTP 200 and exposes Flemmix as an outbound CTA (`Accéder maintenant`) rather than a 30x. `refresh_authoritative_hub_domains.py` therefore now accepts only a high-score, same-brand terminal from a configured `redirect` source as fresh authority. The stale contradiction test now uses `operator_pin`; a dedicated regression reproduces this 200-page redirector case.
- Domain Refresh validation is still **pending** after the last redirect-source fix. Do not claim Flemmix or other domains updated until the new workflow is green and the committed `provider-hubs.json`, `provider-overrides.json`, provider bytes and domain history are inspected.
- Repair validation is also still **pending**. After architecture/CI is green, the first real provider test must be targeted to one representative that previously showed current-byte route/chain proof but Deep false zero-request (MalluMV is a strong candidate). Require an actual Deep provider-owned request before expanding by causal family; do not relaunch all symptomatic providers blindly.
...
### 2026-09-25 — Brain Repair portfolio multi-advisor closure before 14-provider run

- `PROVIDER_CENSUS_STATUS.md` / `automation/provider-census-status.json` still contain the same 14-provider repair queue: 4khdhub, allanime, allwish, anime-ultime, animesalt, animesultra, animevost-fr, animevostfr, flemmix, mallumv, moviebox, moviesmod, vidfast, yflix. The MalluMV representative run did **not** repair or requalify the census; it only proved that the former false `no_provider_request_observed` harness symptom is gone and Deep now reaches provider-owned lookup execution.
- Brain-LLM targeted run `36189111014` correctly used MalluMV only and NiakVIO source SHA `833e350a45e9bf0dd5b52ded736241b1927bce36`. It produced three valid non-mutating advisor hypotheses, but the concrete Force generation was truncated (`JSONDecodeError: Unterminated string`) and the old workflow discarded the otherwise valid advisor page because Force produced zero executable mutations.
- Brain-LLM now retries truncated compact Force JSON with budgets consistent with the bounded find/replace schema, keeps malformed JSON fail-closed, and publishes valid advisor hypotheses even when concrete Force abstains/fails. Explicit NiakVIO Force remains fail-closed later when an external mutation is actually required. Brain-LLM CI is green on `c27480936b4bf488dd61e2276f1d753d8f15b7af`.
- NiakVIO portfolio Repair now supports multiple advisor hypotheses **inside one run**. `advisor_wave_budget()` expands the effective portfolio wave count up to three when distinct sanitized advisor fingerprints exist. `untried_advisor_fingerprints()` prevents a provider from entering Learning while B/C remain untried after A fails. The sequence is attributable and isolated: A -> failure memory A -> B -> failure memory B -> C. Alternatives are never stacked into one mutation.
- This is portfolio-wide, not provider-by-provider orchestration: all 14 providers can run in one bounded Fast Brain Repair cohort, each rotating independently through its remaining advisor fingerprints while validated reusable strategies may transfer across compatible provider families.
- `BRAIN_REPAIR_ARCHITECTURE.md` is the dedicated contract and now explicitly documents the 14-provider portfolio/multi-advisor behavior. `tests/brain_multi_advisor_same_run_test.py` locks advisor wave budgeting and exhaustion-before-Learning; Fast Repair runs that test in its own preflight.
- No claim of provider repair is made yet. Next valid execution sequence is: all architecture gates green -> regenerate sanitized Brain-LLM guidance for the exact current 14-provider cohort/current NiakVIO SHA -> one Fast Brain Repair portfolio run over the same 14 -> current-byte Retest only for accepted candidates -> persist census and memory. Do not fall back to another MalluMV-only proof unless the 14-provider run isolates a new architecture defect.

## 2026-09-25 — Positive Brain program memory loss identified and repaired

- The MalluMV representative Fast Repair proved the Deep harness fix: the old false `no_provider_request_observed` did not reproduce. Current Brain evidence reaches the provider/player path and classifies the unresolved case as `chain_terminal_gap`; MalluMV is still **not repaired**.
- A second orchestration defect was then confirmed: current `automation/brain-positive-program-memory.json` had been reset to `entries: []` even though historical Brain Repair had already produced strict accepted MalluMV programs.
- Historical report `automation/provider-brain-repair-35920323353.json` is the strongest retained proof: MalluMV accepted `player_media_extractor_v1` with `strict_playable_stream_improvement`, playable `0 -> 1`, accepted-program compilation succeeded, and positive-program persistence was recorded.
- Git history identified the exact destructive commit: `b704f60a0a1b32b4cd319ace4066f23969a04cc3` (`fix(brain): cut redundant waves and preserve Tailscale verdicts`) replaced 167 lines of validated positive memory with `entries: []`. Earlier commits `9b6a383c793a3bad1ac8e3e9682dc457f3a9779a` and `b38f896fad8df278c021c9ebd5b78e87e098b567` contained the valid MalluMV positive entries.
- The durable positive memory was restored on current main from the last intact state in commit `3ad4c4f182d540ce5d9287dae61c7da8d728dbf9`.
- `scripts/recover_brain_positive_program_memory.py` now provides generic fail-safe recovery. It accepts only historical rows that were strict playable improvements, improved playable count, were recorded as compiled, and still compile with the current Provider v3 compiler. Recovered programs remain prior-only and must pass current-byte playback/identity/non-regression again.
- `scripts/run_provider_brain_repair.py` performs this recovery before planning. `tests/brain_positive_program_recovery_test.py` proves idempotent recovery and rejection of non-improvements. `tests/brain_positive_program_repository_continuity_test.py` fails CI if committed positive memory is missing a strict historical program still recoverable by the current compiler.
- Another architecture mismatch was fixed: same-provider strict positive programs were previously available only in Learning after experiment exhaustion. The planner now permits the **same provider only** to replay `provider_positive_program_replay_v1` in canonical Repair. The replay is keyed by the exact aggregate positive-program fingerprint; it is not peer transfer and grants no acceptance authority.
- Existing Brain-LLM production rescue remains separate and preserved. Regression coverage now verifies both paths: LLM rescue and same-provider positive-program Repair replay.
...
## 2026-09-26 Europe/Paris — local FORCE harness network permission root cause

- Remote Desktop validation on the user's Mac showed the local FORCE farm was initially failing before meaningful provider evaluation. First blocker was the Nuvio client drift guard in detached worktrees; the local farm now uses the existing explicit test-only override `NIAKVIO_SKIP_CLIENT_DRIFT_GUARD=1`, while GitHub FORCE remains the publication/current-byte authority and must re-run the real guard.
- After that isolation, local Quick generated candidates but provider requests failed with Node errors such as `getaddrinfo ERR_ACCESS_DENIED api.mkissa.net`, `api.allanime.day`, and `v5.anime-ultime.net`, while direct `curl` from the same Mac returned HTTP 200 for representative provider sites.
- Root cause: `scripts/health_check.mjs` launches provider workers with Node `--permission` and filesystem grants, but omitted `--allow-net`. On the user's Node v26.5.0 this blocks provider DNS/fetch and falsely appears as provider HTTP/network failure.
- Fixed harness by adding `--allow-net` to the worker spawn. Targeted local smoke on AllAnime then changed from `provider_http_error / ERR_ACCESS_DENIED` to real provider execution: Quick baseline `no_streams score=100`, one generated candidate, and one sandbox exploration progression. That progression correctly escalated to Deep; Deep did not accept it, which is now an informative provider-level result rather than an environment failure.
- Added regression assertion in `tests/deep_health_tmdb_bootstrap_test.py` requiring Node network permission under the permission system.
- Full 14-provider local FORCE farm was relaunched on corrected harness SHA `96ebaf865ea1909bd42e32113b1fccdb4d8348ff` with 24 variants/provider, 4 workers, and Deep=3. Its results are local evidence only until targeted GitHub FORCE revalidates any winner.


## 2026-09-26 Europe/Paris — local FORCE farm made representative + Brain-LLM current-cohort regeneration

- Local FORCE evidence before the fixes was not representative. Three independent harness/tooling faults were confirmed and corrected:
  1. detached local worktrees could not establish all Nuvio client repos, so the non-authoritative farm now uses the existing explicit test-only `NIAKVIO_SKIP_CLIENT_DRIFT_GUARD=1`; canonical GitHub FORCE retains the real drift guard;
  2. Node provider workers used `--permission` without `--allow-net`, producing false `getaddrinfo ERR_ACCESS_DENIED` provider failures; `scripts/health_check.mjs` now grants network permission and a regression assertion covers it;
  3. `evaluate_brain_llm_force_candidates.py` used an OS `/tmp` sandbox which could again produce permission failures; its default sandbox now lives under repo `local-output/`.
- After `--allow-net`, the farm changed from zero useful observations to real candidate execution: generated candidates matched experiment count and Quick routinely produced non-publishable `exploration_progress` that correctly escalated to Deep.
- Quick→Deep handoff now treats either accepted Quick improvement OR exploration progress as promising; summary aggregates per-round generated/progress counters instead of nonexistent top-level fields.
- Search-space generation now round-robins all six executable advisor strategy/profile families instead of spending a provider budget inside one family. Negative evidence and resumability are scoped by strategy/profile/fingerprint.
- Deep current-byte baseline requalification is explicit: `anime-ultime` repeatedly validated `healthy` in Deep (7 fixtures, 1 playable stream, score 100, no runtime error). The local farm now stops further repair variants for Deep-baseline-healthy providers and reports `deepBaselineHealthyProviders`; this is local revalidation evidence, not yet a canonical census promotion.
- Adaptive local early-stop was added for scale: only after at least two experiments in every one of the six strategy profiles, no winner/healthy baseline, and identical outcome signature within each profile. `allwish` and `animesultra` were observed as strategy-exhausted under that rule. `animesultra` also had direct local TLS failure against `v2.animesultra.org`; do not rewrite its domain without fresh authority.
- Farm shutdown now tracks Quick/Deep subprocess groups and terminates them on SIGINT/SIGTERM/timeout to prevent orphaned Node/Python probes from contaminating later runs.
- The local farm was confirmed to have been using only deterministic generated rows, not live Brain-LLM guidance. It now reads the sanitized non-authoritative `NiakVIO-Brain-LLM/niakvio-guidance/guidance/niakvio-guidance.json` prior when available, with deterministic fallback preserved.
...
### 2026-09-26 — Architecture FORCE timeout retry
- Architecture FORCE run 36247455635 reached the structural materializer on main 473ea42f; all prior targeted Learning/Qwen stages passed.
- The request no longer failed HTTP 400, proving the context-bound fix worked, but the local llama.cpp response exceeded the old 120s socket budget and ended with TimeoutError.
...
### 2026-09-26 — Census -> Autopilot handoff is event-driven
- Sharded census run 36267103445 persisted fresh authority but its final gh workflow run Autopilot call failed with HTTP 403 Resource not accessible by integration.
- Autopilot now listens to successful Provider Census - Sharded workflow_run completion from the same repository; census no longer depends on an internal workflow-dispatch API call.


### 2026-09-26 — WAF transport evidence must precede census classification
- Separate WAF evidence requalified the original WAF-blocked providers, but a later sharded census moved them back to HARNESS/ENV BLOCKED because final classification used only the current GitHub-hosted browser probe.
- Sharded census now merges persisted latest residential/private-exit evidence with the current WAF probe before final status rendering and repair-batch planning.
