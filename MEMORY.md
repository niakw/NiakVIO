...
## 2026-09-18 — authoritative continuation checkpoint: manifest-46 census established, repair persistence hardened, publication still blocked

- Public `main` remains **c02c9919c98dd8ff6729350f44a48f19a5cb571f / 5.21.51** and its MEMORY is stale. Active repair work remains isolated on **`fix/provider-activation-certification-v1`** and must not be merged wholesale.
- Current repair branch generation is **5.21.56**, with **46 Provider JS rows total / 44 enabled / 2 disabled-retained (DESIFLIX, FULLANIME)**. The only authoritative global provider-certification denominator is **46**. Targeted runs are diagnostics only and must print `global-kpi=false`.
- First authoritative full-manifest exact-bundle census: workflow run **35292935002 / #17** reached **19/46 certified**, `fullManifestCensus=true`. This is **41.30%**, below the 75% architecture floor; state is **architecture-defect-low-auto-yield**. Mass-disable remains blocked. Node-negative evidence alone must never disable a provider.
- #17 exact positives: Anime-Sama, AnimeKai, AnimeSama.co, AnimeVOSTFR, Coflix, French-Manga, HindMoviez, Kurage, Mugiwarastream, Neko-Sama, PlayIMDB, Purstream, Sekai, StreamZo, VidEasy, VidFast, VidRock, VoirAnime.homes and VoirAnime.rip.
- #17 main failure clusters: anime route/catalog resolution zero-result (9 providers); movie HTTP/upstream transport failures (5); movie route/catalog zero-result (5); tv route/catalog zero-result (5); anime HTTP/upstream failures (4); tv HTTP/upstream failures (4); smaller runtime-exception / timeout clusters. Repair common causes before provider-local disable decisions.
- #17 evidence generation itself succeeded: certification, positive-fixture merge, Learning queue, activation consensus and activation dry-run all completed. Final evidence push failed only because the remote repair branch advanced concurrently (`fetch first` / non-fast-forward). The run artifact/log remains valid evidence, but persistent automation evidence must be regenerated and committed from a stabilized head.
- Positive fixture memory invariant is now fixed: **only a full manifest census may mark an unobserved provider stale**. A targeted certification modifies only providers it actually observed; it cannot degrade positive memory for the rest of the manifest.
- Repair-survival authority invariant is hardened again: AllWish must restore only `allwish_current_runtime_v2.py`; VidFast only `vidfast_current_runtime_v2.py`; VidLove only `vidlove_current_api_v2.py`. `restore_provider_repair_survival_v1.py` now replaces historical v1 references with these v2 scripts **and purges superseded v1 option keys**. Tests assert that old v1 Lego cannot coexist in restored authority.
- Current `provider-overrides.json` and survival registry are clean for these three providers: **no AllWish v1, no VidFast embed v1, no VidLove API v1**. This closes the restoration-loop regression where every run could silently re-add obsolete Lego after a modern repair.
- Run **35292490806 / #16** remained a targeted diagnostic and certified **16 selected of 27**, explicitly `manifest_total=46 global-kpi=false`; do not convert this into a global ratio. It confirmed VidFast movie+tv and VoirAnime.rip anime on the regenerated bundle, while AllWish, AnikotoTV, Castle, VidLove, WookaFR and other clusters still required work.
- Semantic regressions remain fixed and gated: HellMode **S02E12** season identity cannot silently fall back to S01E12; French-Manga/VoirAnime.homes share the season-aware DLE resolver; presentation Core V25 preserves explicit VF/VOSTFR evidence, avoids invented VF, and restores Kurage VO fallback/badge.
- Non-display repair survival remains mandatory after minimization/finalization. Historical #122 proof is evidence only; exact regenerated bundles require current proof.
- Publication remains **blocked** until a stabilized-head full census is persisted, Learning/consensus are rebuilt from it, Node negatives are reconciled with Native evidence, and the clean main-based publication branch is constructed. Never merge the diagnostic branch wholesale.
...
## 2026-09-18 — Exact 5.21.57 census + ProviderBase/adaptive-runtime repair checkpoint

- User acceptance contract remains **>=35/46 providers with at least one certified playable semantic lane**. FULL/all-lanes is a separate stricter metric and must never be substituted for the >=1-green KPI.
- Pure frozen exact census run **35343597111** for manifest **5.21.57 / 46 rows** completed green as a workflow and proved only **20/46 providers with >=1 green lane, 18/46 FULL**. Exact 5.21.56 baseline was also **20/46 >=1 green, 19/46 FULL**. **45/46 lane verdicts are unchanged**; HindMoviez alone regressed from FULL movie+tv to PARTIAL movie. Therefore the earlier static estimate ~32–36/46 was wrong and is retired.
- Root cause of that bad estimate: the repair-survival markers and most #122/#127 repair scripts were already physically present in the measured 5.21.56 bundles. 44/46 changed bundle hashes in 5.21.57 were mostly shared Core/rematerialization/presentation churn, not 44 new provider extraction repairs. Repair-survival green != provider-output green.
- **Never ask the user to repeat provider/manual tests already supplied.** Existing user TV, macOS/Windows/iOS/Android artifacts, browser/manual evidence, exact Node census, positive fixtures, and repo knowledge are sufficient inputs. Node negative must still be reconciled with Native/manual positive evidence before activation decisions.
- Materializer V26 contract added: non-provider helpers such as `arm.haglund.dev`, Cinemeta, TMDB may remain metadata/identity fallback but **cannot become execution authority that suppresses provider-owned routes**. Targeted V26 run **35346378742** showed no gain by itself because Anime-Ultime / AnimesUltra / Vostfree static knowledge already lacks real provider routes.
- AniKoto V3 defect found and patched: MegaPlay server selector may live in path `/stream/s-N/<id>/...`, not query `?s=N`; V3 now preserves that selector in `getSources`. A temporary bad regex escaping was caught by the raw-artifact validator and corrected before publication. Global Lego contract now forbids QuickJS-incompatible `URLSearchParams.set/delete` mutations in provider patch scripts.
- ProviderBase V35 added: absolute learned routes are passed through Domain Refresh `_substituteDomain()` instead of bypassing substitutions.
- Major durability defect found: persisted clean ProviderBase rows were treated as current solely by `clean_reconstruction_authoring_version=3`, so common-template changes could be silently ignored while 46 providers were rematerialized from an old common skeleton. This is unacceptable for the incoming ~300-provider hub expansion.
- ProviderBase authoring advanced to **v4** with exact common-template **SHA-256 identity**. Store/provenance now records `template_sha256`; stale clean skeletons are detectable; `refresh-template` rebuilds verified clean ProviderBase rows only from the owned current skeleton. Publication pipeline now auto-refreshes stale clean bases before compile; `--check` fails closed when a template refresh is required. Synthetic template-identity test added.
- V27d run **35347788982** used old persisted bases through `reapply_published_overrides.py`, proving why V35 did not reach 4KHDHub. This run yielded no new green and must not be counted as a repair success.
- V28b run **35349217928** materialized directly from the current ProviderBase template and completed successfully. All selected bundles carried V35. Targeted score remained **1/6**, only Anime-Sama green, so no repair is considered complete yet.
- V28b sanitized response-shape evidence:
  - **AniKotoTV**: full chain reaches AniKoto search -> episode list -> server list -> server URL -> `megaplay.buzz/stream/getSources` with HTTP 200. Current payload shape has keys `tracks,t,intro,outro,server,enc`; tracks are subtitle-like rows and `enc` is present. Failure is terminal decryption/extraction, not catalogue/network routing.
  - **AnimeVOST.fr**: `/api/animes/search` returns JSON `{results:[], animes:[]}` for current fixtures; route itself is live but current query yields no catalogue match.
  - **Castle**: `getSecurityKey` returns JSON `{code,msg,data}` HTTP 200; search flow remains encrypted and runtime stops before usable movie-id/details. Treat as crypto/parser chain, not dead network.
  - **ShowBox**: current generated route hits `id-mapping-api-showbox-proxy.hf.space/` and receives only service metadata `{status,service,documentation}`; generated endpoint contract is wrong/incomplete. Historical ShowBox knowledge in `providers/showbox.js` includes ShowBox->FebBox share-key/file-list/quality-list flow and must be mined as knowledge, never executed as upstream seed.
  - **4KHDHub**: V35 is physically present in current-template bundle and DATA carries `hdhub4u.bi -> hdhub4u.ms`, but targeted trace still reports requests on `.bi`. Before another Core change, distinguish requested host from response host/redirect and reconcile Domain Refresh authority with current site evidence.
- Existing repo migration **`scripts/upgrade_anikototv_megaplay_aes_v3.py`** contains a more complete clean-room MegaPlay terminal implementation than current AniKoto V3: WebCrypto AES-256-CBC + native `__crypto_aes_decrypt_raw` + CryptoJS fallback, current `data-id`, media-host normalization, host-aware terminal headers. It currently targets runtime V2 and was not fully absorbed by the published V3. Next repair is to port that already-owned logic into V3 while preserving the current `s-N` selector and then prove a terminal media URL.
- Scaling rule for the incoming ~300 hub/provider set: **compile/survival/markers are not activation proof**. A provider remains non-activated until at least one declared lane returns a terminal playable stream; learned DATA must merge monotonically by capability family so newer proof cannot erase unrelated still-valid routes/plans.
### 2026-09-18 — Stage-trace persistence checkpoint

- Remaining exact activation deficit is **26/46 providers with no green lane**; **15 additional providers** are required to move from 20/46 to the >=35/46 contract.
...
### 2026-09-18 — ShowBox knowledge recovery + AniKoto correction

- Correction to the previous checkpoint: current `scripts/provider_patches/anikototv_runtime_v3.py` **already contains** owned MegaPlay WebCrypto AES-CBC, native `__crypto_aes_decrypt_raw`, CryptoJS fallback, current `data-id`, media-host normalization **and HMAC media signing**. Therefore AniKoto V28b zero must be diagnosed from the exact internal `anikoto_megaplay_*` trace; do not describe the repair as merely “port AES from V2”.
- Historical `providers/showbox.js` was decoded **knowledge-only** (never used as executable seed). Its rotated custom-base64 string table resolves the real default API base to **`https://id-mapping-api-showbox-proxy.hf.space/api/media`**.
- ShowBox historical request contract: movie `/movie/{tmdbId}?cookie={uiToken}`; TV `/tv/{tmdbId}/{season}/{episode}?cookie={uiToken}`; when configured, OSS variant inserts `/oss={ossGroup}/...`. Successful proxy response may expose `id`/`mid` (or under `data`), then FebBox knowledge uses `/file/file_share_list?share_key=...` and `/console/video_quality_list?fid=...&share_key=...`.
- Current structured ShowBox DATA is wrong/truncated: it executes the proxy root through `/?tmdbId=...`, which V28b proved returns only service metadata `{status,service,documentation}`. Repair must restore the structured `/api/media` contract and model required provider setting(s), not special-case the service root.
- Materializer execution authority is now monotone per capability family (routes/API/search/provider-value/external identity), so fresh proof in one family must not erase independently proven knowledge in another. Preserve this invariant for the incoming ~300-provider expansion.

### 2026-09-18 — V29 terminal diagnostic + active-red recovery target

- Exact 5.21.57 census has **26/46 zero-green providers total**, but DesiFlix and FullAnime are explicitly OFF. The executable repair queue is therefore **24 active red providers**. To satisfy the acceptance contract **35/46 >=1 green**, NiakVIO must recover **at least 15 of those 24**.
- Recovery priority is output-oriented, not marker-oriented. First cluster: live-network / 200-heavy providers where provider logic stops before terminal output: 4KHDHub, AllAnime, AniKotoTV, Anime-Ultime, AnimesUltra, Animetsu, AnimeVOST.fr, Castle, MalluMV, ShowBox. Second cluster: MoviesHunt, MoviesMod, UHDMovies, VoirAnime, WookaFR, MovieBox where the chain is substantially live but later route/downstream stages fail. Pure 403/config-only cases come after these.
- V29 diagnostic run **35359410144** (artifact **10553842518**) proved AniKotoTV much further than the census summary:
  - Jujutsu Kaisen S1E1 chain: AniKoto search 200 -> watch 200 -> episode list 200 -> server list 200 -> server 200 -> MegaPlay `/stream/s-2/10789/sub` 200 -> `/stream/getSources?id=116363&s=2` 200.
  - provider trace: `anikoto_megaplay_enc enc=1`, `anikoto_megaplay_crypto webcrypto=1`, `anikoto_megaplay_sign webcrypto=1`, `anikoto_megaplay_terminal direct=1;context=1`.
  - Runtime successfully decrypts and signs a `fetch.nexabloom.top/.../master.m3u8?token=...` terminal URL, but Core terminal HTTP validation receives **403**. AniKoto is therefore **terminal-CDN authorization/signature context failure**, not catalogue/parser/AES failure.
  - `getSources` also carries field `t`; current signer ignores it and uses `Date.now()/1000 + 90`. Whether `t` participates in the current MegaPlay token contract is **not yet proven** and must be diagnosed before changing signing logic.
- V29 stopped after the expected AniKoto no-stream nonzero probe because the temporary workflow used `set -o pipefail`; its 4KHDHub step therefore did not execute. Future diagnostic probes that intentionally inspect a red provider must preserve the probe output while tolerating its nonzero exit.
- Iteration-speed rule from now on: use **`scripts/materialize_provider_v3_one.py <provider>`** for micro-repair/probe loops instead of rematerializing all active providers. Full/current-template materialization and exact manifest census are reserved for checkpoint validation after targeted gains.
- Generic HTML-catalogue false-positive evidence is now explicit: MalluMV, UHDMovies and some MoviesMod probes follow unrelated recent titles because the current generic title/URL correlation threshold can accept weak token overlap. Strengthen correlation in ProviderBase with regression tests before the ~300-provider import; do not let generic crawlers turn category/latest pages into provider identity proof.

### 2026-09-18 — V30 fast one-provider diagnostics

- Fast diagnostic infrastructure is now proven: run **35360578568** / PR #157 uses `materialize_provider_v3_one.py` and executes multiple red providers without allowing one expected no-stream exit to abort the batch. This replaces expensive full-44 rematerialization for micro-debug loops.
- **4KHDHub** current-template state: Domain authority no longer executes the old `.bi` site. Model now projects `official_site=https://4khdhub.one`; Interstellar search/detail pages are HTTP 200 and an exact current detail `/interstellar-movie-458/` is found. The chain then extracts `https://player.autoembed.cc/embed/movie/`, but provider fetch fails with `TypeError` before HTTP (status 0). Current blocker is player handoff/incomplete embed URL, not catalogue discovery.
- **Castle** current-template state: TMDB 200 -> `getSecurityKey` 200 returning `data` string length 16 -> encrypted `searchByKeyword` 200, then no usable decrypted search result/details call. Current blocker is the Castle decrypt/search parse stage. Existing language tests are synthetic contract tests only; they do not prove current network decryption.
- **Animetsu**: every tested search/API route on `animetsu.live` redirects to the same `omg10.com/4/10829438` HTML page. Static learning incorrectly promoted this HTTP-200 redirect/app path as reusable execution knowledge. Treat as stale/domain/redirect authority, not a working provider route; HTTP 200 alone is not promotion proof.
- **AnimeVOST.fr**: exact Steins;Gate fixture and aliases all return HTTP 200 JSON with `results=[]` and `animes=[]`. Current search endpoint is alive but catalogue search contract/query is not yielding titles; no parser exception.
- **MalluMV**: current site/search is alive, but Avengers fixture leads to unrelated current Malayalam/Tamil 2026 title pages. This proves both weak catalogue correlation and a fixture-model problem for region/specialty providers. Brain/Learning must be able to derive provider-specific positive fixtures from provider catalogue + TMDB identity instead of assuming generic global titles cover every movie provider.
...
### 2026-09-18 — V32 resolver-family result + Vostfree V36 follow-up

- V32 run **35364467981** discovered exactly three active-red Lego providers using `__niakvioProviderRuntimeResolverV1`: **VoirAnime, Vostfree, Yflix**.
- Generic ProviderBase consumer V36 produced the first real recovery of this campaign: **VoirAnime is now anime FULL certified** on the rematerialized current bundle, with provider-returned streams through `voir-anime.to` and terminal hosts including Streamtape/VOE. Treat this as **+1 potential exact-census gain: 20 -> 21/46**, pending the final frozen full census.
- Vostfree V32 still hit only ARM because its owned resolver rejected the Nuvio anime transport alias `tv` before provider network. Commit **627f6a285a9ba4f099aeba12a844e9f9a376392d** maps transport `tv` back to semantic `anime` inside the Vostfree resolver; dedicated alias test added.
- V36 targeted run **35365207458** proves that fix changes execution correctly: Vostfree now executes its provider-owned resolver and contacts `ipv4.vostfree.ws/index.php` directly. However all 12 exact anime fixtures receive **HTTP 403** on the DLE search POST. Therefore Vostfree is no longer a consumer/transport bug; it is currently an upstream/anti-bot HTTP blocker and is removed from the quick-gain queue.
- Yflix remains red after V36 consumer, with provider/network exceptions across `enc-dec.app`, `yflix.to`, `1movies.bz`, `solarmovie.fi`; no gain claimed.
- V35 hardcoded-domain audit is advisory only: it found explicit provider/player/API hosts in 12 Lego scripts, but many are legitimate external resolver authorities. Do not mass-replace them. The invariant is narrower: provider catalogue/current-site authority must come from structured DATA/Domain Refresh, while independently owned external player/API endpoints may remain explicit.
### 2026-09-18 — V34 result: MoviesHunt second recovered provider

- V34 run **35364818625** completed and certified **2/15** selected providers: **MoviesHunt** and **VoirAnime**. VoirAnime duplicates the V32 recovery; **MoviesHunt is a new unique recovery**.
- **MoviesHunt = movie FULL certified** on the current rematerialized branch, with provider-returned terminal streams across `movieshunt.monster`, `huntplay.online`, `abhilinks.site`, `hubcloud.ist`, `new4.gdflix.io`.
