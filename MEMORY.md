...
- Repair #10 run `35506385523` is active on tested SHA `0b22446a2d08`; it passed the complete Repair preflight and entered real provider work. Do not attribute later HEAD fixes to this run.
- The long Learning destination was reactivated from current main: launcher `35506487133` succeeded; phase-1 Brain Lab run `35506491155` is active on SHA `5eb1e4b4d391` with a 12h persisted slot request. It passed Learning preflight, restored sanitized memory and imported native diagnostics; at this checkpoint it is inside `Build isolated current provider stage`, the exact step that failed on older runs.
