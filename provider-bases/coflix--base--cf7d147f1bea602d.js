...
const NIAKVIO_PROVIDER_MODEL = Object.freeze({"authoring":"niakvio-owned-v2","displayName":"Coflix","fixedApi":null,"knownSite":"https://coflix.esq","observedUrls":["https://npm/","https://video/","https://${/","https://vidmoly.to/","https://${_}/","https://my.mail.ru/+/video/meta/${i}","https://my.mail.ru/","https://${a}${n}/","https://${a}/","https://${n}/","https://f/","https://vidzy.live/","https://${i}/pa","https://${i}/","https://www.myvi.ru/","https://www.myvi.ru/api/video/${a[1]}","https://younetu.org/","https://vidoza.net/","https://lecteurvideo.com/","https://wookafr.center/","https://up4fun.top/","https://arm.haglund.dev/api/v2","https://v3-cinemeta/","https://${t}${n}/","https://kit/","https://coflix.esq/","https://loda/","https://openj/","http://under/"],"officialApi":null,"officialHub":"https://coflix.domains/","officialSite":"https://coflix.esq","origins":["https://npms.io","https://video.sibnet.ru","https://vidmoly.me","https://streamtape.com","https://sendvid.com","https://www.myvi.ru","https://younetu.org","https://vidoza.net","https://lecteurvideo.com","https://up4fun.top","https://arm.haglund.dev","https://v3-cinemeta.strem.io","https://kitsu.io","https://coflix.esq","https://lodash.com","https://openjsf.org","http://underscorejs.org","https://old.invalid","https://www.themoviedb.org","https://npm","https://video","https://${","https://vidmoly.to","https://${_}"],"providerId":"coflix","reconstructionState":"learning-clean-seed","routePlanVersion":1,"routes":["/","/+/video/meta/${i}","/pa","/api/video/${a[1]}","/api/v2"],"runtimeDiscovery":false,"runtimeRole":"reader","strategy":"mixed_embed_resolver","supportedTypes":["movie","tv"],"upstreamCodeEmbedded":false,"upstreamCodeExecuted":false});
...
(function(g){
  var BASE="https://coflix.esq",TMDB=(g&&g.TMDB_API_KEY)||"";
  ...
  async function meta(req){if(!TMDB)return null;var kind=req.mediaType==="tv"?"tv":"movie",data=await request("https://api.themoviedb.org/3/"+kind+"/"+encodeURIComponent(req.tmdbId)+"?api_key="+TMDB+"&language=fr-FR",true);if(!data)return null;var title=clean(data.title||data.name),original=clean(data.original_title||data.original_name),date=clean(data.release_date||data.first_air_date),year=Number(date.slice(0,4))||0;return title?{title:title,original:original,year:year}:null}
  ...
  function playerRows(html,pageUrl,title){var out=[],seen={},patterns=[/<iframe\b[^>]*(?:src|data-src)=["']([^"']+)["']/gi,/<(?:a|button)\b[^>]*(?:data-src|data-url|data-embed|data-player)=["']([^"']+)["']/gi,/["'](https?:\/\/[^"'<>\s]+(?:lecteurvideo\.com\/embed\.php|\/embed[^"'<>\s]*|\/e\/[^"'<>\s]*))["']/gi];for(var p=0;p<patterns.length;p++){var re=patterns[p],m;while((m=re.exec(String(html||"")))!==null){var u=abs(String(m[1]).split("\\/").join("/"),pageUrl);if(!/^https?:\/\//i.test(u)||seen[u]||/\.(?:css|js|png|jpe?g|gif|svg|ico)(?:[?#]|$)/i.test(u))continue;seen[u]=1;out.push({name:"Coflix",title:"[VF] Coflix - "+title,url:u,headers:{Referer:pageUrl},isDirect:false,language:"fr"});if(out.length>=16)return out}}return out}
...
/* NUVIO_TV_TARGET_MEDIA_V3:864037ea48af */
...
function clean(v){return s(v).replace(/&amp;|&#038;/gi,"&").replace(/&quot;/gi,'"').replace(/&#39;|&apos;/gi,"'").split("\\/").join("/").replace(/\\u0026/gi,"&").replace(/\\u003d/gi,"=").replace(/\\x2f/gi,"/")}
...
})(typeof globalThis!=="undefined"?globalThis:this,{"providerName":"Coflix","maxDepth":5,"maxCandidates":20,"timeoutMs":22000});
...
/* NUVIO_STREAM_OUTPUT_SANITIZER_V4:440716a7faf9 */
;(function(g,config){
  "use strict";
...
function install(container,key){
    ...
    var wrapped=async function(){
      ...
      var checked=await Promise.all(candidates.map(async function(item){
        if(!item.probe)return config.probeAllUrls?null:item.stream;
...
/* NUVIO_VF_CATALOGUE_RECOVERY_V1:3fc48281f38e */
;(function(g,config){
  "use strict";
...
})(typeof globalThis!=="undefined"?globalThis:this,{"implementationVersion":3,"strategy":"html","baseUrl":"https://coflix.esq","apiUrl":"","types":["movie","tv","anime"],"searchPaths":["/?s={query}"],"directPaths":["/film/{slug}/"],"apiRoutes":[],"blockedHosts":["fstream.top"],"blockedPathPatterns":["/wp-admin/","/wp-content/plugins/ajax-search-lite/","/wp-json/"],"preferredPlayerGroups":["VFF","VFQ","VF","Default","VOSTFR"],"maxPlayers":12,"timeoutMs":7000,"providerName":"Coflix","recoveryFirst":true,"skipNativeWhenUnresolved":false});
