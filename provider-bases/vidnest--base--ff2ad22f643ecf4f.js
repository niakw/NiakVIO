...
/* NUVIO_PROVIDER_SECURITY_HARDENING_V1:dd4a97437ea7 */
function __nuvioHostMatches(value,expected){
  try{
...
function decryptAesGcm(encryptedB64, passphraseB64) {
    __nuvioProviderSilentLog('[Vidnest] Starting AES-GCM decryption via server...');
    ...
    .then(data => {
        ...
        __nuvioProviderSilentLog('[Vidnest] Server decryption successful');
        return data.decrypted;
    })
    .catch(error => {
        __nuvioProviderSilentLog(`[Vidnest] Server decryption failed: ${error.message}`);
        throw error;
    });
}
...
function validateStreamUrl(url, headers) {
    __nuvioProviderSilentLog(`[Vidnest] Validating stream URL: ${url.substring(0, 60)}...`);
    ...
    if (url.includes('.m3u8') || url.includes('/streamsvr/') || url.includes('/stream2/')) {
        __nuvioProviderSilentLog(`[Vidnest] Skipping validation for HLS/protected stream`);
    ...
    .then(response => {
        ...
        __nuvioProviderSilentLog(`[Vidnest] URL validation result: ${response.status} - ${isValid ? 'VALID' : 'INVALID'}`);
        return isValid;
    })
    .catch(error => {
        __nuvioProviderSilentLog(`[Vidnest] URL validation failed: ${error.message}`);
...
function makeRequest(url, options = {}) {
    ...
    return fetch(url, {
        ...
        __nuvioProviderSilentLog(`[Vidnest] Request failed for ${url}: ${error.message}`);
        throw error;
    });
}
...
function wrapUrlWithProxy(url) {
    ...
    if (__nuvioHostMatches(url,"flashstream.cc") || 
        ...
        __nuvioProviderSilentLog(`[Vidnest] Wrapping URL through proxy: ${url.substring(0, 60)}...`);
        return proxiedUrl;
    }
...
function processVidnestResponse(data, serverName, mediaInfo, seasonNum, episodeNum) {
    ...
    try {
        __nuvioProviderSilentLog(`[Vidnest] Processing response from ${serverName}:`, JSON.stringify(data, null, 2));
        ...
        if (!Array.isArray(sources) || sources.length === 0) {
            __nuvioProviderSilentLog(`[Vidnest] ${serverName}: No sources/streams found in response`);
            return streams;
        }
        ...
        sources.forEach((source, index) => {
            ...
            if (!videoUrl) {
                __nuvioProviderSilentLog(`[Vidnest] ${serverName}: Source ${index} has no video URL`);
                return;
            }
            ...
            __nuvioProviderSilentLog(`[Vidnest] ${serverName}: Added ${quality}${languageInfo} stream: ${videoUrl.substring(0, 60)}...`);
        });
        
    } catch (error) {
        __nuvioProviderSilentLog(`[Vidnest] Error processing ${serverName} response: ${error.message}`);
    }
...
function fetchFromServer(serverName, mediaType, tmdbId, mediaInfo, seasonNum, episodeNum) {
    __nuvioProviderSilentLog(`[Vidnest] Fetching from ${serverName}...`);
    ...
    __nuvioProviderSilentLog(`[Vidnest] ${serverName} API URL: ${apiUrl}`);
    ...
    return makeRequest(apiUrl)
        ...
        .then(function(responseText) {
            __nuvioProviderSilentLog(`[Vidnest] ${serverName} response length: ${responseText.length} characters`);
            ...
            try {
                ...
                if (data.encrypted && data.data) {
                    __nuvioProviderSilentLog(`[Vidnest] ${serverName}: Detected encrypted response, decrypting...`);
                    
                    return decryptAesGcm(data.data, PASSPHRASE)
                        .then(function(decryptedText) {
                            __nuvioProviderSilentLog(`[Vidnest] ${serverName}: Decryption successful`);
                            
                            try {
                                const decryptedData = JSON.parse(decryptedText);
                                return processVidnestResponse(decryptedData, serverName, mediaInfo, seasonNum, episodeNum);
                            } catch (parseError) {
                                __nuvioProviderSilentLog(`[Vidnest] ${serverName}: JSON parse error after decryption: ${parseError.message}`);
                                return [];
                            }
                        });
                } else {
                ...
                __nuvioProviderSilentLog(`[Vidnest] ${serverName}: Invalid JSON response: ${parseError.message}`);
                return [];
            }
        })
        .catch(function(error) {
            __nuvioProviderSilentLog(`[Vidnest] ${serverName} error: ${error.message}`);
            return [];
        });
}
...
function getStreams(tmdbId, mediaType, seasonNum, episodeNum) {
    __nuvioProviderSilentLog(`[Vidnest] Starting extraction for TMDB ID: ${tmdbId}, Type: ${mediaType}${mediaType === 'tv' ? `, S:${seasonNum}E:${episodeNum}` : ''}`);
    
    return new Promise((resolve, reject) => {
        // First, fetch media details from TMDB
        getTMDBDetails(tmdbId, mediaType)
            .then(function(mediaInfo) {
                __nuvioProviderSilentLog(`[Vidnest] TMDB Info: "${mediaInfo.title}" (${mediaInfo.year || 'N/A'})`);
                ...
                return Promise.all(serverPromises)
                    .then(function(results) {
                        ...
                        __nuvioProviderSilentLog(`[Vidnest] Validating ${uniqueStreams.length} streams...`);
                        ...
                        return Promise.all(validationPromises)
                            .then(function(results) {
                                ...
                                __nuvioProviderSilentLog(`[Vidnest] Filtered ${uniqueStreams.length - validStreams.length} broken links`);
                        ...
                        const getQualityValue = (quality) => {
                            ...
                            return 1;
                                ...
                                __nuvioProviderSilentLog(`[Vidnest] Total valid streams found: ${validStreams.length}`);
                                resolve(validStreams);
                            });
                    });
            })
            .catch(function(error) {
                __nuvioProviderSilentLog(`[Vidnest] Error fetching media details: ${error.message}`);
