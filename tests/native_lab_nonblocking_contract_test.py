...
for name, workflow in texts.items():
    ...
    assert "\n  schedule:" not in workflow, f"{name} full native Lab must be manual-only"
    assert "cron:" not in workflow, f"{name} full native Lab must not keep an automatic cron"
...
print("native Lab nonblocking contract passed: manual-only=true push=false cron=false tv=single-job android=separate ios=separate brain=decoupled")
