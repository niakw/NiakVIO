...
def result(client: str, fixture: str, provider: str, count: int, request_type: str = "movie") -> str:
    return (
        f"FIELD_NATIVE_RESULT client={client} fixture={fixture} provider64={b64(provider)} "
        f"request_type={request_type} route_mode=declared enabled=true duration_ms=1 count={count}"
    )
...
with tempfile.TemporaryDirectory(prefix="niakvio-cross-runtime-") as tmp:
    ...
    # A client returning zero everywhere while its official peer succeeds is blocking.
