...
def candidate(cid: str, source: str, *, language: list[str] | None = None, types: list[str] | None = None, formats: list[str] | None = None, enabled: bool = True, manifest_origin: str = "live") -> dict:
    return {"canonical_id": cid, "upstream_id": cid.upper(), "source": source, "source_priority": {"gowaru": 0, "aio": 1, "yoru": 2}.get(source, 99), "manifest_origin": manifest_origin, "metadata": {"id": cid.upper(), "name": cid.title(), "enabled": enabled, "contentLanguage": language or [], "supportedTypes": types or [], "formats": formats or []}, "canonical_metadata": {"contentLanguage": language or [], "supportedTypes": types or [], "formats": formats or []}}
...
def main() -> int:
    ...
    assert french["sources"] == ["aio", "yoru"] and french["variantCount"] == 2 and french["liveManifestObserved"] is True
    assert french["reviewRequired"] is True and french["automaticImportAllowed"] is False and french["automaticActivationAllowed"] is False
    ...
    assert "new-french" in md.lower() and "never imports, enables, disables or publishes" in md and "LKG snapshot" in md
    ...
    assert "git push" not in workflow and "gh pr" not in workflow and "gh issue" not in workflow
