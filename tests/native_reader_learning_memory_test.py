...
with tempfile.TemporaryDirectory(dir=ROOT) as tmp_raw:
    tmp = Path(tmp_raw)
    base = tmp / "base.json"
    base.write_text(json.dumps({"publicationAllowed": False, "productionWritesAllowed": False, "proposals": []}), encoding="utf-8")
    ...
    assert run.returncode == 0, run.stdout + run.stderr
    recovered_entry = json.loads(recovered.read_text(encoding="utf-8"))["nativeReaderRepairMemory"]["entries"][0]
    assert recovered_entry["successes"] == 1 and recovered_entry["consecutiveFailures"] == 0, recovered_entry

print("native reader learning memory tests passed")
