...
assert 'python scripts/sync_release_versions.py --manifest manifest.json --previous "$PREVIOUS_MANIFEST"' in workflow

build = workflow.index("Build canonical publication transaction")
version = workflow.index('python scripts/sync_release_versions.py --manifest manifest.json --previous "$PREVIOUS_MANIFEST"', build)
catalog = workflow.index("node engine_v2/scripts/bootstrap-provider-catalog.mjs", version)
activation = workflow.index("python scripts/validate_activation_preservation.py", version)
...
assert build < version < activation < catalog < hashes < commit < push < verify
