...
assert "NIAKVIO_MUGIWARA_EPISODES_SCRIPT_V1" in text
assert '"/api/suggest/lookup?q="+encodeURIComponent(title)' in text
assert '"/api/episodes-script/version?url="+encodeURIComponent(rel)' in text
assert '"/api/episodes-script?url="+encodeURIComponent(rel)+"&filever="+encodeURIComponent(filever)' in text
...
assert 'JSON.parse(raw)' in text
block = text[text.index("NIAKVIO_MUGIWARA_EPISODES_SCRIPT_V1"):text.index("async function resolve(a,ctx)")]
...
print("Mugiwara same-origin discovery-first + episodes-script V2 contract passed")
