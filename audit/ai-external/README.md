...
- `.github/workflows/external-code-audit.yml` — connects to SonarQube Cloud, DeepSource GraphQL API and CodeScene REST API and writes the AI-readable evidence in this directory.
...
A manual run of **Refresh External Code Audit** always forces a new collection. Use that after changing the Sonar organization key, rotating `DEEPSOURCE_API`, rotating the CodeScene token, changing an external analysis configuration, or whenever fresh evidence is wanted immediately.
