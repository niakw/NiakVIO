...
Hard invariant: executable top-level route alternatives are <=3. Multi-hop,
player fan-out and source resolution belong inside one structured recipe/plan;
they are evidence/steps, not independent top-level routes.
"""
...
def cap_runtime_routes(
    ...
    """Return <=3 top-level executable routes while preserving semantic coverage."""
    ...
    if len(current) <= MAX_RUNTIME_ROUTE_PLANS:
        ...
        return current, {
            "before": len(current), "after": len(current),
            "supportedLanes": sorted(lanes), "coveredLanes": sorted(covered), "capped": False,
        }
    ...
    # Prefer one route proven across several semantic lanes. Otherwise select
    # the strongest entry per missing movie/tv/anime lane.
    ...
    # Old baselines may contain routes with insufficient routeData. They are not
    # exempt from the runtime cap; keep only the strongest identity-bearing
    # entries. Downstream fan-out remains available through routeData/recipes.
    if not selected:
    ...
    selected = unique(selected, MAX_RUNTIME_ROUTE_PLANS)
    covered = set().union(*(meta.get(route, {}).get("lanes", set()) for route in selected)) if selected else set()
    return selected, {
        "before": len(current), "after": len(selected),
        "supportedLanes": sorted(lanes), "coveredLanes": sorted(covered),
...
def enforce_runtime_route_cap(
    ...
    *,
    ...
    for provider_id, static_row in providers.items():
        ...
        before_routes = unique([*(model.get("routes") or []), *(patch.get("learned_routes") or [])], 256)
        ...
        proof.update({
            ...
            "runtimeFanoutBelongsInsideStructuredPlan": True,
        })
    ...
    return {
        "providerCount": len(providers), "changedProviders": changed,
        "cappedProviders": capped, "maxBefore": max_before, "maxAfter": max_after,
