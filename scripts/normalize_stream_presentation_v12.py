#!/usr/bin/env python3
"""Enforce the permanently materialized NiakVIO stream presentation V13.
...
REVISION = "all-providers-title-quality-ordered-description-v13"
...
def normalize(*, apply: bool) -> list[str]:
    ...
    if REVISION not in text:
        raise ValueError(
            "stream presentation V13 source is not materialized; "
...
def assert_contract() -> None:
    ...
    for token in (
        ...
        if token not in text:
            raise ValueError(f"stream presentation V13 contract missing: {token}")
    ...
    if technical_start < 0 or technical_end < 0:
        raise ValueError("stream presentation V13 technical line missing")
...
def main() -> int:
    ...
    if args.check and changed:
        raise SystemExit("stream presentation V13 normalization required: " + ", ".join(changed))
    if args.apply or args.check:
        assert_contract()
    print(f"FIELD_STREAM_PRESENTATION_V13 changed={len(changed)} revision={REVISION} badge_feeds=external_owner")
    return 0
