#!/usr/bin/env python3
"""Enforce the permanently materialized NiakVIO stream presentation V16.
...
def normalize(*, apply: bool) -> list[str]:
    ...
    # but V16 is now a committed fixed point: no hidden staging or one-shot rewrite
    # is permitted during a build/test run.
    _ = apply
    text = CORE.read_text(encoding="utf-8")
    if REVISION not in text:
        raise ValueError(
            "stream presentation V16 source is not materialized; "
            "global_stream_presentation_v1.py must contain the canonical V15 source"
        )
    return []
...
def assert_contract() -> None:
    ...
    for token in (
        ...
        if token not in text:
            raise ValueError(f"stream presentation V16 contract missing: {token}")
    ...
    if technical_start < 0 or technical_end < 0:
        raise ValueError("stream presentation V16 technical line missing")
...
def main() -> int:
    ...
    if args.check and changed:
        raise SystemExit("stream presentation V16 normalization required: " + ", ".join(changed))
    if args.apply or args.check:
        assert_contract()
    print(f"FIELD_STREAM_PRESENTATION_V16 changed={len(changed)} revision={REVISION} badge_feeds=external_owner")
    return 0
