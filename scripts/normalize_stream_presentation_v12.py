#!/usr/bin/env python3
"""Validate the permanently materialized NiakVIO stream presentation V19.
...
def normalize(*, apply: bool) -> list[str]:
    # ``apply`` is retained for compatibility only. V19 is a committed fixed point:
    # no hidden staging, formatting or one-shot rewrite is permitted during a build/test run.
    _ = apply
    text = CORE.read_text(encoding="utf-8")
    if REVISION not in text:
        raise ValueError(
            "stream presentation V19 source is not materialized; "
...
def assert_contract() -> None:
    ...
    for token in (
        ...
        if token not in text:
            raise ValueError(f"stream presentation V19 contract missing: {token}")
    ...
    if technical_start < 0 or technical_end < 0:
        raise ValueError("stream presentation V19 technical line missing")
...
def main() -> int:
    ...
    if args.check and changed:
        raise SystemExit("stream presentation V19 validation drift: " + ", ".join(changed))
    if args.apply or args.check:
        assert_contract()
    print(f"FIELD_STREAM_PRESENTATION_V19 changed={len(changed)} revision={REVISION} badge_feeds=external_owner read_only=true")
    return 0
