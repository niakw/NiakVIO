...
echo "FIELD_NATIVE_CORPUS_TV_PROFILE fixtures=${#FIXTURES[@]} provider=${TARGET_PROVIDER:-all} player_probes=$PLAYER_PROBES require_reader_success=$REQUIRE_READER_SUCCESS reuse_avd=true reuse_gradle_daemon=true"
...
echo "FIELD_NATIVE_CORPUS_TV_SUITE_STATUS status=$STATUS fixtures=${#FIXTURES[@]} clients=1 provider=${TARGET_PROVIDER:-all} require_reader_success=$REQUIRE_READER_SUCCESS"
exit "$STATUS"
