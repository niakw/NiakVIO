...
def instrument_tv(repo: Path) -> None:
    ...
    text = replace_once(
        ...
        "performNativeFetch(url, method, headersJson, body, scraperId, inFlightCalls)",
        "tv fetch call",
    )
    text = replace_once(
        text,
    ...
    text = replace_once(
        ...
        """            // Default User-Agent\n            if (!headers.containsKey(\"User-Agent\")) {\n                headers[\"User-Agent\"] = \"Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36\"\n            }\n            val evidenceHeaderNames = headers.keys.map { it.lowercase() }.distinct().sorted().joinToString(\",\")\n            Log.i(\"NiakvioEvidence\", \"FIELD_NATIVE_HTTP_REQUEST client=tv provider=$scraperId method=${method.uppercase()} endpoint=$evidenceEndpoint header_names=$evidenceHeaderNames body_bytes=${body.toByteArray(Charsets.UTF_8).size}\")\n\n            val requestBuilder = Request.Builder()\n""",
        "tv request evidence",
    )
    text = replace_once(
        text,
...
def instrument_mobile(repo: Path) -> None:
    ...
    runtime_text = replace_once(runtime_text, "addModule(FetchBridge())", "addModule(FetchBridge(scraperId))", "mobile bridge ownership")
    bridge_text = replace_once(
        bridge_text,
        "internal class FetchBridge : HostModule {",
        "internal class FetchBridge(private val scraperId: String) : HostModule {",
    ...
    bridge_text = replace_once(
        ...
        """        val responseHeaders = response.headers.mapKeys { (key, _) -> key.lowercase() }\n            .mapValues { (_, value) -> truncateString(value, MAX_FETCH_HEADER_VALUE_CHARS) }\n        val finalEndpoint = response.url.substringBefore('?').substringBefore('#').replace(Regex(\"\\\\s+\"), \"%20\")\n        val responseHeaderNames = responseHeaders.keys.sorted().joinToString(\",\")\n        log.i { \"FIELD_NATIVE_HTTP_RESPONSE client=mobile provider=$scraperId method=${method.uppercase()} endpoint=$endpoint final_endpoint=$finalEndpoint status=${response.status} duration_ms=${evidenceStartedAt.elapsedNow().inWholeMilliseconds} response_header_names=$responseHeaderNames body_chars=${response.body.length}\" }\n        val result = JsonObject(\n""",
