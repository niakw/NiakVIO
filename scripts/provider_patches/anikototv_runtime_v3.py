...
function megaDecrypt(data){var enc=s(data&&data.enc);diag("anikoto_megaplay_enc","enc="+(enc?1:0)+";len="+String(enc.length));if(!enc||typeof require!=="function"){diag("anikoto_megaplay_crypto","require="+(typeof require==="function"?1:0));return""}try{var C=require("crypto-js");diag("anikoto_megaplay_crypto","crypto="+(C&&C.AES?1:0));var params=C.lib.CipherParams.create({ciphertext:C.enc.Base64.parse(b64std(enc))}),plain=C.AES.decrypt(params,megaKey(C),{iv:C.enc.Utf8.parse("W0;27ToaUpl_P%'c"),mode:C.mode.CBC,padding:C.pad.Pkcs7}).toString(C.enc.Utf8);diag("anikoto_megaplay_plain","plain="+(plain?1:0)+";len="+String(plain.length));var m=/"file"\s*:\s*"([^"]+)"/i.exec(plain);diag("anikoto_megaplay_file","file="+(m&&m[1]?1:0));if(!m)return"";var u=s(m[1]).replace(/\\\//g,"/");if(!/^https?:\/\//i.test(u)||/[?&]token=/i.test(u))return u;var p=/\/([a-f0-9]{32})\/([a-f0-9]{32})\//i.exec(u);if(!p){diag("anikoto_megaplay_token","pattern=0");return u}var expiry=Math.floor(Date.now()/1000)+90,payload=String(expiry)+"|"+p[1].toLowerCase()+"/"+p[2].toLowerCase(),token=b64urlWord(C.enc.Utf8.parse(payload),C)+"."+b64urlWord(C.HmacSHA256(payload,"MpCdnT0k3n!9f2K#xQ7vL5mR8wN1pY4s"),C);diag("anikoto_megaplay_token","pattern=1;token="+(token?1:0));try{var parsedUrl=new URL(u);parsedUrl.search=(parsedUrl.search?parsedUrl.search+"&":"?")+"token="+encodeURIComponent(token);return parsedUrl.toString()}catch(_e){return u}}catch(_e){diag("anikoto_megaplay_error","error=1");return""}}
