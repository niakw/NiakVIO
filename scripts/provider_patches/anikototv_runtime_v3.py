...
async function decryptMega(enc){try{var keyRaw=utf8Bytes("i?LMTAx0Q6,:}50U"),key=new Uint8Array(32),iv=utf8Bytes("W0;27ToaUpl_P%'c"),cipher=b64urlBytes(enc);for(var i=0;i<keyRaw.length&&i<32;i++)key[i]=keyRaw[i];diag("anikoto_megaplay_enc","enc=1;len="+String(s(enc).length));try{if(g&&g.crypto&&g.crypto.subtle&&typeof g.crypto.subtle.importKey==="function"){var cryptoKey=await g.crypto.subtle.importKey("raw",key,{name:"AES-CBC"},false,["decrypt"]),plainBuffer=await g.crypto.subtle.decrypt({name:"AES-CBC",iv:iv},cryptoKey,cipher),webFile=megaDecodedFile(bytesText(new Uint8Array(plainBuffer)));diag("anikoto_megaplay_webcrypto","file="+(/^https?:\/\//i.test(webFile)?1:0));if(/^https?:\/\//i.test(webFile))return webFile}}catch(_e){diag("anikoto_megaplay_webcrypto","error=1")}try{if(g&&typeof g.__crypto_aes_decrypt_raw==="function"){var plain=g.__crypto_aes_decrypt_raw("AES-CBC",new Int8Array(key.buffer.slice(0)),new Int8Array(iv.buffer.slice(0)),new Int8Array(cipher.buffer.slice(0))),nativeFile=megaDecodedFile(bytesText(plain));diag("anikoto_megaplay_native","file="+(/^https?:\/\//i.test(nativeFile)?1:0));if(/^https?:\/\//i.test(nativeFile))return nativeFile}}catch(_e){diag("anikoto_megaplay_native","error=1")}if(typeof require!=="function"){diag("anikoto_megaplay_cryptojs","require=0");return""}var CryptoJS=require("crypto-js"),keyWord=CryptoJS.enc.Hex.parse(bytesHex(key)),ivWord=CryptoJS.enc.Hex.parse(bytesHex(iv)),cipherWord=CryptoJS.enc.Hex.parse(bytesHex(cipher)),params=CryptoJS.lib.CipherParams.create({ciphertext:cipherWord}),plainWord=CryptoJS.AES.decrypt(params,keyWord,{iv:ivWord,mode:CryptoJS.mode.CBC,padding:CryptoJS.pad.Pkcs7}),cryptoFile=megaDecodedFile(plainWord.toString(CryptoJS.enc.Utf8));diag("anikoto_megaplay_cryptojs","file="+(/^https?:\/\//i.test(cryptoFile)?1:0));return /^https?:\/\//i.test(cryptoFile)?cryptoFile:""}catch(_e){diag("anikoto_megaplay_error","error=1");return""}}
...
async function finalSource(raw){var data=parsed(raw),src=data&&data.sources,file="";if(typeof src==="string"&&/^https?:\/\//i.test(s(src)))file=s(src);if(!file&&src&&typeof src==="object"&&!Array.isArray(src))file=s(src.file||src.url||src.src);if(!file&&Array.isArray(src)){for(var i=0;i<src.length;i++){var row=src[i],u=s(row&&typeof row==="object"?(row.file||row.url||row.src):row);if(/^https?:\/\//i.test(u)){file=u;break}}}if(!file&&data&&data.enc)file=await decryptMega(data.enc);file=normalizeMegaMedia(file,data&&data.tracks);diag("anikoto_megaplay_final","file="+(/^https?:\/\//i.test(file)?1:0));return /^https?:\/\//i.test(file)?file:""}
