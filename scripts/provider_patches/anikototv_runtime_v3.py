...
async function resolveMega(url,referer,cookie){if(!/^https?:\/\/(?:www\.)?megaplay\.buzz\//i.test(s(url)))return null;var page=await get(url,referer,false,cookie,"https://megaplay.buzz"),fid=megaFileId(page.body);if(!fid)return null;var api="https://megaplay.buzz/stream/getSources?id="+encodeURIComponent(fid);try{var playerUrl=new URL(page.url||url),sv=playerUrl.searchParams.get("s");if(sv)api+="&s="+encodeURIComponent(sv)}catch(_e){}var sources=await get(api,page.url||url,true,page.cookie||cookie,"https://megaplay.buzz"),direct=finalSource(sources.body),data=parsed(sources.body);if(!direct&&data&&typeof data==="object")direct=megaDecrypt(data);if(direct)return{url:direct,direct:true,player:page.url||url};if(data&&typeof data==="object"&&s(data.enc))return{url:page.url||url,direct:false,player:page.url||url};return null}
