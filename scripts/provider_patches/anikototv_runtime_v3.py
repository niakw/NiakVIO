...
function b64urlBytesOut(bytes){var raw="";for(var i=0;i<bytes.length;i++)raw+=String.fromCharCode(bytes[i]);return btoa(raw).replace(/\+/g,"-").replace(/\//g,"_").replace(/=+$/g,"")}
async function signMegaMedia(file){var u=s(file);if(!/^https?:\/\//i.test(u)||/[?&]token=/i.test(u))return u;var m=/\/([a-f0-9]{32})\/([a-f0-9]{32})\//i.exec(u);if(!m)return u;var expiry=Math.floor(Date.now()/1000)+90,payload=String(expiry)+"|"+m[1].toLowerCase()+"/"+m[2].toLowerCase(),secret="MpCdnT0k3n!9f2K#xQ7vL5mR8wN1pY4s",payloadBytes=utf8Bytes(payload),sig="";try{if(g&&g.crypto&&g.crypto.subtle&&typeof g.crypto.subtle.importKey==="function"){var key=await g.crypto.subtle.importKey("raw",utf8Bytes(secret),{name:"HMAC",hash:"SHA-256"},false,["sign"]),signed=await g.crypto.subtle.sign("HMAC",key,payloadBytes);sig=b64urlBytesOut(new Uint8Array(signed));diag("anikoto_megaplay_sign","webcrypto="+(sig?1:0))}}catch(_e){}if(!sig&&typeof require==="function")try{var C=require("crypto-js");sig=C.enc.Base64.stringify(C.HmacSHA256(payload,secret)).replace(/\+/g,"-").replace(/\//g,"_").replace(/=+$/g,"");diag("anikoto_megaplay_sign","cryptojs="+(sig?1:0))}catch(_e){}if(!sig)return u;var token=b64urlBytesOut(payloadBytes)+"."+sig;try{var parsedUrl=new URL(u);parsedUrl.search=(parsedUrl.search?parsedUrl.search+"&":"?")+"token="+encodeURIComponent(token);return parsedUrl.toString()}catch(_e){return u}}
...
async function resolveMega(url,referer,cookie){if(!/^https?:\/\/(?:www\.)?megaplay\.buzz\//i.test(s(url)))return null;var page=await get(url,referer,false,cookie,"https://megaplay.buzz"),fid=megaFileId(page.body);if(!fid)return null;var api="https://megaplay.buzz/stream/getSources?id="+encodeURIComponent(fid);try{var playerUrl=new URL(page.url||url),sv=playerUrl.searchParams.get("s");if(!sv){var sm=/\/stream\/s-(\d+)\//i.exec(playerUrl.pathname);if(sm)sv=sm[1]}if(sv)api+="&s="+encodeURIComponent(sv)}catch(_e){}var sessionCookie=mergeCookies(cookie,page.cookie),sources=await get(api,page.url||url,true,sessionCookie,"https://megaplay.buzz"),direct=await finalSource(sources.body),data=parsed(sources.body),playbackCookie=mergeCookies(sessionCookie,sources.cookie);if(direct){diag("anikoto_megaplay_terminal","direct=1;context=1");return{url:direct,direct:true,player:page.url||url,headers:megaPlaybackHeaders(direct,page.url||url,playbackCookie)}}if(data&&typeof data==="object"&&s(data.enc)){diag("anikoto_megaplay_terminal","direct=0;fallback=1");return{url:page.url||url,direct:false,player:page.url||url,headers:megaPlaybackHeaders(page.url||url,page.url||url,playbackCookie)}}return null}
