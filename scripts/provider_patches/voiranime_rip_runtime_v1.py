...
async function search(names){var queries=names.concat(fallbackQueries(names));for(var i=0;i<queries.length;i++){var q=queries[i],r=await T(c.base+"/template-php/defaut/fetch.php",{method:"POST",headers:{"Accept":"text/html, */*","Content-Type":"application/x-www-form-urlencoded","X-Requested-With":"XMLHttpRequest"},referer:c.base+"/",body:"query="+encodeURIComponent(q)});if(!r)continue;var rows=results(r.text),best=[];for(var ni=0;ni<names.length;ni++){var wanted=names[ni],ranked=rows.slice().sort(function(a,b){return score(b.title,wanted)-score(a.title,wanted)});for(var ri=0;ri<ranked.length;ri++)if(score(ranked[ri].title,wanted)>=40)best.push(ranked[ri])}var seen={},uniq=[];for(var bi=0;bi<best.length&&uniq.length<5;bi++)if(!seen[best[bi].slug]){seen[best[bi].slug]=1;uniq.push(best[bi])}if(uniq.length)return uniq}return[]}
