...
(function(g){
  ...
  function argsOf(args){var first=args[0],out={},ctx={};if(first&&typeof first==="object"&&!Array.isArray(first))out=Object.assign({},first);else{out.tmdbId=String(first||"");out.mediaType=String(args[1]||"movie");out.season=args[2];out.episode=args[3]}try{ctx=g&&g.__nuvioMediaContext||{}}catch(_e){}out.tmdbId=String(out.tmdbId||ctx.tmdbId||out.id||"");out.tmdbMetadata=out.tmdbMetadata||out.tmdb_metadata||ctx.tmdbMetadata||null;out.mediaType=String(out.canonicalMediaType||out.mediaType||out.type||ctx.canonicalMediaType||"movie").toLowerCase();return out}
  ...
  async function meta(req){var data=req.tmdbMetadata||req.tmdb_metadata||null;try{if(!data){var ctx=g&&g.__nuvioMediaContext;data=ctx&&ctx.tmdbMetadata||null}if(data&&data.state==="ok"&&data.metadata)data=data.metadata}catch(_e){}if(!data)return null;var title=clean(data.title||data.name),original=clean(data.original_title||data.original_name),date=clean(data.release_date||data.first_air_date),year=Number(date.slice(0,4))||0;return title?{title:title,original:original,year:year}:null}
  ...
  function playerRows(html,pageUrl,title){html=String(html||"").split("\/").join("/");var out=[],seen={},patterns=[/<iframe\b[^>]*(?:src|data-src)=["']([^"']+)["']/gi,/<(?:a|button)\b[^>]*(?:data-src|data-url|data-embed|data-player)=["']([^"']+)["']/gi,/["'](https?:\/\/[^"'<>\s]+(?:lecteurvideo\.com\/embed\.php|\/embed[^"'<>\s]*|\/e\/[^"'<>\s]*))["']/gi];for(var p=0;p<patterns.length;p++){var re=patterns[p],m;while((m=re.exec(html))!==null){var u=abs(String(m[1]).split("\\/").join("/"),pageUrl);if(!/^https?:\/\//i.test(u)||seen[u]||/\.(?:css|js|png|jpe?g|gif|svg|ico)(?:[?#]|$)/i.test(u))continue;seen[u]=1;out.push({name:"Coflix",title:"[VF] Coflix - "+title,url:u,headers:{Referer:pageUrl},isDirect:false,language:"fr"});if(out.length>=16)return out}}return out}
  function anchors(html,base){var out=[],re=/<a\b[^>]*href=["']([^"']+)["'][^>]*>([\s\S]*?)<\/a>/gi,m;while((m=re.exec(html))!==null){var u=abs(m[1],base),label=clean(m[2].replace(/<[^>]+>/g," ")).replace(/\s+/g," ");if(!u||!label||/\/wp-(?:content|admin|json|includes)\//i.test(u))continue;out.push({url:u,label:label})}return out}
