...
function megaSource(raw){var data=parsed(raw);if(!data||typeof data!=="object")return"";var src=data.sources||data.source||data.links||null;if(typeof src==="string")return /^https?:\/\//i.test(src)?src:"";if(src&&typeof src==="object"&&!Array.isArray(src)){var u=s(src.file||src.url||src.src);if(/^https?:\/\//i.test(u))return u}if(Array.isArray(src)){for(var i=0;i<src.length;i++){var row=src[i],v=typeof row==="string"?row:s(row&&(row.file||row.url||row.src));if(/^https?:\/\//i.test(v))return v}}return""}
