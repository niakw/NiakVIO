...
def apply(text: str, options: dict[str, Any] | None = None, **_kwargs: Any) -> str:
    ...
    payload = {
        ...
        "experimentVariant": max(0, min(int(cfg.get("experiment_variant", 0)), 3)),
...
function expandRoute(p,q,m,sl){var out=s(p),query=encodeURIComponent(m.title||"");return out.replace(/\{query\}/gi,query).replace(/\{slug\}/gi,sl).replace(/\{(?:id|tmdbId)\}/gi,s(q.tmdbId||q.id)).replace(/\{imdbId\}/gi,s(q.imdbId||q.imdb_id)).replace(/\{year\}/gi,String(m.year||"")).replace(/\{season\}/gi,String(Number(q.season)||1)).replace(/\{episode\}/gi,String(Number(q.episode)||1)).replace(/\{(?:mediaType|type)\}/gi,s(q.mediaType||"movie"))}
async function recover(q){if(c.types.indexOf(q.mediaType)<0)return[];var m=await meta(q);if(!m.title)return[];var cand=[],sl=slug(m.title),sharedSeen={};for(var i=0;i<c.directPaths.length;i++)cand.push(abs(expandRoute(c.directPaths[i],q,m,sl),c.baseUrl+"/"));for(var j=0;j<c.searchPaths.length;j++){var u=abs(expandRoute(c.searchPaths[j],q,m,sl),c.baseUrl+"/"),doc=await req(u,false,c.baseUrl+"/");if(doc&&doc.body)cand=cand.concat(urls(doc.body,doc.url||u).sort(function(a,b){return score(b,m,q)-score(a,m,q)}).slice(0,c.maxPages))}cand=Array.from(new Set(cand)).filter(Boolean).sort(function(a,b){return score(b,m,q)-score(a,m,q)}).slice(0,c.maxPages);var found=[];for(var k=0;k<cand.length&&found.length<c.maxEmbeds;k++){var r=await resolve(cand[k],c.baseUrl+"/",0,sharedSeen);found=found.concat(r)}return unique(found).slice(0,c.maxEmbeds).map(function(row,i){return{name:c.providerName+(i?" #"+(i+1):""),title:c.providerName+" - "+m.title,url:row.url,quality:"HD",headers:hdr(row.referer||c.baseUrl+"/",row.url),isDirect:row.direct===true||media(row.url,"","")}})}
