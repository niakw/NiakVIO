...
function addUnique(out,value){value=s(value);if(!value||placeholder(value))return;var n=norm(value);if(!n)return;for(var i=0;i<out.length;i++){var p=norm(out[i]);if(p===n||p.indexOf(n)>=0||n.indexOf(p)>=0)return}out.push(value)}
