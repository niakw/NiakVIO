...
def apply(text: str, options: dict[str, Any] | None = None, **_kwargs: Any) -> str:
    ...
    payload = {
        ...
        "searchPaths": [str(v) for v in cfg.get("search_paths", ["/?s={query}", "/search?q={query}", "/search?query={query}"]) if str(v).strip()],
        "directPaths": [str(v) for v in cfg.get("direct_paths", ["/{slug}"]) if str(v).strip()],
        "detailIdAttributes": [str(v) for v in cfg.get("detail_id_attributes", []) if str(v).strip()],
...
function install(o,k){if(!o||typeof o[k]!=="function"||o[k].__nuvioGlobalCatalogueAliasV2)return false;var native=o[k];var wrap=async function(){var q=args(arguments),v;try{v=await native.apply(this,arguments)}catch(_){v=[]}var x=slot(v);if(!x||!x.list.length)return v;var m=null;try{m=await meta(q)}catch(_){m=null}if(!m||!m.titles||!m.titles.length)return v;var kept=x.list.filter(function(row){return !nativeIdentityReject(row,q,m)});return rebuild(v,x,kept)};wrap.__nuvioGlobalCatalogueAliasV2=true;o[k]=wrap;return true}
