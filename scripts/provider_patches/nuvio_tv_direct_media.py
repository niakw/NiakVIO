...
function unescapeJs(v){var raw=str(v),out="";for(var i=0;i<raw.length;i++){var ch=raw.charAt(i);if(ch!=="\\"||i+1>=raw.length){out+=ch;continue}var next=raw.charAt(++i),hex;if(next==="u"&&(hex=raw.slice(i+1,i+5)).length===4&&/^[0-9a-fA-F]{4}$/.test(hex)){out+=String.fromCharCode(parseInt(hex,16));i+=4;continue}if(next==="x"&&(hex=raw.slice(i+1,i+3)).length===2&&/^[0-9a-fA-F]{2}$/.test(hex)){out+=String.fromCharCode(parseInt(hex,16));i+=2;continue}if(/[0-7]/.test(next)){var oct=next;while(oct.length<3&&i+1<raw.length&&/[0-7]/.test(raw.charAt(i+1)))oct+=raw.charAt(++i);out+=String.fromCharCode(parseInt(oct,8));continue}if(next==="n"){out+="\n";continue}if(next==="r"){out+="\r";continue}if(next==="t"){out+="\t";continue}if(next==="b"){out+="\b";continue}if(next==="f"){out+="\f";continue}if(next==="v"){out+="\v";continue}out+=next}return out}
