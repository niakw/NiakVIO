#!/usr/bin/env python3
"""Global ID-first catalogue fallback for HTML/mixed providers.

The provider contract remains TMDB/IMDb-first. This wrapper only runs when the
native provider returns no streams. It resolves TMDB metadata itself, builds a
bounded set of localized/original aliases, searches the provider's configured
official site, validates the content page against title/year (and episode when
applicable), and returns player/media candidates. No title-specific behaviour is
encoded here.
"""
...
MARKER = "NUVIO_GLOBAL_CATALOGUE_ALIAS_RECOVERY_V1"
...
def apply(text: str, options: dict[str, Any] | None = None, **_kwargs: Any) -> str:
    ...
    payload = {
        ...
        "maxAliases": max(2, min(int(cfg.get("max_aliases", 6)), 10)),
