...
function movieLinks(html,expected){var out=[],re=/<a[^>]+href=["']([^"']*\/movie\/[^"'#?]+)["'][^>]*>([\s\S]*?)<\/a>/gi,m,n=norm(expected);while((m=re.exec(html))!==null){var u=decode(m[1]),label=norm(m[2].replace(/<[^>]+>/g,' '));if(n&&label&&label!==n&&label.indexOf(n)<0&&n.indexOf(label)<0)continue;try{u=new URL(u,c.base).toString()}catch(_e){continue}if(u.indexOf(c.base+"/movie/")===0&&!out.includes(u))out.push(u)}return out}
