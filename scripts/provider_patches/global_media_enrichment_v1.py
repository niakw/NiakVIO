...
NuvioTV's native QuickJS Response is text-oriented and can lack arrayBuffer().
Opaque binary media URLs therefore cannot rely only on file signatures. The
resolver also proves direct containers from trustworthy final response metadata
(Content-Type, Content-Disposition and the final redirect URL) without consuming
the binary body. This is important for opaque Worker/CDN URLs and one-shot media.
"""
...
def apply(text: str, options: dict[str, Any] | None = None, **_kwargs: Any) -> str:
    ...
    payload = {
        ...
        "dropResolvedOriginal": bool(cfg.get("drop_resolved_original", True)),
        "defaultUserAgent": str(cfg.get("default_user_agent") or ""),
        "implementationRevision": "scoped-playback-context-v5-opaque-media",
    }
...
async function fetchResource(url,row,referer,jar){try{
  ...
  // Do not consume an opaque binary response merely to prove its container. In
  // native QuickJS this avoids both the missing-arrayBuffer blind spot and
  // unnecessary second reads of tokenized/one-shot media endpoints.
