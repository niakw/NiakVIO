#!/usr/bin/env python3
"""Globally enrich embed/player rows with proven direct media when available.

This is not a transcoder. It preserves the provider's original stream row and
adds a direct HLS/DASH/container row only after probing the returned resource or
a bounded child candidate. It therefore improves Nuvio playback without
requiring a proxy/transmux service and without breaking iframe-only providers.
"""
