...
or a clearly content-like title/year contradiction.

For episodic requests TMDB series and episode titles are both accepted identities,
which prevents legitimate anime/TV episode labels from being discarded merely
because they do not repeat the series name.
"""
...
def apply(text: str, options: dict[str, Any] | None = None, **kwargs: Any) -> str:
    ...
    payload = {
        ...
        "implementationRevision": "cross-client-positive-mismatch-anime-episode-v2",
    }
...
function explicitCandidates(row){var out=[];var title=s(row&&row.title);if(title)out.push({text:title,kind:"title"});var filename=s(row&&row.filename);if(filename)out.push({text:filename,kind:"filename"});try{var base=decodeURIComponent(new URL(s(row&&row.url)).pathname.split("/").filter(Boolean).pop()||"").replace(/\.(?:m3u8|mpd|mp4|mkv|webm|m4v|ts)$/i,"");if(base)out.push({text:base,kind:"url"})}catch(_e){}var name=s(row&&row.name);if(name&&norm(name)!==norm(c.providerId))out.push({text:name,kind:"name"});return out}
function contentLike(candidate){var w=tokens(candidate.text),se=episode(candidate.text),years=norm(candidate.text).match(/\b(?:19|20)\d{2}\b/g)||[];if(se)return true;if(years.length&&w.length>=1)return true;if(candidate.kind==="title"&&w.length>=3)return true;if(candidate.kind==="filename"&&w.length>=3)return true;return false}
function candidateContradicts(candidate,q,m,expected){var text=candidate.text,se=episode(text);if(q.mediaType==="movie"&&se)return true;if(se&&episodic(q)){var ss=Number(se[1])||0,ee=Number(se[2])||0;if((q.season&&ss&&ss!==q.season)||(q.episode&&ee&&ee!==q.episode))return true;if((!q.season||ss===q.season)&&(!q.episode||ee===q.episode))return false}var years=norm(text).match(/\b(?:19|20)\d{2}\b/g)||[];if(m.year&&years.length&&!years.some(function(y){return Math.abs(Number(y)-Number(m.year))<=1}))return true;if(!contentLike(candidate))return false;var w=tokens(text);if(w.length<2)return false;for(var i=0;i<w.length;i++)if(expected[w[i]])return false;var hasYear=years.length>0;if(hasYear)return true;return w.length>=3&&(candidate.kind==="title"||candidate.kind==="filename")}
function mismatch(row,q,m){var ids=explicitIds(row);if(ids.tmdbId&&q.tmdbId&&ids.tmdbId!==q.tmdbId)return true;if(ids.imdbId&&(q.imdbId||m.imdbId)&&ids.imdbId!==(q.imdbId||m.imdbId))return true;var expected=expectedTokens(m),cands=explicitCandidates(row);for(var i=0;i<cands.length;i++)if(candidateContradicts(cands[i],q,m,expected))return true;return false}
function install(o,k){if(!o||typeof o[k]!=="function"||o[k].__nuvioGlobalStreamIdentityV1)return false;var native=o[k];var wrap=async function(){var q=req(arguments),v=await native.apply(this,arguments),x=slot(v);if(!x||!x.list.length)return v;var m=await tmdb(q);var kept=x.list.filter(function(row){return !mismatch(row,q,m)});return rebuild(v,x,kept)};wrap.__nuvioGlobalStreamIdentityV1=true;o[k]=wrap;return true}
