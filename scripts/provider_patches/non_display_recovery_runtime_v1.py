...
function strip(v){var src=s(v),out="",inTag=false,tag="",tagDone=false,closing=false,suppress=false;for(var i=0;i<src.length;i++){var ch=src.charAt(i);if(!inTag&&ch==="<"){inTag=true;tag="";tagDone=false;closing=false;if(src.charAt(i+1)==="/"){closing=true;i++}out+=" ";continue}if(inTag){if(ch===">"){var name=tag.toLowerCase();if(closing&&(name==="script"||name==="style"))suppress=false;else if(!closing&&(name==="script"||name==="style"))suppress=true;inTag=false;continue}if(!tagDone){if(/\s/.test(ch)||ch==="/")tagDone=true;else tag+=ch}continue}if(!suppress)out+=ch}return out.replace(/&(?:nbsp|amp|quot|#0*39);/gi," ").replace(/\s+/g," ").trim()}
