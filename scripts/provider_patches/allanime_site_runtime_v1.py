...
function siteCandidates(html,page){var out=[],seen={};function add(v){var u=s(v).replace(/&amp;/gi,"&").replace(/\\\//g,"/");try{u=new URL(u,page).toString()}catch(_e){return}if(!/^https?:\/\//i.test(u)||seen[u])return;seen[u]=1;out.push(u)}var re=/(?:src|href|data-src)\s*=\s*["']([^"']+)["']/gi,m;while((m=re.exec(html||""))!==null&&out.length<60)add(m[1]);var raw=/https?:\\?\/\\?\/[^"'\\s<>]+/gi,x;while((x=raw.exec(html||""))!==null&&out.length<80)add(x[0]);return out}
