...
async function recover(q){if(["movie","tv","anime"].indexOf(q.mediaType)<0)return [];var m=await meta(q);if(!m.title)return [];var candidates=[c.baseUrl+"/"+slug(m.title)],searches=[c.baseUrl+"/?s="+encodeURIComponent(m.title),c.baseUrl+"/search?q="+encodeURIComponent(m.title)];for(var i=0;i<searches.length;i++){var sr=await request(searches[i],"text",c.baseUrl+"/");if(sr)candidates=candidates.concat(detailLinks(sr.body,sr.url,m).slice(0,5))}candidates=unique(candidates);var streams=[];for(var j=0;j<candidates.length&&streams.length<12;j++){var page=await request(candidates[j],"text",c.baseUrl+"/");if(!page)continue;var data=pageData(page.body,page.url);if(scoreText(page.body,m)<20&&!data.id&&!data.urls.length)continue;var urls=data.urls.slice();if(data.id){var mirrors=await request(c.baseUrl+"/api/mirrors/film/"+encodeURIComponent(data.id),"json",page.url);if(mirrors)urls=urls.concat(mirrorUrls(mirrors.body,c.baseUrl+"/",[]))}urls=unique(urls);for(var k=0;k<urls.length;k++){var u=urls[k];streams.push({name:c.providerName+(streams.length?" #"+(streams.length+1):""),title:c.providerName+" - "+m.title,url:u,quality:"HD",language:"fr",isDirect:/\.(?:m3u8|mp4|mpd)(?:[?#]|$)/i.test(u),headers:{Referer:page.url,Origin:c.baseUrl}})}if(streams.length)break}return streams}
