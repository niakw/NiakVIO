...
function args(a){var q=a[0]&&typeof a[0]==="object"?Object.assign({},a[0]):{tmdbId:a[0],mediaType:a[1],season:a[2],episode:a[3],settings:a[4]||{}},ctx={};try{ctx=g&&g.__nuvioMediaContext||{}}catch(_){}q.tmdbId=s(q.tmdbId||ctx.tmdbId||q.id);q.tmdbMetadata=q.tmdbMetadata||q.tmdb_metadata||ctx.tmdbMetadata||null;q.mediaType=s(q.canonicalMediaType||q.mediaType||q.type||ctx.canonicalMediaType||"movie").toLowerCase();return q}
...
async function meta(q){var title=s(q.title||q.name||q.label).replace(/\s*\(\d{4}\)\s*$/,""),year=Number(q.year)||0,d=q.tmdbMetadata||q.tmdb_metadata||null;try{if(!d){var ctx=g&&g.__nuvioMediaContext;d=ctx&&ctx.tmdbMetadata||null}if(d&&d.state==="ok"&&d.metadata)d=d.metadata}catch(_){}if(d){title=title||s(d.title||d.name);year=year||Number(s(d.release_date||d.first_air_date).slice(0,4))||0}return {title:title,year:year}}
