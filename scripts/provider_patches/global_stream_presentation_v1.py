...
function languageLine(f){var tracks=(f.languageTracks||[]).map(fullTrack).filter(Boolean);if(tracks.length)return"🌐 "+tracks.join(" • ");var subs=(f.subtitles||[]).filter(Boolean),code=languageCode(f.language),lang=code?languageName(code):"";if(!lang&&!subs.length)return"";return"🌐 "+[lang,subs.length?"💬 "+subs.join(" • "):""].filter(Boolean).join(" • ")}
