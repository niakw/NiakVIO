...
function language(r){var direct=first(r&&r.language,r&&r.lang,r&&r.audioLanguage),u=(direct||blob(r)).toUpperCase();if(/\bMULTI(?:[- ]?AUDIO|LANG(?:UE)?S?)?\b/.test(u))return"Multi";if(/\bDUAL(?:[- ]?AUDIO)?\b/.test(u))return"Dual Audio";if(/\bVOSTFR\b/.test(u))return"VOSTFR";if(/\bVFQ\b/.test(u))return"VFQ";if(/\bVFF\b/.test(u))return"VFF";if(/\bVF\b/.test(u))return"VF";if(/\bVO\b/.test(u))return"VO";return direct}
...
function providerName(r){var raw=first(r&&r.name,r&&r.provider),n=raw.split(/[|\n]/)[0].trim();if(n&&n.length<=48&&!/^(?:4k|2160p|1080p|720p|vf|vff|vfq|vostfr|dual audio)$/i.test(n))return n;var id=s(c.providerId).replace(/[-_]+/g," ");return id?id.replace(/\b\w/g,function(x){return x.toUpperCase()}):"Source"}
function fileSize(r){var values=[r&&r.fileSize,r&&r.filesize,r&&r.size];for(var i=0;i<values.length;i++){var v=s(values[i]);if(meaningful(v)&&/\b\d+(?:[.,]\d+)?\s*(?:KB|MB|GB|TB)\b/i.test(v))return v}return""}
