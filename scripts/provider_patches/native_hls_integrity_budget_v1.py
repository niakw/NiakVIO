...
Nuvio native clients expose a synchronous __native_fetch host bridge. The HLS
integrity wrapper's AbortController timer cannot reliably interrupt an HTTP call
...
On native runtimes this patch skips *additional* HLS-integrity network probes. The
provider's own resolution still runs, and the final runtime-capability safety layer
still performs deterministic URL rejection. Desktop/web-like runtimes retain the
full HLS validation/recovery path.
"""
