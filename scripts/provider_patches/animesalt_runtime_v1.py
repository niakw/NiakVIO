...
async function postJson(url,body,ref,origin){try{var r=await g.fetch(url,{method:"POST",headers:headers(ref,{"Content-Type":"application/x-www-form-urlencoded","Origin":origin||c.base,"X-Requested-With":"XMLHttpRequest"}),body:body,redirect:"follow"});if(!r||!r.ok)return null;return await r.json()}catch(_e){return null}}
...
async function findSeries(m){var p=await getText(c.base+"/?s="+encodeURIComponent(m.title),c.base+"/");if(!p)return null;var rows=searchRows(p.text,p.url,m.title,m.year);for(var i=0;i<rows.length;i++)if(rows[i].type==="series")return rows[i];return null}
...
async function episodePage(series,q){var root=await getText(series.url,c.base+"/");if(!root)return null;var direct=episodeHref(root.text,q.season,q.episode,root.url);if(direct)return await getText(direct,series.url);
  ...
  var ajax=c.base+"/wp-admin/admin-ajax.php?action=action_select_season&season="+encodeURIComponent(String(q.season))+"&post="+encodeURIComponent(chosen.post),ap=await getText(ajax,series.url);if(!ap)return null;var href=episodeHref(ap.text,q.season,q.episode,ap.url);return href?await getText(href,series.url):null}
async function terminal(page,q,m){var hit=/src=["'](https:\/\/as-cdn\d+\.top\/video\/([a-f0-9]+))["']/i.exec(page.text||"");if(!hit)return[];var player=hit[1],hash=hit[2],origin="";try{origin=new URL(player).origin}catch(_e){return[]}
  var data=await postJson(origin+"/player/index.php?data="+encodeURIComponent(hash)+"&do=getVideo","hash="+encodeURIComponent(hash)+"&r="+encodeURIComponent(c.base+"/"),c.base+"/",origin);if(!data)return[];var url=s(data.videoSource||data.securedLink);if(!/^https?:/i.test(url))return[];var row={name:"AnimeSalt",title:m.title+" | S"+q.season+"E"+q.episode+" | Multi",url:url,quality:"1080p",language:"Multi",provider:"animesalt",headers:{Referer:origin+"/",Origin:origin,"User-Agent":c.userAgent}};return[row]}
