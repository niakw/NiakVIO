...
budget. In NuvioMobile/NuvioTV (and native Desktop bridges) __native_fetch is a
synchronous host call, so JS AbortSignal cannot reliably interrupt an in-flight HTTP
request. A loop of 8 aliases x 3 search routes can therefore block for tens of seconds.

This patch is provider-agnostic and only rewrites the already-injected global
catalogue alias recovery wrapper. Desktop/web-like runtimes retain the broad recovery;
