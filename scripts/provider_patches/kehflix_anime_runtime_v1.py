...
API, resolves media declared inside correlated player pages, and only then falls
back to generic player handling. No provider response token, title, fixture id,
player/CDN hostname or media URL is baked into the patch.
"""
...
function absRef(raw,ref){var v=txt(raw);if(!v)return"";try{return new URL(v,ref).toString()}catch(_e){return""}}
...
async function embeddedPlayerMedia(url,referer,source){var r=await response(url,{headers:{Referer:referer}});if(!r||r.ok===false)return[];var html="";try{html=await r.text()}catch(_e){return[]}var page=txt(r.url)||url,out=[],seen=[],patterns=[/(?:src|file|url)\s*:\s*["']([^"']+\.(?:mp4|m3u8)(?:\?[^"']*)?)["']/gi,/["']([^"']+\.(?:mp4|m3u8)(?:\?[^"']*)?)["']/gi];for(var p=0;p<patterns.length;p++){var re=patterns[p],m;while((m=re.exec(html))!==null&&seen.length<8){var raw=htmlDecode(m[1]).replace(/\\\//g,"/"),u=absRef(raw,page);if(!u||seen.indexOf(u)>=0)continue;seen.push(u)}}for(var i=0;i<seen.length&&out.length<c.targetStreams;i++){var media=seen[i],rows=await crawl(media,page,source,1);if(rows.length){out=out.concat(rows);continue}try{if(typeof _streams==="function")out=out.concat(decorate(_streams([media],page),source))}catch(_e){}}return out.slice(0,c.targetStreams)}
...
for(var p=0;p<players.length&&out.length<c.targetStreams;p++){var pr=players[p],direct=await embeddedPlayerMedia(pr.url,titleUrl,pr.source);if(!direct.length)direct=await crawl(pr.url,titleUrl,pr.source,2);if(direct.length)out=out.concat(direct);else out=out.concat(correlatedPlayer(pr.url,titleUrl,pr.source))}
...
def apply(text: str, options: dict[str, Any] | None = None, **_kwargs: Any) -> str:
    ...
    return replace_managed_fix(
        ...
        data={
            ...
            "sourcePriority": ["same-origin-gateway", "embedded-player-media", "episode-correlated-player", "verified-direct-media"],
            "embeddedPlayerMediaExtraction": "generic-mp4-m3u8-declaration",
