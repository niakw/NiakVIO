...
async function driveSeed(url){var cur=url;if(/\/r\?key=/i.test(cur)){var rr=await request(cur,{referer:base()+"/"});if(!rr)return[];var t=redirectTarget(rr.text,rr.url);if(t)cur=t}var page=await request(cur,{referer:base()+"/"});if(!page)return[];var direct=directLinks(page.text,page.url);if(direct.length)return direct.slice(0,4);var buttons=downloadButtons(page.text,page.url);for(var i=0;i<buttons.length&&i<5;i++){var u=buttons[i].url;if(directCandidate(u))return[u];var rr=await request(u,{referer:page.url});if(!rr)continue;var q="";try{q=new URL(rr.url).searchParams.get("url")||""}catch(_e){}if(q){try{q=decodeURIComponent(q)}catch(_e){}if(directCandidate(q))return[q]}if(directCandidate(rr.url))return[rr.url];var more=directLinks(rr.text,rr.url);if(more.length)return more.slice(0,4)}try{if(typeof _crawlDirectMedia==="function"){var crawled=await _crawlDirectMedia([cur],cur,2);if(Array.isArray(crawled)&&crawled.length)return crawled.map(function(x){return x&&x.url}).filter(Boolean).slice(0,4)}}catch(_e){}return[]}
...
def apply(text: str, options: dict[str, Any] | None = None, **_kwargs: Any) -> str:
    ...
    return replace_managed_fix(
        ...
        js.lstrip(),
        data={
            "runtimeFamily": "uhdmovies-search-gateway-driveseed-v1",
