...
function headers(json){return {"User-Agent":c.userAgent,"Accept":json?"application/json,text/html,*/*":"text/html,application/xhtml+xml,*/*","Accept-Language":"fr-FR,fr;q=0.9,en;q=0.7","Referer":c.base+"/","X-Requested-With":"XMLHttpRequest"}}
...
async function find(als,isSeries){for(var i=0;i<als.length&&i<4;i++){var rows=await getJson(c.base+"/search?q="+encodeURIComponent(als[i]));if(!Array.isArray(rows))continue;var pool=rows.map(function(r){return {url:abs(r&&r.url,c.base),title:s(r&&r.title),series:s(r&&r.type).toLowerCase()==="tvshow"}}).filter(function(r){return r.url&&r.title});var typed=pool.filter(function(r){return r.series===isSeries});if(typed.length)pool=typed;pool.sort(function(a,b){return score(b.title,als[i])-score(a.title,als[i])});if(pool.length&&score(pool[0].title,als[i])>=30)return pool[0]}return null}
...
async function resolve(args){var q=request(args);if(!q)return null;var md=await metadata(q),als=aliases(md);if(!als.length)return[];var hit=await find(als,q.type==="tv");if(!hit)return[];
  ...
  var root=await getText(hit.url);if(!root)return[];var seasons=seasonLinks(root,hit.url),season=seasons.find(function(x){return x.season===q.season});if(!season)return[];var sh=await getText(season.url);if(!sh)return[];var eps=episodeLinks(sh,season.url),ep=eps.find(function(x){return x.season===q.season&&x.episode===q.episode});if(!ep)return[];var eh=await getText(ep.url);if(!eh)return[];return await resolveTabs(serverTabs(eh,"episode",ep.url),ep.url,q)}
