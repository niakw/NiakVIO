...
async function meta(q){var out={title:q.title,year:q.year};if(out.title&&out.year)return out;if(!q.id||!TMDB_KEY)return out;var kind=q.type==="tv"||q.type==="series"||q.type==="anime"?"tv":"movie";try{var r=await g.fetch("https://api.themoviedb.org/3/"+kind+"/"+encodeURIComponent(q.id)+"?api_key="+TMDB_KEY+"&language=fr-FR",{headers:{Accept:"application/json"}});if(r&&r.ok){var d=await r.json();out.title=s(d.title||d.name)||out.title;var date=s(d.release_date||d.first_air_date);out.year=Number(date.slice(0,4))||out.year}}catch(_e){}return out}
