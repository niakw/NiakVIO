...
'''
    ...
    src = replace_once(src, "preResolved=await resolve(originalArgs);", "preResolved=await resolve(originalArgs,incomingContext);", "pre-output resolve")
    src = replace_once(src, "verified=preResolved||await resolve(originalArgs);", "verified=preResolved||await resolve(originalArgs,incomingContext);", "post-output resolve")
    for token in (MARKER, "matchingIncomingContext(originalArgs)", "provisional(originalArgs,incomingContext)", "resolve(originalArgs,incomingContext)", "tmdbMetadata:incomingContext&&incomingContext.tmdbMetadata||null"):
        if token not in src:
            raise AssertionError(f"missing Core metadata handoff token: {token}")
    CORE.write_text(src, encoding="utf-8")

    test = TEST.read_text(encoding="utf-8")
    marker = "# REQUEST_SCOPED_METADATA_HANDOFF_V1_TEST"
