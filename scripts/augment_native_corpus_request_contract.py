...
def augment(path: Path, client: str, slug: str, manifest: Path) -> None:
    ...
    replacement = f'''        for (provider in providers) {{\n            val requestRoutes = requestRoutesFor(provider.id, mediaType)\n            if (requestRoutes.isEmpty()) {{\n                emit("FIELD_NATIVE_PROVIDER_SKIPPED client={client} fixture=$fixtureSlug provider64=${{b64(provider.id)}} enabled=${{provider.enabled}} requested_type=$mediaType declared_types64=${{b64((declaredTypesByProvider[provider.id.lowercase()] ?: emptySet<String>()).sorted().joinToString(","))}} reason=unsupported_type")\n                continue\n            }}\n            for (requestRoute in requestRoutes) {{\n                val requestMediaType = requestRoute.mediaType\n                val routeMode = if (requestRoute.declared) "declared" else "capability_probe"\n                val started = System.currentTimeMillis()\n                emit("FIELD_NATIVE_PROVIDER_BEGIN client={client} fixture=$fixtureSlug provider64=${{b64(provider.id)}} enabled=${{provider.enabled}} request_type=$requestMediaType route_mode=$routeMode declared_types64=${{b64((declaredTypesByProvider[provider.id.lowercase()] ?: emptySet<String>()).sorted().joinToString(","))}}")\n                try {{'''
