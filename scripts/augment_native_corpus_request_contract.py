...
def augment(path: Path, client: str, slug: str, manifest: Path) -> None:
    ...
    if client in {"tv", "mobile"}:
        ...
        if client == "mobile":
            ...
            # its applicationId to com.nuviodebug.com. Android package visibility
            # may hide that separate app from getLaunchIntentForPackage(), so a
            # known exported launcher component is the authoritative fallback.
            launch_helper = '''    private fun launchClientUi() {\n        val instrumentation = InstrumentationRegistry.getInstrumentation()\n        val context = instrumentation.targetContext\n        var packageName: String = "com.nuviodebug.com"\n        var intent: android.content.Intent? = context.packageManager.getLaunchIntentForPackage(packageName)\n        if (intent == null) {\n            intent = android.content.Intent(android.content.Intent.ACTION_MAIN).apply {\n                addCategory(android.content.Intent.CATEGORY_LAUNCHER)\n                setClassName(packageName, "com.nuvio.app.MainActivity")\n            }\n        }\n        val launchIntent: android.content.Intent = intent\n        try {\n            launchIntent.addFlags(android.content.Intent.FLAG_ACTIVITY_NEW_TASK or android.content.Intent.FLAG_ACTIVITY_CLEAR_TOP)\n            context.startActivity(launchIntent)\n        } catch (error: Throwable) {\n            val instrumentationPackage: String = context.packageName\n            packageName = if (instrumentationPackage.endsWith(".test")) instrumentationPackage.removeSuffix(".test") else instrumentationPackage\n            val fallbackIntent: android.content.Intent? = context.packageManager.getLaunchIntentForPackage(packageName)\n            if (fallbackIntent == null) {\n                emit("FIELD_NATIVE_UI_LAUNCH_ERROR client=mobile package64=${b64(packageName)} reason=start_failed error64=${b64(error.message ?: error.javaClass.simpleName)}")\n                return\n            }\n            try {\n                fallbackIntent.addFlags(android.content.Intent.FLAG_ACTIVITY_NEW_TASK or android.content.Intent.FLAG_ACTIVITY_CLEAR_TOP)\n                context.startActivity(fallbackIntent)\n            } catch (fallbackError: Throwable) {\n                emit("FIELD_NATIVE_UI_LAUNCH_ERROR client=mobile package64=${b64(packageName)} reason=fallback_start_failed error64=${b64(fallbackError.message ?: fallbackError.javaClass.simpleName)}")\n                return\n            }\n        }\n        Thread.sleep(1200L)\n        emit("FIELD_NATIVE_UI_LAUNCHED client=mobile package64=${b64(packageName)}")\n    }\n\n'''
        else:
