...
def main():
    with tempfile.TemporaryDirectory() as tmp:
        ...
        subprocess.run(['python',str(ROOT/'scripts/validate_route_overrides.py'),'--config',str(cp),'--results',str(rp),'--report',str(out)],check=True)
        ...
        completed=subprocess.run(['python',str(ROOT/'scripts/validate_route_overrides.py'),'--config',str(cp),'--results',str(rp),'--report',str(out),'--strict'],capture_output=True,text=True)
