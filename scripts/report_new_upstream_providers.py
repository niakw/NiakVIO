...
"""Report live upstream providers absent from NiakVIO without mutating publication."""
...
def build_report(stage: dict[str, Any], catalog: dict[str, Any], sources: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "providers": providers,
        "upstreamStatus": {key: (stage.get("upstreams") or {}).get(key, {}) for key in sorted(allowed_sources)},
        "policy": {
