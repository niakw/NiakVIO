...
def patch_timeouts() -> None:
    ...
    replacements = (
        (
            'cfg.get("provider_timeout_ms", 60_000)',
            'cfg.get("provider_timeout_ms", 25_000)',
            "provider timeout",
        ),
        (
            'cfg.get("tv_provider_timeout_ms", 60_000)',
    ...
    for test in (ROOT / "tests").glob("*.py"):
        ...
        old = "tmdb-data-contract-launch-gate-v31-pre-network-semantic-gate"
        if old in source:
            test.write_text(
                source.replace(old, "tmdb-data-contract-launch-gate-v32-25s-navigation-budget"),
                encoding="utf-8",
            )
