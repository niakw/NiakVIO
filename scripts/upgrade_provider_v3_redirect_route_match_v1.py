...
def validate(text: str) -> None:
    ...
    for needle in forbidden:
        if needle in text:
            raise AssertionError(f"redirect route match retained final-url-only rule: {needle}")
...
def main() -> int:
    ...
    print(
        ...
        f"changed={str(changed).lower()} request_url=route_identity final_url=redirect_evidence"
    )
    return 0
