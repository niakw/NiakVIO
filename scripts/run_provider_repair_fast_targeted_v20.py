#!/usr/bin/env python3
"""Run canonical targeted repair with V20.2 immediately before live recovery."""
...
def _run_with_v20_boundary(*args: str, timeout: int | None = None) -> None:
    global _v20_applied
    # Preserve the complete legacy migration sequence unchanged. V20.2 composes
    # with the final V11/V18 owners only at the boundary immediately before the
    # first upstream census consumes provider responses.
    if not _v20_applied and any(str(value).endswith("recover_provider_routes_from_upstreams.py") for value in args):
        ...
        _v20_applied = True
        print(
            "FIELD_PROVIDER_V20_BOUNDARY ready=true revision=v20.2 "
            "legacy_migrations_first=1 v18_4_resolver_composed=1 all_v20_owners_before_census=1",
            flush=True,
        )
