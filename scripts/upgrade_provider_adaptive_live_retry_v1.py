...
This historical migrator is intentionally idempotent across both the legacy
``validatedTypes`` gate and the newer stricter ``playableChainValidatedTypes``
gate. Migration tests must not break merely because catalogue-resampling semantics
were added later.
"""
...
def _candidate_break(key: str) -> str:
    return f'''            if is_qualified(evaluation) or semantic_type in {{\n                str(value or "").strip().casefold() for value in evaluation.get("{key}") or []\n            }}:\n                break\n'''


def _candidate_replacement(key: str) -> str:
...
def patch() -> bool:
    ...
    candidate_key = None
    for key in ("playableChainValidatedTypes", "validatedTypes"):
        if text.count(_candidate_break(key)) == 1:
            candidate_key = key
            break
    if candidate_key is None:
        raise AssertionError("adaptive retry candidate anchor missing")
    text = text.replace(
        _candidate_break(candidate_key),
        _candidate_replacement(candidate_key),
        1,
    )
