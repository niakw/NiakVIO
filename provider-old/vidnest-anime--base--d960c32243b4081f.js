...
/* NUVIO_PROVIDER_SECURITY_HARDENING_V1:428964b9cb8a */
...
function decryptAesGcm(encryptedB64, passphraseB64) {
    __nuvioProviderSilentLog('[VidnestAnime] Starting AES-GCM decryption via server...');
    ...
    .then(data => {
        ...
        __nuvioProviderSilentLog('[VidnestAnime] Server decryption successful');
        return data.decrypted;
    })
    .catch(error => {
        __nuvioProviderSilentLog(`[VidnestAnime] Server decryption failed: ${error.message}`);
        throw error;
    });
}
...
function makeRequest(url, options = {}) {
    ...
    return fetch(url, {
        ...
        __nuvioProviderSilentLog(`[VidnestAnime] Request failed for ${url}: ${error.message}`);
        throw error;
    });
}
...
function mapTMDBToAniList(tmdbId, title, year) {
    __nuvioProviderSilentLog(`[VidnestAnime] Mapping TMDB ${tmdbId} to AniList...`);
    ...
    .then(function(data) {
        if (data.data && data.data.Media) {
            ...
            __nuvioProviderSilentLog(`[VidnestAnime] Mapped to AniList ID: ${anilistId} (${data.data.Media.title.english || data.data.Media.title.romaji})`);
            return anilistId;
        }
...
function getTMDBSeasonEpisodeCounts(tmdbId, targetSeason) {
    __nuvioProviderSilentLog(`[VidnestAnime] Fetching season info for TMDB ${tmdbId}, seasons 1-${targetSeason}`);
    ...
    for (let s = 1; s < targetSeason; s++) {
        ...
        seasonPromises.push(
            makeRequest(url)
                ...
                .catch(function(error) {
                    __nuvioProviderSilentLog(`[VidnestAnime] Failed to fetch season ${s}: ${error.message}`);
    ...
    return Promise.all(seasonPromises)
        .then(function(episodeCounts) {
            ...
            __nuvioProviderSilentLog(`[VidnestAnime] Previous seasons episode counts: ${episodeCounts.join(', ')} = ${totalPreviousEpisodes} total`);
...
function getAnimeMetadata(anilistId, episodeNum) {
    __nuvioProviderSilentLog(`[VidnestAnime] Fetching metadata for AniList ID: ${anilistId}, Episode: ${episodeNum}`);
    ...
    return makeRequest(`https://api.ani.zip/mappings?anilist_id=${anilistId}`)
        ...
        .catch(function(error) {
            __nuvioProviderSilentLog(`[VidnestAnime] Failed to fetch anime metadata: ${error.message}`);
...
function fetchFromAnimeServer(serverName, serverConfig, anilistId, episodeNum, subDub) {
    __nuvioProviderSilentLog(`[VidnestAnime] Fetching from ${serverName}...`);
    ...
    __nuvioProviderSilentLog(`[VidnestAnime] ${serverName} API URL: ${url}`);
    ...
    return makeRequest(url)
        ...
        .then(function(responseText) {
            __nuvioProviderSilentLog(`[VidnestAnime] ${serverName} response length: ${responseText.length} characters`);
            ...
            try {
                ...
                if (serverConfig.needsDecryption && data.encrypted && data.data) {
                    __nuvioProviderSilentLog(`[VidnestAnime] ${serverName}: Detected encrypted response, decrypting...`);
                    
                    return decryptAesGcm(data.data, PASSPHRASE)
                        .then(function(decryptedText) {
                            __nuvioProviderSilentLog(`[VidnestAnime] ${serverName}: Decryption successful`);
                            
                            try {
                                const decryptedData = JSON.parse(decryptedText);
                                return processAnimeResponse(decryptedData, serverName, serverConfig, subDub);
                            } catch (parseError) {
                                __nuvioProviderSilentLog(`[VidnestAnime] ${serverName}: JSON parse error after decryption: ${parseError.message}`);
                                return [];
                            }
                        });
                } else {
                ...
                __nuvioProviderSilentLog(`[VidnestAnime] ${serverName}: Invalid JSON response: ${parseError.message}`);
                return [];
            }
        })
        .catch(function(error) {
            __nuvioProviderSilentLog(`[VidnestAnime] ${serverName} error: ${error.message}`);
            return [];
        });
}
...
function processAnimeResponse(data, serverName, serverConfig, subDub) {
    ...
    try {
        __nuvioProviderSilentLog(`[VidnestAnime] Processing response from ${serverName}`);
        ...
        if (!Array.isArray(sources) || sources.length === 0) {
            __nuvioProviderSilentLog(`[VidnestAnime] ${serverName}: No sources/streams array found`);
            return streams;
        }
        ...
        sources.forEach((source, index) => {
            ...
            if (!videoUrl) {
                __nuvioProviderSilentLog(`[VidnestAnime] ${serverName}: Source ${index} has no video URL`);
                return;
            }
            ...
            __nuvioProviderSilentLog(`[VidnestAnime] ${serverName}: Added ${language} stream with ${processedSubtitles.length} subtitles`);
            __nuvioProviderSilentLog(`[VidnestAnime] ${serverName}: Stream URL: ${videoUrl}`);
            
            // Log complete stream object for testing
            __nuvioProviderSilentLog(`[VidnestAnime] ${serverName}: Complete Stream Object:`, JSON.stringify({
                name: `VidnestAnime ${serverName.charAt(0).toUpperCase() + serverName.slice(1)} [${language}] - Adaptive`,
                url: videoUrl,
                quality: 'Adaptive',
                subtitles: processedSubtitles,
        ...
        __nuvioProviderSilentLog(`[VidnestAnime] Error processing ${serverName} response: ${error.message}`);
    }
...
function getStreams(tmdbId, mediaType, seasonNum, episodeNum) {
    __nuvioProviderSilentLog(`[VidnestAnime] Starting extraction for TMDB ID: ${tmdbId}, Type: ${mediaType}, S${seasonNum}E${episodeNum}`);
    
    return new Promise(function(resolve, reject) {
        // Step 1: Get TMDB details
        getTMDBDetails(tmdbId, mediaType)
            .then(function(tmdbInfo) {
                __nuvioProviderSilentLog(`[VidnestAnime] TMDB: "${tmdbInfo.title}" (${tmdbInfo.year})`);
                ...
                if (mediaType === 'tv' && season > 1) {
                    ...
                    return getTMDBSeasonEpisodeCounts(tmdbId, season)
                        .then(function(previousEpisodesCount) {
                            ...
                            __nuvioProviderSilentLog(`[VidnestAnime] Converted S${season}E${episode} → Absolute Episode ${absoluteEpisode}`);
            ...
            .then(function(metadata) {
                __nuvioProviderSilentLog(`[VidnestAnime] Anime: "${metadata.title}" - Episode ${metadata.absoluteEpisode}`);
                ...
                return Promise.all(serverPromises)
                    .then(function(results) {
                        ...
                        __nuvioProviderSilentLog(`[VidnestAnime] Total streams found: ${sortedStreams.length}`);
                        resolve(sortedStreams);
                    });
            })
            .catch(function(error) {
                __nuvioProviderSilentLog(`[VidnestAnime] Error: ${error.message}`);
