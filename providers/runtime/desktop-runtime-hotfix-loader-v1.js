/* NUVIO_DESKTOP_RUNTIME_HOTFIX_LOADER_V1 */
;(function(g){
  if(!g||g.__installNuvioDesktopHotfixV1)return;
  g.__installNuvioDesktopHotfixV1=function(config){
    ...
    wrapped=async function(tmdbId,mediaType,season,episode){
      ...
      if(series&&config.normalizeMissingEpisodes){
        season=positive(season,config.fallbackSeason||1);
        episode=positive(episode,config.fallbackEpisode||1);
      }
      if(!original){
        if(!loading){
