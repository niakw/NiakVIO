"use strict";
/* NUVIO_PROVIDER_SECURITY_HARDENING_V1:77e110a2a7b7 */var __nuvioProviderSilentLog=function(){};const NIAKVIO_PROVIDER_MODEL=Object.freeze({authoring:"niakvio-owned-v2",displayName:"Kehflix",fixedApi:null,knownSite:"https://kehflix.com/",observedUrls:["https://kehflix.com/","https://www.youtube.com/embed/kTGDxqUowq4","https://www.youtube.com/iframe_api"],officialApi:null,officialHub:"https://kehflix.wiki/",officialSite:"https://kehflix.com/",origins:["https://kehflix.com","https://kehflix.wiki"],providerId:"kehflix",reconstructionState:"learning-clean-seed",routes:["/title/movie/{id}-{slug}","/title/tv/{id}-{slug}","/api/report","/api/track-view","/player","/api/track","/api/fp","/api/ad-link","/api/streams/working","/api/streams/dead","/api/streams/warm","/api/stream-gw","/api/streams/{media}?id&k"],strategy:"html_scraper",supportedTypes:["movie","tv","anime"],upstreamCodeEmbedded:false,upstreamCodeExecuted:false});function _uniq(values){return[...new Set((values||[]).filter(Boolean))]}function _origin(value){try{return new URL(value).origin}catch(_){return""}}function _absolute(value,base){try{return new URL(value,base).toString()}catch(_){return""}}function _text(value){return String(value==null?"":value)}function _embeddedText(value){return _text(value).replace(/\\u002[fF]|\\u003[aA]|\\u0026|\\u003[dD]|\\\/|\\"|&quot;|&#34;|&amp;/gi,token=>{const normalized=token.toLowerCase();if(normalized==="\\u002f"||normalized==="\\/")return"/";if(normalized==="\\u003a")return":";if(normalized==="\\u0026"||normalized==="&amp;")return"&";if(normalized==="\\u003d")return"=";if(normalized==='\\"'||normalized==="&quot;"||normalized==="&#34;")return'"';return token})}function _slug(value){return _text(value).normalize("NFD").replace(/[\u0300-\u036f]/g,"").toLowerCase().replace(/[^a-z0-9]+/g,"-").replace(/^-+|-+$/g,"")}function _directMedia(url){return/\.(?:m3u8|mpd|mp4|mkv|webm)(?:[?#]|$)|\/(?:hls|dash|stream)(?:\/|[?#]|$)/i.test(_text(url))}function _extractUrls(text,base){const out=[];const normalized=_embeddedText(text);const patterns=[/(?:src|href|file|url|pathname|permalink|embedUrl|embed_url|contentUrl)\s*["']?\s*[:=]\s*["']([^"'<>\s]+)["']/gi,/["'](\/(?:api|watch|embed|player|play|video|videos|stream|streams|source|sources|server|servers|resolve|proxy|manifest|action)(?:[^"'<>\\\s]{0,500}))["']/gi,/https?:\/\/[^"'<>\s]+/gi];for(const pattern of patterns){let match;while(match=pattern.exec(normalized)){const raw=match[1]||match[0]||"";const absolute=_absolute(raw,base);if(absolute&&/^https?:/i.test(absolute))out.push(absolute);if(out.length>=240)break}}return _uniq(out)}function _playerLike(url){try{const parsed=new URL(url);return/\/(?:watch|embed|player|play|video|videos|stream|streams|source|sources|server|servers|resolve|proxy)(?:[/?#.-]|$)/i.test(parsed.pathname+parsed.search)}catch(_){return false}}async function _crawlDirectMedia(seedUrls,referer,maxDepth){const queue=_uniq(seedUrls).filter(_playerLike).slice(0,8).map(url=>({url,depth:0,referer}));const seen=new Set;const streams=[];let requests=0;while(queue.length&&requests<12&&streams.length<12){const row=queue.shift();if(!row||seen.has(row.url))continue;seen.add(row.url);requests+=1;try{const response=await _fetch(row.url,{headers:row.referer?{Referer:row.referer}:{}});const responseUrl=response.url||row.url;const contentType=_text(response.headers.get("content-type")).toLowerCase();if(_directMedia(responseUrl)||/(?:mpegurl|dash\+xml|video\/)/i.test(contentType)){streams.push(..._streams([responseUrl],row.referer||referer||""));continue}let urls=[];if(contentType.includes("json")){urls=_jsonUrls(await response.json())}else{urls=_extractUrls(await response.text(),responseUrl)}const direct=urls.filter(_directMedia);if(direct.length){streams.push(..._streams(direct,responseUrl));continue}if(row.depth<Math.max(0,Number(maxDepth)||0)){for(const next of urls.filter(_playerLike).slice(0,6)){if(!seen.has(next))queue.push({url:next,depth:row.depth+1,referer:responseUrl})}}}catch(_){}}return streams.slice(0,40)}function _candidateScore(url,meta){let parsed;try{parsed=new URL(url)}catch(_){return-1}const path=decodeURIComponent(parsed.pathname||"").toLowerCase();if(!path||path==="/"||/\/(?:_next|static|assets?|images?|icons?|fonts?)(?:\/|$)/i.test(path))return-1;const slug=_slug(meta&&meta.title);const tokens=slug.split("-").filter(token=>token.length>=3);let score=0;if(slug&&path.includes(slug))score+=120;for(const token of tokens)if(path.includes(token))score+=18;if(meta&&meta.year&&path.includes(String(meta.year)))score+=20;if(meta&&meta.tmdbId&&path.includes(String(meta.tmdbId)))score+=45;if(/\/(?:movie|movies|film|films|series|tv|show|watch|title|media)\//i.test(path))score+=12;return score}function _detailGuesses(meta,mediaType){const out=[];const slug=_slug(meta&&meta.title);const id=_text(meta&&meta.tmdbId);const titleKind=mediaType==="movie"?"movie":"tv";const routes=mediaType==="movie"?["movie","movies","film","films","watch","title"]:["series","tv","show","watch","title"];for(const base of _searchBases()){if(id&&slug){out.push(_absolute("/title/"+titleKind+"/"+encodeURIComponent(id)+"-"+slug,base))}if(id){out.push(_absolute("/title/"+titleKind+"/"+encodeURIComponent(id),base))}if(slug){out.push(_absolute("/"+slug,base));for(const route of routes)out.push(_absolute("/"+route+"/"+slug,base))}if(id){out.push(_absolute("/"+id,base));for(const route of routes)out.push(_absolute("/"+route+"/"+encodeURIComponent(id),base))}}return _uniq(out)}async function _fetch(url,options){const response=await fetch(url,{redirect:"follow",headers:Object.assign({Accept:"application/json,text/html,application/xhtml+xml,text/plain,*/*","User-Agent":"Mozilla/5.0 NiakVIO/2"},options&&options.headers||{})});if(!response.ok)throw new Error("provider_http_"+response.status);return response}async function _tmdb(tmdbId,mediaType){const key=typeof globalThis!=="undefined"?globalThis.TMDB_API_KEY:null;if(!key||!tmdbId)return null;const type=String(mediaType||"movie").toLowerCase()==="movie"?"movie":"tv";try{const response=await _fetch("https://api.themoviedb.org/3/"+type+"/"+encodeURIComponent(tmdbId)+"?api_key="+encodeURIComponent(key)+"&language=en-US");const row=await response.json();return{title:row.title||row.name||row.original_title||row.original_name||"",year:String(row.release_date||row.first_air_date||"").slice(0,4),tmdbId:String(tmdbId||"")}}catch(_){return null}}function _searchBases(){return _uniq([NIAKVIO_PROVIDER_MODEL.officialSite,NIAKVIO_PROVIDER_MODEL.knownSite,NIAKVIO_PROVIDER_MODEL.officialHub,...NIAKVIO_PROVIDER_MODEL.origins||[]]).filter(value=>/^https?:/i.test(value))}function _searchUrls(title){const query=encodeURIComponent(title||"");const out=[];for(const base of _searchBases()){out.push(_absolute("/?s="+query,base));out.push(_absolute("/search?q="+query,base));out.push(_absolute("/search/"+query,base))}return _uniq(out)}function _apiUrls(tmdbId,mediaType,season,episode){const bases=_uniq([NIAKVIO_PROVIDER_MODEL.fixedApi,NIAKVIO_PROVIDER_MODEL.officialApi,...(NIAKVIO_PROVIDER_MODEL.observedUrls||[]).filter(value=>/api|stream|source|embed|player/i.test(value))]);const out=[];for(const base of bases){if(!/^https?:/i.test(base))continue;let url=base.replace(/\{(?:tmdb_?id|id)\}/gi,encodeURIComponent(tmdbId||"")).replace(/\{(?:media_?type|type)\}/gi,encodeURIComponent(mediaType||"movie")).replace(/\{season\}/gi,encodeURIComponent(season==null?"":season)).replace(/\{episode\}/gi,encodeURIComponent(episode==null?"":episode));out.push(url);try{const parsed=new URL(url);if(!parsed.searchParams.size){parsed.searchParams.set("tmdbId",tmdbId||"");parsed.searchParams.set("type",mediaType||"movie");if(season!=null)parsed.searchParams.set("season",String(season));if(episode!=null)parsed.searchParams.set("episode",String(episode));out.push(parsed.toString())}}catch(_){}}return _uniq(out)}function _directPlayerUrls(tmdbId,mediaType){if(!tmdbId)return[];const hasPlayerRoute=(NIAKVIO_PROVIDER_MODEL.routes||[]).some(route=>/^\/player(?:[?#]|$)/i.test(_text(route)));if(!hasPlayerRoute)return[];const transportType=mediaType==="movie"?"movie":"tv";const out=[];for(const base of _searchBases()){try{const url=new URL("/player",base);url.searchParams.set("m",transportType);url.searchParams.set("id",_text(tmdbId));out.push(url.toString())}catch(_){}}return _uniq(out)}function _runtimeApiUrls(playerUrl,mediaType,tmdbId,season,episode){let player;try{player=new URL(playerUrl)}catch(_){return[]}const out=[];const desiredMedia=mediaType==="movie"?"movie":"tv";const observedMedia=_text(player.searchParams.get("m")||player.searchParams.get("media")||player.searchParams.get("type")).toLowerCase();for(const pattern of NIAKVIO_PROVIDER_MODEL.routes||[]){if(!/^\/api\/(?:streams?(?:\/|$)|source|sources|resolve|proxy)/i.test(_text(pattern)))continue;if(/\/(?:working|dead|warm)(?:[?#]|$)/i.test(_text(pattern)))continue;const parts=_text(pattern).split("?",2);let path=parts[0].replace(/\{media\}/gi,encodeURIComponent(desiredMedia));if(observedMedia&&/\/(?:movie|tv|anime)$/i.test(path)){path=path.replace(/\/(?:movie|tv|anime)$/i,"/"+encodeURIComponent(desiredMedia))}const keys=(parts[1]||"").split("&").map(part=>part.split("=",1)[0]).filter(Boolean);if(!keys.length)continue;let target;try{target=new URL(path,player.origin)}catch(_){continue}let missing=false;for(const key of keys){const lower=key.toLowerCase();let value=player.searchParams.get(key);if(value==null&&lower==="id")value=_text(tmdbId);if(value==null&&/^(?:m|media|type)$/.test(lower))value=desiredMedia;if(value==null&&/^(?:season|s)$/.test(lower)&&season!=null)value=_text(season);if(value==null&&/^(?:episode|e)$/.test(lower)&&episode!=null)value=_text(episode);if(value==null||value===""){missing=true;break}target.searchParams.set(key,value)}if(!missing)out.push({url:target.toString(),referer:player.toString()})}const seen=new Set;return out.filter(row=>row.url&&!seen.has(row.url)&&seen.add(row.url))}function _jsonUrls(value,out){out=out||[];if(typeof value==="string"){if(/^https?:/i.test(value))out.push(value);return out}if(Array.isArray(value)){for(const child of value)_jsonUrls(child,out);return out}if(value&&typeof value==="object"){for(const child of Object.values(value))_jsonUrls(child,out)}return out}function _sourceUrls(value,base,out){out=out||[];if(Array.isArray(value)){for(const child of value)_sourceUrls(child,base,out);return out}if(!value||typeof value!=="object")return out;for(const[key,child]of Object.entries(value)){if(typeof child==="string"&&/^(?:src|url|file|stream|source)$/i.test(key)){const absolute=_absolute(child,base);if(absolute&&/^https?:/i.test(absolute)&&!/\.(?:jpe?g|png|gif|webp|svg|avif)(?:[?#]|$)/i.test(absolute)){out.push(absolute)}}if(child&&typeof child==="object")_sourceUrls(child,base,out)}return out}function _streams(urls,referer){return _uniq(urls).slice(0,40).map((url,index)=>({name:NIAKVIO_PROVIDER_MODEL.displayName,title:NIAKVIO_PROVIDER_MODEL.displayName+(index?" #"+(index+1):""),url,headers:referer?{Referer:referer}:undefined}))}async function _resolveApi(tmdbId,mediaType,season,episode){const streams=[];for(const url of _apiUrls(tmdbId,mediaType,season,episode).slice(0,12)){try{const response=await _fetch(url);const type=_text(response.headers.get("content-type")).toLowerCase();if(type.includes("json")){const value=await response.json();streams.push(..._jsonUrls(value).filter(_directMedia))}else{const text=await response.text();streams.push(..._extractUrls(text,response.url||url).filter(_directMedia))}}catch(_){}if(streams.length)break}return _streams(streams,_searchBases()[0]||"")}async function _resolveRuntimeApi(playerUrls,mediaType,tmdbId,season,episode){const streams=[];for(const playerUrl of _uniq(playerUrls).slice(0,6)){for(const row of _runtimeApiUrls(playerUrl,mediaType,tmdbId,season,episode).slice(0,8)){try{const response=await _fetch(row.url,{headers:row.referer?{Referer:row.referer}:{}});const type=_text(response.headers.get("content-type")).toLowerCase();if(type.includes("json")){const value=await response.json();const sources=_sourceUrls(value,response.url||row.url);if(sources.length)streams.push(..._streams(sources,row.referer))}else{const text=await response.text();const urls=_extractUrls(text,response.url||row.url);const direct=urls.filter(_directMedia);if(direct.length)streams.push(..._streams(direct,row.referer))}}catch(_){}if(streams.length)break}if(streams.length)break}return streams.slice(0,40)}async function _resolveHtml(meta,mediaType,season,episode){if(!meta||!meta.title&&!meta.tmdbId)return[];const candidates=[];if(meta.title){for(const searchUrl of _searchUrls(meta.title).slice(0,9)){try{const response=await _fetch(searchUrl);const html=await response.text();const urls=_extractUrls(html,response.url||searchUrl).filter(value=>{const host=_origin(value);return host&&_searchBases().some(base=>_origin(base)===host)}).map(value=>({url:value,score:_candidateScore(value,meta)})).filter(row=>row.score>=18).sort((a,b)=>b.score-a.score).map(row=>row.url);candidates.push(...urls)}catch(_){}if(candidates.length)break}}candidates.push(..._detailGuesses(meta,mediaType));const streams=[];for(const detailUrl of _uniq(candidates).slice(0,24)){try{const response=await _fetch(detailUrl);const html=await response.text();let urls=_extractUrls(html,response.url||detailUrl);if(mediaType!=="movie"&&season!=null&&episode!=null){const token=new RegExp("(?:s(?:eason)?\\s*0*"+Number(season)+"[^\\n]{0,80}e(?:pisode)?\\s*0*"+Number(episode)+"|0*"+Number(season)+"x0*"+Number(episode)+")","i");const episodeLinks=urls.filter(value=>token.test(value));if(episodeLinks.length){for(const episodeUrl of episodeLinks.slice(0,4)){try{const episodeResponse=await _fetch(episodeUrl);const episodeHtml=await episodeResponse.text();urls=urls.concat(_extractUrls(episodeHtml,episodeResponse.url||episodeUrl))}catch(_){}}}}const direct=urls.filter(_directMedia);if(direct.length)streams.push(..._streams(direct,response.url||detailUrl));if(!direct.length&&/iframe|mixed_embed|html_scraper/i.test(NIAKVIO_PROVIDER_MODEL.strategy)){const discoveredNested=_uniq(urls.filter(_playerLike));if(discoveredNested.length){const crawled=await _crawlDirectMedia(discoveredNested,response.url||detailUrl,2);if(crawled.length){streams.push(...crawled)}else{const runtimeCandidates=_uniq([...discoveredNested,..._directPlayerUrls(meta.tmdbId,mediaType)]);const runtime=await _resolveRuntimeApi(runtimeCandidates,mediaType,meta.tmdbId,season,episode);if(runtime.length)streams.push(...runtime)}}else{const runtimeCandidates=_directPlayerUrls(meta.tmdbId,mediaType);if(runtimeCandidates.length){const runtime=await _resolveRuntimeApi(runtimeCandidates,mediaType,meta.tmdbId,season,episode);if(runtime.length)streams.push(...runtime)}}}}catch(_){}if(streams.length>=12)break}return streams.slice(0,40)}async function getStreams(tmdbId,mediaType,season,episode){const type=String(mediaType||"movie").toLowerCase();if(NIAKVIO_PROVIDER_MODEL.supportedTypes.length&&!NIAKVIO_PROVIDER_MODEL.supportedTypes.includes(type)&&!(type==="tv"&&NIAKVIO_PROVIDER_MODEL.supportedTypes.includes("anime"))){return[]}const strategy=NIAKVIO_PROVIDER_MODEL.strategy;if(/api_stream_resolver|direct_media/i.test(strategy)){const api=await _resolveApi(tmdbId,type,season,episode);if(api.length)return api}const meta=await _tmdb(tmdbId,type)||{title:"",year:"",tmdbId:String(tmdbId||"")};const html=await _resolveHtml(meta,type,season,episode);if(html.length)return html;if(!/api_stream_resolver|direct_media/i.test(strategy)){return _resolveApi(tmdbId,type,season,episode)}return[]}module.exports={getStreams,__niakvioProviderBase:NIAKVIO_PROVIDER_MODEL};
/* NUVIO_GLOBAL_CATALOGUE_ALIAS_RECOVERY_V2:4dcf1f372b11 */
...
function extractPlayers(html,base,q){var text=String(html||"").split("\\/").join("/"),out=[],seen={};function add(v){var u=abs(v,base);if(!u||seen[u]||!/^https?:\/\//i.test(u)||!mediaish(u))return;seen[u]=1;out.push(u)}var scoped=text;if((q.mediaType==="tv"||q.mediaType==="anime")&&q.season&&q.episode){var patterns=[new RegExp("s(?:aison|eason)?[ ._-]*0?"+q.season+"[ ._-]*e(?:p(?:isode)?)?[ ._-]*0?"+q.episode,"i"),new RegExp("(?:episode|ep)[ ._-]*0?"+q.episode,"i")],chunks=text.split(/(?=<[^>]+(?:episode|season|saison|data-ep))/i).filter(function(x){return patterns.some(function(p){return p.test(x)})});if(chunks.length)scoped=chunks.join("\n");else return[]}var patterns2=[/(?:src|href|data-src|data-url|data-embed|data-player|data-video|data-file)=["']([^"']+)["']/gi,/(?:file|source|src|url|playlist|embedUrl|embed_url|contentUrl)\s*[:=]\s*["'](https?:\/\/[^"']+)["']/gi],m;for(var i=0;i<patterns2.length;i++){patterns2[i].lastIndex=0;while((m=patterns2[i].exec(scoped))!==null){add(m[1]);if(out.length>=c.maxPlayers)return out}}return out}
...
async function mirrorPlayers(id,page,q){if(!id||!c.mirrorRoutes.length||c.mirrorTypes.indexOf(q.mediaType)<0)return[];if(!c.mirrorAllowEpisodic&&(q.mediaType==="tv"||q.mediaType==="anime")&&q.season&&q.episode)return[];var out=[];for(var i=0;i<c.mirrorRoutes.length&&out.length<c.maxPlayers;i++){var endpoint=abs(route(c.mirrorRoutes[i],id,q),c.baseUrl+"/"),r=await request(endpoint,true,page);if(r)collectMedia(r.body,c.baseUrl+"/",out,{})}return unique(out).slice(0,c.maxPlayers)}
...
async function recover(q,knownMeta,deadline){if(["movie","tv","anime"].indexOf(q.mediaType)<0||Date.now()>=deadline)return[];var m=knownMeta||await meta(q);if(!m.titles.length||Date.now()>=deadline)return[];var guessed=[],found=[],searches=[];m.titles.forEach(function(t){for(var d=0;d<c.directPaths.length;d++)guessed.push(abs(String(c.directPaths[d]).replace(/\{slug\}/g,slug(t)).replace(/\{query\}/g,encodeURIComponent(t)).replace(/\{tmdbId\}/g,encodeURIComponent(m.tmdbId||"")),c.baseUrl+"/"));for(var sp=0;sp<c.searchPaths.length;sp++)searches.push(abs(String(c.searchPaths[sp]).replace(/\{slug\}/g,slug(t)).replace(/\{query\}/g,encodeURIComponent(t)).replace(/\{tmdbId\}/g,encodeURIComponent(m.tmdbId||"")),c.baseUrl+"/"))});for(var i=0;i<searches.length&&found.length<c.maxCandidates*4&&Date.now()<deadline;i++){var sr=await request(searches[i],false,c.baseUrl+"/");if(sr)found=found.concat(links(sr.body,sr.url,m).map(function(x){return x.url}))}var candidates=unique(found.concat(guessed)).slice(0,c.maxCandidates);for(var j=0;j<candidates.length&&Date.now()<deadline;j++){var page=await request(candidates[j],false,c.baseUrl+"/");if(!page)continue;var identity=aliasScore(page.url+" "+String(page.body||"").slice(0,180000),m);if(identity<90&&!idEvidence(page.body,m))continue;var p=extractPlayers(page.body,page.url,q);if(!p.length){var did=detailId(page.body);if(did)p=await mirrorPlayers(did,page.url,q)}if(p.length)return rows(p,m,page.url)}return[]}
...
})(typeof globalThis!=="undefined"?globalThis:this,{"baseUrl":"https://kehflix.com","providerName":"kehflix","maxAliases":8,"maxCandidates":8,"maxPlayers":8,"timeoutMs":7000,"budgetMs":45000,"languageHint":"","searchPaths":["/?s={query}","/search?q={query}","/search?query={query}"],"directPaths":["/{slug}"],"detailIdAttributes":[],"mirrorRoutes":[],"mirrorTypes":["movie"],"mirrorAllowEpisodic":false,"implementationRevision":"core-tmdb-context-declarative-mirrors-v4"});
...
/* NUVIO_GLOBAL_RUNTIME_MEDIA_SAFETY_V1:2a55e0c03bbb */
;(function(g,c){
  "use strict";
...
/* NUVIO_GLOBAL_STREAM_IDENTITY_V1:ebec1909cecb */
;(function(g,c){"use strict";
function s(v){return String(v==null?"":v).replace(/\\\//g,"/").trim()}
...
var ok=false;try{if(typeof module!=="undefined"&&module.exports){ok=install(module.exports,"getStreams")||install(module.exports,"streams")}}catch(_e){}try{if(g&&typeof g.getStreams==="function"){if(ok&&typeof module!=="undefined"&&module.exports)g.getStreams=module.exports.getStreams;else install(g,"getStreams")}}catch(_e){}
})(typeof globalThis!=="undefined"?globalThis:this,{"providerId":"kehflix","tmdbRuntimeKeyRequired":true,"tmdbTimeoutMs":1200,"implementationRevision":"cross-client-positive-mismatch-runtime-tmdb-fail-open-v6"});
/* NUVIO_GLOBAL_STREAM_PRESENTATION_V1:52b6f1e2c28e */
;(function(g,c){"use strict";
function s(v){return String(v==null?"":v).trim()}
function meaningful(v){var x=s(v);return x&&!/^(?:unknown|inconnue?|n\/?a|null|undefined|none|-+)$/i.test(x)}
...
async function tmdb(q){var key=runtimeTmdbKey();if(!key||!/^\d+$/.test(q.tmdbId||""))return null;var kind=(q.mediaType==="tv"||q.mediaType==="series"||q.mediaType==="anime")?"tv":"movie",append=kind==="movie"?"release_dates":"content_ratings",base="https://api.themoviedb.org/3/"+kind+"/"+encodeURIComponent(q.tmdbId),d=await tmdbJson(base+"?api_key="+encodeURIComponent(key)+"&language=fr-FR&append_to_response="+append);if(!d)return null;var date=s(d.release_date||d.first_air_date),runtime=Number(d.runtime||0);if(!runtime&&Array.isArray(d.episode_run_time)&&d.episode_run_time.length)runtime=Number(d.episode_run_time[0]||0);var meta={title:s(d.title||d.name||q.title),year:Number((date.match(/(?:19|20)\d{2}/)||[])[0]||q.year||0)||0,runtime:runtime>0?Math.round(runtime):0,age:certification(d,kind)};if(kind==="tv"&&q.season>0&&q.episode>0){var ep=await tmdbJson(base+"/season/"+encodeURIComponent(q.season)+"/episode/"+encodeURIComponent(q.episode)+"?api_key="+encodeURIComponent(key)+"&language=fr-FR");if(ep){var er=Number(ep.runtime||0);if(er>0)meta.runtime=Math.round(er)}}return meta}
...
var ok=false;try{if(typeof module!=="undefined"&&module.exports){ok=install(module.exports,"getStreams")||install(module.exports,"streams")}}catch(_e){}try{if(g&&typeof g.getStreams==="function"){if(ok&&typeof module!=="undefined"&&module.exports)g.getStreams=module.exports.getStreams;else install(g,"getStreams")}}catch(_e){}
})(typeof globalThis!=="undefined"?globalThis:this,{"providerId":"kehflix","providerLanguageMode":"vf","languageFallback":"VF","tmdbRuntimeKeyRequired":true,"tmdbTimeoutMs":1200,"implementationRevision":"all-providers-title-quality-ordered-description-runtime-tmdb-fail-open-v16-jvm-json-utf8"});
/* NUVIO_GLOBAL_PROVIDER_BRANDING_V1:bea16d156b00 */
;(function(g,c){"use strict";
function slot(v){if(Array.isArray(v))return{key:null,list:v};if(v&&typeof v==="object"){for(var i=0;i<3;i++){var k=["streams","results","data"][i];if(Array.isArray(v[k]))return{key:k,list:v[k]}}}return null}
function rebuild(v,x,list){if(x.key===null)return list;var o=Object.assign({},v);o[x.key]=list;return o}
...
var ok=false;try{if(typeof module!=="undefined"&&module.exports){ok=install(module.exports,"getStreams")||install(module.exports,"streams")}}catch(_e){}try{if(g&&typeof g.getStreams==="function"){if(ok&&typeof module!=="undefined"&&module.exports)g.getStreams=module.exports.getStreams;else install(g,"getStreams")}}catch(_e){}
})(typeof globalThis!=="undefined"?globalThis:this,{"providerId":"kehflix","providerName":"Kehflix","providerEmoji":"🎬","implementationRevision":"post-presentation-emoji-title-quality-v5"});
/* NUVIO_GLOBAL_MEDIA_TYPE_RESOLUTION_V1:3e5b442399da */
;(function(g,c){"use strict";
function s(v){return String(v==null?"":v).trim()}
...
})(typeof globalThis!=="undefined"?globalThis:this,{"timeoutMs":1800,"providerTimeoutMs":25000,"semanticTypes":["movie","tv","anime"],"revision":"tmdb-api-first-every-provider-v10-core-identity","tmdbKeySalt":"26e924f27770847b22e162407e2c0292","tmdbKeyCipher":[14,208,241,103,174,223,126,201,204,235,5,19,107,95,169,8,81,49,64,111,119,96,235,169,188,253,89,154,66,154,215,254]});
