...
/* FIXDATA:PROVIDER.STREAMZO.RUNTIME.V1: */
...
function stream(u,lang,quality,q){var episodic=q&&q.type!=="movie",tag=episodic?(" | S"+String(q.season||1).padStart(2,"0")+"E"+String(q.episode||1).padStart(2,"0")):"";var row={name:"StreamZo | "+lang,title:"StreamZo | "+lang+tag,url:u,quality:quality||"HD",language:lang,provider:"streamzo",isDirect:true,headers:{Referer:c.base+"/","User-Agent":c.ua}};if(episodic){row.season=Number(q.season)||1;row.episode=Number(q.episode)||1}return row}
async function resolveEmbed(embed,lang,quality,ref,q){var full=abs(embed,c.base);if(!full)return[];var page=await text(full,{referer:ref||c.base+"/"});if(!page)return[];var urls=mediaUrls(page.text);for(var i=0;i<Math.min(3,urls.length);i++){var row=stream(urls[i],lang,quality,q);try{if(typeof _crawlDirectMedia==="function"){var z=await _crawlDirectMedia([urls[i]],full,1);if(Array.isArray(z)&&z.length){for(var j=0;j<z.length;j++){z[j].name=z[j].name||row.name;z[j].title=z[j].title||row.title;z[j].language=z[j].language||lang;z[j].provider="streamzo";z[j].headers=Object.assign(row.headers,z[j].headers||{});if(q&&q.type!=="movie"){z[j].season=Number(q.season)||1;z[j].episode=Number(q.episode)||1;if(!/S\d+E\d+/i.test(String(z[j].title||"")))z[j].title=String(z[j].title||row.title)+" | S"+String(q.season||1).padStart(2,"0")+"E"+String(q.episode||1).padStart(2,"0")}}return z.slice(0,2)}}}catch(_e){}return[row]}return[]}
async function resolve(a,_ctx){var q=req(a);if(!q)return[];var m=await metadata(q),tt=titles(m);if(!tt.length)return[];var hit=await suggest(tt,q,m);if(!hit)return[];var pageUrl=abs(hit.href,c.base),page=await text(pageUrl,{referer:c.base+"/"});if(!page)return[];if(hit.kind==="movie"){var e=movieEmbed(page.text);if(!e)return[];return await resolveEmbed(e,/-vostfr(?:\/|$|\?)/i.test(hit.href)?"VOSTFR":"VF",hit.quality,page.url,q)}if(q.type==="movie")return[];var vars=episodes(page.text,q),out=[];for(var i=0;i<vars.length;i++){var rows=await resolveEmbed(vars[i].embed,vars[i].lang,hit.quality,page.url,q);for(var j=0;j<rows.length;j++)if(rows[j]&&rows[j].url)out.push(rows[j])}return out.slice(0,4)}
...
/* FIXDATA:CORE.MEDIA_ENRICHMENT.V1: */
/* NUVIO_GLOBAL_MEDIA_ENRICHMENT_V1:997c9d845881 */
;(function(g,c){"use strict";
...
var ok=false;try{if(typeof module!=="undefined"&&module.exports)ok=install(module.exports,"getStreams")}catch(_){}try{if(g&&typeof g.getStreams==="function"){if(ok&&typeof module!=="undefined"&&module.exports)g.getStreams=module.exports.getStreams;else install(g,"getStreams")}}catch(_){}
})(typeof globalThis!=="undefined"?globalThis:this,{"maxRows":6,"maxDepth":2,"maxCandidates":10,"timeoutMs":6500,"preserveOriginal":true,"defaultUserAgent":"Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36","implementationRevision":"scoped-playback-context-v9-browser-default-ua"});
/* CLOSEFIX:CORE.MEDIA_ENRICHMENT.V1 */

/* STARTFIX:CORE.HLS_RUNTIME_INTEGRITY.V1 */
/* FIXDATA:CORE.HLS_RUNTIME_INTEGRITY.V1: */
/* NUVIO_HLS_RUNTIME_INTEGRITY_V1:346d2e92987d */
;(function(g,config){
  "use strict";
  ...
  install();
})(typeof globalThis!=="undefined"?globalThis:this,{"timeoutMs":9000,"maxChildren":3,"maxRecoveryPages":6,"maxRecoveryCandidates":18,"implementationRevision":"native-master-facts-v10","probeAllUrls":true,"failClosedUnknown":false,"probeFirstSegmentNative":true,"nativeProbeMaxRows":8,"nativeProbeTimeoutMs":2500,"minimumVodDurationSeconds":90,"inspectMasterFacts":true});
...
/* FIXDATA:CORE.STREAM_PRESENTATION.V1: */
/* NUVIO_GLOBAL_STREAM_PRESENTATION_V1:a8afbef55d66 */
;(function(g,c){"use strict";
function s(v){return String(v==null?"":v).trim()}
function meaningful(v){var x=s(v);return x&&!/^(?:unknown|inconnue?|n\/?a|null|undefined|none|-+)$/i.test(x)}
...
var ok=false;try{if(typeof module!=="undefined"&&module.exports){ok=install(module.exports,"getStreams")||install(module.exports,"streams")}}catch(_e){}try{if(g&&typeof g.getStreams==="function"){if(ok&&typeof module!=="undefined"&&module.exports)g.getStreams=module.exports.getStreams;else install(g,"getStreams")}}catch(_e){}
})(typeof globalThis!=="undefined"?globalThis:this,{"providerId":"streamzo","providerLanguageMode":"vf","languageFallback":"VF","tmdbCoreCapabilityRequired":true,"tmdbTimeoutMs":1200,"implementationRevision":"all-providers-client-projection-evidence-language-v26"});
...
/* FIXDATA:CORE.RUNTIME_MEDIA_SAFETY.V4: */
/* NUVIO_GLOBAL_RUNTIME_MEDIA_SAFETY_V1:9381fed787f4 */
;(function(g,c){
  "use strict";
  function s(v){return String(v==null?"":v).trim()}
  function slot(v){if(Array.isArray(v))return{key:null,list:v};if(v&&typeof v==="object"){for(var i=0;i<3;i++){var k=["streams","results","data"][i];if(Array.isArray(v[k]))return{key:k,list:v[k]}}}return null}
...
/* FIXDATA:CORE.PROVIDER_BRANDING.V1:eyJpbXBsZW1lbnRhdGlvblJldmlzaW9uIjoicG9zdC1zYWZldHktdW5pZm9ybS1maW5hbC1sYWJlbC12OSIsInByb3ZpZGVyRW1vamkiOiLwn4yKIiwicHJvdmlkZXJJZCI6InN0cmVhbXpvIiwicHJvdmlkZXJOYW1lIjoiU3RyZWFtWm8ifQ== */
/* NUVIO_GLOBAL_PROVIDER_BRANDING_V1:e55df09e24e8 */
;(function(g,c){"use strict";
function slot(v){if(Array.isArray(v))return{key:null,list:v};if(v&&typeof v==="object"){for(var i=0;i<3;i++){var k=["streams","results","data"][i];if(Array.isArray(v[k]))return{key:k,list:v[k]}}}return null}
function rebuild(v,x,list){if(x.key===null)return list;var o=Object.assign({},v);o[x.key]=list;return o}
...
var ok=false;try{if(typeof module!=="undefined"&&module.exports){ok=install(module.exports,"getStreams")||install(module.exports,"streams")}}catch(_e){}try{if(g&&typeof g.getStreams==="function"){if(ok&&typeof module!=="undefined"&&module.exports)g.getStreams=module.exports.getStreams;else install(g,"getStreams")}}catch(_e){}
})(typeof globalThis!=="undefined"?globalThis:this,{"providerId":"streamzo","providerName":"StreamZo","providerEmoji":"🌊","implementationRevision":"post-safety-uniform-final-label-v9"});
