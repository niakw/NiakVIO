...
- Corrige le Learning natif pour distinguer les **capacités sémantiques** des **alias de transport** : une fixture anime jouable via `tv` ne propose plus d'ajouter `tv` comme type sémantique ; elle produit une proposition `transport_alias` non mutante, avec régression exécutée dans la suite normale.
