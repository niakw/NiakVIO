...
- Ajoute un **gate de capacité avant TMDB** : `movie` exige explicitement `provider.movie`; un provider `tv`, `anime` ou `tv+anime` sort donc à coût réseau nul sur un film. Un film anime exige `movie + anime`, puis le gate sémantique final tranche `movie/tv/anime`.
