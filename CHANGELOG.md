...
- Préserve les artefacts clients hashés comme générations immuables avec une **fenêtre glissante de 10 générations** par provider et protection des références manifest/LKG/provenance ; une révocation sécurité explicite prime toutefois sur la rétention et purge les générations historiques unsafe non référencées.
- Aligne les types média : `series/show = tv` au niveau transport, puis impose un **gate TMDB canonique avant toute exécution provider** ; la clé cache/identité est `namespace:id` (`tv:<id>` / `movie:<id>`), un même nombre peut exister dans les deux namespaces, et l'anime reste une classification sémantique plutôt qu'un namespace TMDB.
- Corrige le **gate de capacité avant TMDB** : `movie` est admissible si le provider déclare `movie` **ou** `anime`. Un provider `tv` seul sort toujours à coût réseau nul sur un film ; un provider `anime` peut recevoir les transports `movie` et `tv/series`, puis TMDB tranche `movie/tv/anime` avant tout fetch provider. Les manifests exposent donc les deux alias de transport `movie` + `tv` aux providers anime tout en conservant `canonicalSupportedTypes` séparé.
- Corrige le Learning natif pour distinguer les **capacités sémantiques** des **alias de transport** : une fixture anime jouable via `tv` ne propose plus d'ajouter `tv` comme type sémantique ; elle produit une proposition `transport_alias` non mutante lorsque l'alias manque, et aucune proposition lorsqu'il est déjà déclaré. Le test couvre explicitement `anime→tv`, `anime→movie`, nouvelle capacité sémantique et contrat déjà satisfait.
- Aligne le fixed-point de publication sur cette séparation : `reapply_published_overrides.py` compare et matérialise désormais les types sémantiques via `canonicalSupportedTypes` et les alias Nuvio via `supportedTypes`, y compris dans les projections secondaires. Cela supprime le faux `supported-types-drift:anime-sama` déclenché après un rendu correct du catalogue.
- Aligne également les ProviderBases propres et le compiler : ils préfèrent `canonicalSupportedTypes` aux alias transport, et un ProviderBase sémantiquement `anime` accepte désormais les entrées client `tv` **et** `movie` avant le gate TMDB global. Un rebuild propre ne peut donc plus réintroduire la perte des films d'anime.
- Définit chaque JS provider comme un **lecteur spécialisé** : il consomme un plan de routes déjà appris ; découverte de domaine/routes, guesses et crawl appartiennent à Discovery/Learning. HindMoviez est migré hors de `adaptive_runtime_recovery_v5` vers des `learned_routes` structurées. Movix passe également de la découverte d'API runtime à des routes `/api/catalog/...` apprises et fixes.
