...
- Aligne les types média : `series/show = tv` au niveau transport, puis impose un **gate TMDB canonique tv/anime avant toute exécution provider** ; la clé cache/identité est `namespace:id` (`tv:<id>` / `movie:<id>`) et l'anime épisodique reste dans le namespace TMDB TV.
