...
- Définit chaque JS provider comme un **lecteur spécialisé** : il consomme un plan de routes déjà appris ; découverte de domaine/routes, guesses et crawl appartiennent à Discovery/Learning. HindMoviez est migré hors de `adaptive_runtime_recovery_v5` vers des `learned_routes` structurées.
